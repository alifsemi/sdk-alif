...
- Scan for APs and verify the configured SSID is present
...
- Wi-Fi access point with the SSID/PSK configured via Kconfig
  (``WIFI_SHELL_TEST_SSID`` / ``WIFI_SHELL_TEST_PSK``). These default
  to empty and **must** be set in a board fragment, overlay, or
  ``-DCONFIG_WIFI_SHELL_TEST_SSID=...`` / ``-DCONFIG_WIFI_SHELL_TEST_PSK=...``.
...
- ``WIFI_SHELL_DHCP_TIMEOUT_SEC`` — wait for DHCP after association
