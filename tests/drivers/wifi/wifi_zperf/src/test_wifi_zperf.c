...
static int wifi_iface_state(void)
{
	...
	int ret = net_mgmt(NET_REQUEST_WIFI_IFACE_STATUS, ctx.iface, &status, sizeof(status));
...
static void ensure_connected(void)
{
	...
	zassert_equal(wifi_iface_state(), WIFI_STATE_COMPLETED,
		      "WiFi not connected");
	zassert_true(wait_for_dhcp(CONFIG_WIFI_ZPERF_DHCP_TIMEOUT_SEC),
		     "DHCP timed out");
}

static bool zperf_server_unreachable(int ret)
{
	return ret == -ETIMEDOUT || ret == -ECONNREFUSED ||
	       ret == -ENETUNREACH || ret == -EHOSTUNREACH;
}
				...
				uint16_t packet_size, uint32_t rate_kbps)
{
	...
	int ret = net_addr_pton(AF_INET, CONFIG_WIFI_ZPERF_SERVER_IP,
			&addr4->sin_addr);
	zassert_equal(ret, 0, "Invalid WIFI_ZPERF_SERVER_IP '%s' (%d)",
			CONFIG_WIFI_ZPERF_SERVER_IP, ret);
...
static uint32_t calc_throughput_kbps(struct zperf_results *results)
{
	...
	uint64_t total_bytes = results->total_len;

	if (total_bytes == 0U && results->nb_packets_sent > 0U) {
		total_bytes = (uint64_t)results->nb_packets_sent *
			      results->packet_size;
	}
...
ZTEST(wifi_zperf, test_wifi_connect)
{
	...
	zassert_true(get_sta_ipv4(&addr), "No IP address assigned");
	net_addr_ntop(AF_INET, &addr, addr_str, sizeof(addr_str));
...
ZTEST(wifi_zperf, test_tcp_upload)
{
	...
	if (zperf_server_unreachable(ret)) {
	...
	TC_PRINT("  Throughput: %u kbps (%u.%02u Mbps)\n",
		 throughput_kbps, throughput_kbps / 1000,
		 (throughput_kbps % 1000) / 10);
...
ZTEST(wifi_zperf, test_tcp_upload_large_packet)
{
	...
	TC_PRINT("  Bytes sent: %llu, Packets: %u\n",
		 results.total_len, results.nb_packets_sent);
	TC_PRINT("  Throughput: %u kbps (%u.%02u Mbps)\n",
		 throughput_kbps, throughput_kbps / 1000,
		 (throughput_kbps % 1000) / 10);
...
ZTEST(wifi_zperf, test_udp_upload)
{
	...
	if (zperf_server_unreachable(ret)) {
		TC_PRINT("UDP upload failed (%d), server unreachable, skipping\n", ret);
		ztest_test_skip();
		return;
	}
	zassert_equal(ret, 0, "UDP upload failed: %d", ret);
...
ZTEST(wifi_zperf, test_udp_upload_large_packet)
{
	...
	zassert_equal(ret, 0, "UDP upload (large pkt) failed: %d", ret);
...
ZTEST(wifi_zperf, test_tcp_throughput_stability)
{
	...
	for (int i = 0; i < iterations; i++) {
		...
		TC_PRINT("  Run %d: %u kbps (%u.%02u Mbps)\n",
			 i + 1, throughput_kbps,
	...
	TC_PRINT("  Avg: %u kbps (%u.%02u Mbps)\n",
		 avg_kbps, avg_kbps / 1000, (avg_kbps % 1000) / 10);
...
ZTEST(wifi_zperf, test_udp_packet_loss)
{
	...
	zassert_equal(ret, 0, "UDP packet loss upload failed: %d", ret);
...
ZTEST(wifi_zperf, test_udp_upload_zero_duration)
{
	...
	TC_PRINT("Result: %d, bytes sent: %llu\n", ret, results.total_len);

	zassert_true(ret == 0 || ret == -EINVAL || ret == -EOPNOTSUPP,
		     "Unexpected error for 0 duration: %d", ret);
	if (ret == 0) {
		zassert_true(results.nb_packets_sent <= 1,
			     "0 duration sent too many packets: %u",
			     results.nb_packets_sent);
	}
}
...
static void *wifi_zperf_setup(void)
{
	...
	zassert_true(strlen(CONFIG_WIFI_ZPERF_TEST_SSID) > 0U,
		     "CONFIG_WIFI_ZPERF_TEST_SSID is empty; set it in board or overlay config");
...
static void wifi_zperf_teardown(void *fixture)
{
	...
	if (ctx.iface != NULL) {
		struct wifi_iface_status status = {0};
		int ret = net_mgmt(NET_REQUEST_WIFI_IFACE_STATUS, ctx.iface,
				   &status, sizeof(status));

		if (ret == 0 && status.state == WIFI_STATE_COMPLETED) {
			k_sem_reset(&wifi_event);
			if (wifi_disconnect() == 0) {
				(void)k_sem_take(&wifi_event, K_SECONDS(10));
			}
		}
