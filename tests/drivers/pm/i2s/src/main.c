...
/**
 * As per the test requirements, unused memory blocks can be removed.
 */
...
/**
 * Set the RUN profile parameters for this test.
 */
...
static void app_pm_lock_deeper_states(bool lock)
{
	...
	if (S2RAM_SUPPORTED) {
		...
		if (!pm_policy_state_lock_is_active(PM_STATE_SUSPEND_TO_RAM,
PM_ALL_SUBSTATES)) {
...
void *setup(void)
{
	...
	if (is_mram_boot) {
		LOG_INF("%s RTSS_HE (MRAM boot): I2S PM test "
			"(RUNTIME_IDLE, SUSPEND_TO_IDLE, SOFT_OFF)",
			CONFIG_BOARD);
	} else {
		LOG_INF("%s RTSS_HE (TCM boot): I2S PM test "
			"(RUNTIME_IDLE, SUSPEND_TO_IDLE, S2RAM)",
			CONFIG_BOARD);
	}
#else
	LOG_INF("%s RTSS_HP: I2S PM test "
		"(RUNTIME_IDLE, SUSPEND_TO_IDLE, SOFT_OFF)",
		CONFIG_BOARD);
#endif
...
{
	zassert_true(i2s_start(i2s_dev_rx, i2s_dev_tx), "Failed to start I2S");
...
#if defined(CONFIG_RTSS_HE)
	if (!S2RAM_SUPPORTED) {
		LOG_INF("=== TEST 2: S2RAM_STANDBY SKIPPED (MRAM boot) ===");
		ztest_test_skip();
	}
...
#elif defined(CONFIG_RTSS_HP)
	LOG_INF("=== TEST 2: S2RAM_STANDBY SKIPPED (not supported on HP) ===");
	ztest_test_skip();
#endif
}
...
ZTEST(i2s_test_POS_04_S2RAM_STANDBY, test_POS_05_RUNTIME_IDLE_THEN_S2RAM_STANDBY)
{
	...
	if (S2RAM_SUPPORTED) {
		...
		LOG_INF("=== TEST 5.2: S2RAM_STANDBY SKIPPED (S2RAM not supported) ===");
		ztest_test_skip();
	}
...
ZTEST(i2s_test_POS_04_S2RAM_STANDBY, test_POS_06_S2RAM_STANDBY_NO_SOFT_OFF)
{
	...
	if (S2RAM_SUPPORTED) {
		...
		zassert_false(
			...
			"S2RAM should be unlocked during STANDBY test");
		...
		LOG_INF("=== TEST 6: S2RAM_STANDBY SKIPPED (S2RAM not supported) ===");
		ztest_test_skip();
	}
...
#if defined(CONFIG_RTSS_HE)
	if (!S2RAM_SUPPORTED) {
		LOG_INF("=== TEST 2: S2RAM_STOP SKIPPED (MRAM boot) ===");
		ztest_test_skip();
	}
...
#elif defined(CONFIG_RTSS_HP)
	LOG_INF("=== TEST 2: S2RAM_STOP SKIPPED (not supported on HP) ===");
	ztest_test_skip();
#endif
}
