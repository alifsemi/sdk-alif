...
int spi_pm_thread_init(void)
{
	...
	k_tid_t tids = k_thread_create(&thread_s, SlaveT_stack, STACKSIZE,
			&slave_spi, NULL, NULL, NULL,
			SLAVE_PRIORITY, 0, K_FOREVER);
	if (tids == NULL) {
		LOG_ERR("Error creating Slave Thread");
	}

	k_tid_t tidm = k_thread_create(&thread_m, MasterT_stack, STACKSIZE,
			&master_spi, NULL, NULL, NULL,
			MASTER_PRIORITY, 0, K_FOREVER);
	if (tidm == NULL) {
