...
cores while an SPI master/slave loopback transfer runs concurrently in the
...
loopback (one SPI controller instance acting as master, another as slave):

* **Master thread**: waits for the slave, then performs an SPI transceive.
* **Slave thread**: signals the master and performs the matching SPI transceive.
...
* Two SPI controller instances wired/loopback-configured as master and slave
  (``master-spi`` / ``slave-spi`` aliases, see the snippet overlay files)
...
* **Debugger**: Disconnect debugger before testing - it prevents cores from entering
  OFF states
* **SPI Loopback**: The master and slave SPI controllers must be wired for loopback
