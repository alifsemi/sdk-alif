...
#define SPI_FLASH_SECTOR_4_OFFSET (4 * SPI_FLASH_SECTOR_SIZE)
#define SPI_FLASH_SECTOR_5_OFFSET (5 * SPI_FLASH_SECTOR_SIZE)
...
static void single_sector_test(void)
{
	...
	LOG_INF("Write operation completed in %lld milliseconds (%u cycles)\n",
...
static void multi_page_test(const struct device *dev)
{
	...
	LOG_INF("Write operation test 3: completed in %lld milliseconds\n", tm_taken_ms);
...
static void multi_sector_test(void)
{
	...
	zassert_equal(e_count, 0,
		"Sector 4 data verification failed - %d mismatches found", e_count);
