...
``snps,designware-ospi`` as a compatible.
The tests assume that the OSPI flash node is available with the ``spi_flash0``
device tree alias (set by the ospi-flash snippet overlays).
