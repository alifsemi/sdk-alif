...
.. note::
   ...
   Lease-dependent cases live in the ``alif_eth_dhcpv4`` suite. Its ztest
   ``before()`` hook calls an idempotent ``ensure_dhcp_bound()`` helper so
   each case starts with a lease and the helper is not repeated in every
   test body. Probe-only cases (build, interface detection, NTP callback
   registration) live in ``alif_eth_dhcpv4_bringup`` and do not start DHCP.
