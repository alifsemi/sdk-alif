...
/*
 * Suite before() hook: make sure a DHCP lease is in place for the next case.
 * Idempotent and non-destructive: if the client is already bound the lease
 * is reused. Only a missing lease triggers stop/start, with a short settle
 * so the DWMAC RX path can re-arm before DISCOVER/OFFER.
 */
...
static void eth_dhcpv4_before(void *fixture)
{
	ARG_UNUSED(fixture);
...
ZTEST(alif_eth_dhcpv4_bringup, test_eth_dhcpv4_client_build_alif_board)
{
...
ZTEST(alif_eth_dhcpv4_bringup, test_eth_dhcpv4_ethernet_interface_detection)
{
	struct net_if *iface;
...
ZTEST(alif_eth_dhcpv4_bringup, test_eth_dhcpv4_ntp_option_callback)
{
...
ZTEST(alif_eth_dhcpv4, test_eth_dhcpv4_invalid_ntp_option_length_negative)
{
	...
	/*
	 * Wait one wall-clock second for the stack to handle the short
	 * buffer. K_SECONDS(1) is used instead of K_MSEC(): k_sleep()
	 * converts to SYS_CLOCK_TICKS_PER_SEC ticks, and a small K_MSEC()
	 * can round to 0 ticks (or one coarse tick) on a slow clock.
	 * One second is at least one tick at any legal tick rate.
	 */
	k_sleep(K_SECONDS(1));
...
ZTEST(alif_eth_dhcpv4_manual_cable, test_eth_dhcpv4_cable_disconnect_after_bind)
{
	...
	for (int i = 0; i < CABLE_WAIT_SECONDS; i++) {
		if (!net_if_is_carrier_ok(eth_iface)) {
			dropped = true;
			break;
		}
...
ZTEST_SUITE(alif_eth_dhcpv4_manual_cable, NULL, eth_dhcpv4_setup, NULL, NULL,
	    eth_dhcpv4_teardown);
#endif
/* Probe / API cases that must not start DHCP (no lease required). */
ZTEST_SUITE(alif_eth_dhcpv4_bringup, NULL, eth_dhcpv4_setup, NULL, NULL,
	    NULL);
/* Lease-dependent cases: before() reuses or acquires a DHCP lease. */
ZTEST_SUITE(alif_eth_dhcpv4, NULL, eth_dhcpv4_setup, eth_dhcpv4_before, NULL,
	    eth_dhcpv4_teardown);
