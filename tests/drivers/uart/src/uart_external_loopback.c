...
static int test_Externalloopback_interrupt(const uint8_t *tx_buf, int size,
					  const char *str)
{
	int ret;
...
/* ZTC-162: External loopback for transmitting strings */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackString)
{
	...
	int ret = test_Externalloopback_interrupt(tx_string, size, "String-32B");
...
/* ZTC-161: External loopback for transmitting numbers */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackNumbers)
{
	...
	int ret = test_Externalloopback_interrupt(tx_num, size, "Numbers");
...
/* ZTC-160: External loopback for transmitting binary data (32 bytes) */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackBinaryData)
{
	...
	int size = FIFO_DEPTH;
	int ret = test_Externalloopback_interrupt(tx_binary, size, "Binary-32B");
...
/* ZTC-183: External loopback for transmitting ASCII data (32 bytes) */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackAsciiData)
{
	...
	int ret = test_Externalloopback_interrupt((unsigned char *)tx_ascii,
						 len, "ASCII-32B");
...
/* External loopback for transmitting HEX data (32 bytes) */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackHexData)
{
	...
	int ret = test_Externalloopback_interrupt(tx_hex, size, "HEX-32B");
...
/* External loopback: multi-baud sweep */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackMultiBaud)
{
	...
	for (int i = 0; i < ARRAY_SIZE(bauds); i++) {
		...
		int ret = test_Externalloopback_interrupt(tx_buf, size,
							  "MultiBaud");
		if (ret != 0) {
			count++;
...
/* External loopback: stop-bit variations */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackStopBits)
{
	...
	for (int i = UART_CFG_STOP_BITS_0_5; i <= UART_CFG_STOP_BITS_2; i++) {
		...
		int ret = test_Externalloopback_interrupt(tx_buf, size,
							  "StopBits");
		if (ret != 0) {
			TC_PRINT("External loopback stop_bits %d -> SKIPPED (unsupported)\n",
				 i);
		} else {
			tested++;
	...
	TC_PRINT("Stop-bits: %d configs tested successfully\n", tested);
	zassert_true(tested > 0, "No stop-bit configurations were tested successfully");
}

/* External loopback: parity variations */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackParity)
{
	...
	for (int i = UART_CFG_PARITY_NONE; i <= UART_CFG_PARITY_SPACE; i++) {
		...
		int ret = test_Externalloopback_interrupt(tx_buf, size,
							  "Parity");
		if (ret != 0) {
			TC_PRINT("External loopback parity %d -> SKIPPED (unsupported)\n",
				 i);
		} else {
			tested++;
	...
	TC_PRINT("Parity: %d configs tested successfully\n", tested);
	zassert_true(tested > 0, "No parity configurations were tested successfully");
}

/* External loopback: back-to-back transfers (31, 32 bytes) */
ZTEST(uart_Externalloopback, test_Interrupt_ExternalloopbackBackToBack)
{
	...
	ext_lb_set_config(get_uart_baudrate_from_dt());
	int ret = test_Externalloopback_interrupt(tx_a, size_a, "BackToBack-31B");

	zassert_equal(ret, 0, "Back-to-back transfer 1 (31B) failed");

	ext_lb_set_config(get_uart_baudrate_from_dt());
	ret = test_Externalloopback_interrupt(tx_b, size_b, "BackToBack-32B");
