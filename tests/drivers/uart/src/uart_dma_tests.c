			...
			void *user_data)
{
	...
	case UART_RX_RDY: {
		...
		if (k_msgq_put(&rx_msgq, &chunk, K_NO_WAIT) != 0) {
			TC_PRINT("RX msgq overflow: dropping UART_RX_RDY fragment\n");
		}
		break;
	}
	case UART_TX_DONE:
			 ...
			 size_t len)
{
	...
	ret = k_sem_take(&tx_done_sem, K_SECONDS(5));
	return ret;
}
...
static void uart_dma_after(void *fixture)
{
	...
	if (uart_rx_disable(dma_uart_dev) == 0) {
		(void)k_sem_take(&rx_disabled_sem, K_SECONDS(1));
	}
}
 ...
 *
 * Follows tests/drivers/uart/uart_async_api: enable RX, complete a short
 * TX so the channel is known-good, start a longer TX and abort it at
 * once. Do not sleep first - after the UART FIFO fills, PL330 sits in
 * DMAWFP (wait-for-peripheral). DMAKILL from that state is taken with
 * IRQs locked inside dma_pl330_transfer_stop() and can freeze the
 * console / never return.
 */
ZTEST(uart_dma, test_dma_tx_abort)
{
	...
	ret = uart_rx_enable(dma_uart_dev, rx_dma_buf_a, RX_BUF_SIZE,
			     SYS_FOREVER_US);
	zassert_equal(ret, 0, "uart_rx_enable failed (ret=%d)", ret);

	/* Prime: a short transfer that is allowed to finish. */
	ret = uart_tx(dma_uart_dev, tx_abort_buf, 8, SYS_FOREVER_US);
	zassert_equal(ret, 0, "prime uart_tx failed (ret=%d)", ret);
	ret = k_sem_take(&tx_done_sem, K_MSEC(1000));
	zassert_equal(ret, 0, "prime TX_DONE timeout");
	zassert_equal(last_tx_evt, UART_TX_DONE,
		      "prime TX expected TX_DONE, got %d", last_tx_evt);

	last_tx_evt = -1;
	k_sem_reset(&tx_done_sem);

	ret = uart_tx(dma_uart_dev, tx_abort_buf, sizeof(tx_abort_buf),
		      SYS_FOREVER_US);
	zassert_equal(ret, 0, "uart_tx failed (ret=%d)", ret);
