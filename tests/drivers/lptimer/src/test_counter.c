...
/* Variables declared as extern in test_lptimer.h */
struct k_sem top_cnt_sem;
volatile uint32_t top_cnt;
struct k_sem alarm_cnt_sem;
...
static void test_single_shot_alarm_instance(const struct device *dev, bool set_top)
{
	...
	int ret_sem = k_sem_take(&alarm_cnt_sem, K_MSEC(5000));

	zassert_equal(0, ret_sem, "%s:Alarm did not fire within timeout", dev->name);
...
static void test_alarm_32mhz_clock_input_instance(const struct device *dev)
{
	int err, ret;
	...
	/* Wait for alarm callback with timeout */
	int ret_sem = k_sem_take(&alarm_cnt_sem, K_MSEC(5000));
...
ZTEST(test_LPTimer, test_alarm_32mhz_clock_input)
{
	test_all_instances(test_alarm_32mhz_clock_input_instance,
...
ZTEST(lptimer_128k_clock_source, test_lptimer_128k_clock_source)
{
	...
	test_all_instances(test_alarm_32mhz_clock_input_instance, reliable_cancel_capable);
}
#endif
...
static void test_alarm_stress(const struct device *dev)
{
	...
	for (int i = 0; i < STRESS_TEST_COUNT; i++) {
		...
		/* Wait for alarm with shorter timeout */
		k_sleep(K_MSEC(3000));
