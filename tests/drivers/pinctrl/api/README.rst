...
Validation uses Zephyr pinctrl APIs only (``pinctrl_lookup_state()``,
...
use ``sys_read32()`` or other register-level access.
...
#. ``test_pad_config_fields`` -- REN, STE, SRE, DSC, ODS, and DRV fields
   in the encoded state, then apply via ``pinctrl_apply_state()``.
