...
static const struct gpio_dt_spec stress_pin =
...
ZTEST(gpio_stress, test_gpio_toggle_stress)
{
	...
	TC_PRINT("Toggle stress: %u iters in %u us (%u avg)\n",
		 TOGGLE_STRESS_ITERATIONS, duration_us,
		 TOGGLE_STRESS_ITERATIONS ?
		 (duration_us / TOGGLE_STRESS_ITERATIONS) : 0U);
}
...
ZTEST(gpio_stress, test_gpio_pin_set_performance)
{
	...
	TC_PRINT("pin_set_dt: %u iters %u cyc %u cyc/call\n",
...
ZTEST(gpio_stress, test_gpio_pin_get_performance)
{
	...
	TC_PRINT("pin_get_dt: %u iters %u cyc %u cyc/call\n",
		 PERF_ITERATIONS, total,
		 avg_cycles(total, PERF_ITERATIONS));
}
...
ZTEST(gpio_stress, test_gpio_pin_toggle_performance)
{
	...
	TC_PRINT("pin_toggle_dt: %u iters %u cyc %u cyc/call\n",
		 PERF_ITERATIONS, total,
		 avg_cycles(total, PERF_ITERATIONS));
}
...
ZTEST(gpio_stress, test_gpio_interrupt_storm)
{
	...
	k_msleep(50);
	observed = atomic_get(&storm_cb_cnt);
	...
	zassert_not_equal(observed, 0U,
			  "No callbacks; check led0->led1 jumper");
