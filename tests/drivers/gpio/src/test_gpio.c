...
{
	if (ret == -ENOTSUP) {
		TC_PRINT("gpio_pin_is_input_dt not supported\n");
		ztest_test_skip();
		return -1;
	}
	if (ret < 0) {
		TC_PRINT("gpio_pin_is_input_dt err: %d\n", ret);
		return -1;
	}
	return (ret == 1) ? 0 : -1;
}
...
{
	if (ret == -ENOTSUP) {
		TC_PRINT("gpio_pin_is_output_dt not supported\n");
		ztest_test_skip();
		return -1;
	}
	if (ret < 0) {
		TC_PRINT("gpio_pin_is_output_dt err: %d\n", ret);
		return -1;
	}
	return (ret == 1) ? 0 : -1;
}
...
{
	static const struct gpio_dt_spec gpio_port =
...
ZTEST(gpio_check, test_gpio_reconfigure_cycles)
{
	...
	TC_PRINT("Completed %u reconfigure cycles OK\n",
		 RECONFIGURE_CYCLES);
}
