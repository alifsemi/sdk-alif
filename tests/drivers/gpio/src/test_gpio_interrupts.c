...
static const struct gpio_dt_spec gpio_in1 =
	GPIO_DT_SPEC_GET(GPIO_NODE, gpios);
static const struct gpio_dt_spec gpio_in2 =
	GPIO_DT_SPEC_GET(DT_ALIAS(led2), gpios);

static atomic_t cb_multi_in1_count;
static atomic_t cb_multi_in2_count;

static void callback_multi_in1(const struct device *port,
			       ...
			       gpio_port_pins_t pins)
{
	...
	if (pins & BIT(gpio_in1.pin)) {
		atomic_inc(&cb_multi_in1_count);
	}
}

static void callback_multi_in2(const struct device *port,
			       struct gpio_callback *cb,
			       gpio_port_pins_t pins)
{
	ARG_UNUSED(port);
...
ZTEST(gpio_test_interrupts, test_gpio_multi_input_single_output_interrupt)
{
	...
	zassert_true(gpio_is_ready_dt(&gpio_in1),
		     "GPIO input1 dev is not ready");
	zassert_true(gpio_is_ready_dt(&gpio_in2),
		     "GPIO input2 dev is not ready");
	zassert_true(gpio_is_ready_dt(&led), "GPIO output dev is not ready");

	ret = gpio_pin_interrupt_configure_dt(&gpio_in1, GPIO_INT_DISABLE);
	zassert_equal(check_int_conf(ret), 0,
		      "Disable input1 interrupt failed");
	ret = gpio_pin_interrupt_configure_dt(&gpio_in2, GPIO_INT_DISABLE);
	zassert_equal(check_int_conf(ret), 0,
		      "Disable input2 interrupt failed");

	ret = gpio_pin_configure_dt(&gpio_in1, GPIO_INPUT);
	zassert_equal(check_pin_conf(ret), 0,
		      "Configure input1 pin failed");
	ret = gpio_pin_configure_dt(&gpio_in2, GPIO_INPUT);
	zassert_equal(check_pin_conf(ret), 0,
	...
	gpio_init_callback(&gpio_cb_in1, callback_multi_in1,
			   BIT(gpio_in1.pin));
	ret = gpio_add_callback(gpio_in1.port, &gpio_cb_in1);
	zassert_equal(check_cb_res(ret), 0,
		      "Add callback for input1 failed");

	gpio_init_callback(&gpio_cb_in2, callback_multi_in2,
			   BIT(gpio_in2.pin));
	ret = gpio_add_callback(gpio_in2.port, &gpio_cb_in2);
	zassert_equal(check_cb_res(ret), 0,
	...
	zassert_true(in1_initial >= 0,
		     "gpio_pin_get_dt(in1) failed: %d", in1_initial);
	in2_initial = gpio_pin_get_dt(&gpio_in2);
	zassert_true(in2_initial >= 0,
		     "gpio_pin_get_dt(in2) failed: %d", in2_initial);

	atomic_set(&cb_multi_in1_count, 0);
	atomic_set(&cb_multi_in2_count, 0);
	for (int i = 0; i < 10; i++) {
		drive_led_edges(1);
	...
	if (!in2_toggled) {
		TC_PRINT("Input2 (led2) did not toggle; "
			 "check wiring led0->led2.\n");
		/* Cleanup before skip */
		gpio_pin_interrupt_configure_dt(&gpio_in1, GPIO_INT_DISABLE);
		gpio_pin_interrupt_configure_dt(&gpio_in2, GPIO_INT_DISABLE);
		gpio_remove_callback(gpio_in1.port, &gpio_cb_in1);
		gpio_remove_callback(gpio_in2.port, &gpio_cb_in2);
		gpio_pin_configure_dt(&led, GPIO_INPUT);
		ztest_test_skip();
	}

	zassert_true(in1_toggled, "Input1 did not toggle; check wiring");
	zassert_true(atomic_get(&cb_multi_in1_count) > 0,
		     "No interrupt seen on input1");
	zassert_true(atomic_get(&cb_multi_in2_count) > 0,
		     "No interrupt seen on input2");
	...
	ret = gpio_remove_callback(gpio_in1.port, &gpio_cb_in1);
	zassert_equal(check_cb_res(ret), 0,
		      "Remove callback for input1 failed");
	ret = gpio_remove_callback(gpio_in2.port, &gpio_cb_in2);
	zassert_equal(check_cb_res(ret), 0,
		      "Remove callback for input2 failed");

	/* Restore led0 to input/Hi-Z to avoid later conflicts */
	ret = gpio_pin_configure_dt(&led, GPIO_INPUT);
	zassert_equal(check_pin_conf(ret), 0,
		      "Restore output pin to input failed");
}
...
ZTEST(gpio_test_interrupts, test_gpio_multi_input_single_output_interrupt)
{
	TC_PRINT("led2 alias not present; "
		 "skipping multi-input test.\n");
	ztest_test_skip();
}
