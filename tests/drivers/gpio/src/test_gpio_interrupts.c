...
static volatile int cb_multi_in1_count;
static volatile int cb_multi_in2_count;
...
ZTEST(gpio_test_interrupts, test_gpio_multi_input_single_output_interrupt)
{
	...
	/* Restore led0 to input/Hi-Z to avoid conflicts with later tests */
	zassert_equal(check_pin_conf(gpio_pin_configure_dt(&led, GPIO_INPUT)), 0,
