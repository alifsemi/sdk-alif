...
**cdc_acm_qa suite (15 tests) — requires USB hardware:**
...
- line control: DTR read, baudrate read, RTS read, DCD set, DSR set
  (consolidated into a single ``test_line_ctrl``; DCD/DSR set
   are guarded on DTR=1 to avoid blocking the interrupt endpoint)
...
.. code-block:: console
   ...
   west build -b alif_e7_dk/ae722f80f55d5xx/rtss_hp \
       tests/drivers/usb/cdc-acm/ \
       -DDTC_OVERLAY_FILE=$PWD/tests/drivers/usb/cdc-acm/boards/alif_usb.overlay
