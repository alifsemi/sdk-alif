 ...
 * Line Control - DTR
 *
 * Purpose : Read the Data Terminal Ready (DTR) modem control signal.
 *           DTR is driven by the USB host; it is asserted (1) when a
 *           host terminal application opens the CDC-ACM port.
 * Method  : Call uart_line_ctrl_get() with UART_LINE_CTRL_DTR.
 * Pass    : The API returns 0 (success); DTR value is printed for info.
 * Fail    : The API returns a non-zero error code.
 */
ZTEST_F(cdc_acm_qa, test_line_ctrl_dtr)
{
	uint32_t dtr = 0;
	int ret;
	...
	TC_PRINT("DTR state: %u (host-driven)\n", dtr);
}

/*
 * Line Control - Baudrate
 *
 * Purpose : Retrieve the virtual baud rate configured by the USB host
 *           through the CDC SET_LINE_CODING request.
 * Method  : Call uart_line_ctrl_get() with UART_LINE_CTRL_BAUD_RATE.
 *           The call may return -ENOTSUP if the driver does not track
 *           the host-set baud rate.
 * Pass    : The API returns 0, baudrate is non-zero, and the value is printed.
 * Skip    : The API returns -ENOTSUP (driver does not track host baud rate).
 * Fail    : The API returns any other error code.
 */
ZTEST_F(cdc_acm_qa, test_line_ctrl_baudrate)
{
	uint32_t baudrate = 0;
	int ret;

	ret = uart_line_ctrl_get(fixture->cdc_dev, UART_LINE_CTRL_BAUD_RATE, &baudrate);
	if (ret == -ENOTSUP) {
		TC_PRINT("Baudrate query not supported by driver, skipping\n");
		ztest_test_skip();
		return;
	}
	zassert_equal(ret, 0, "Failed to get baudrate (%d)", ret);
	zassert_not_equal(baudrate, 0, "Baudrate should not be zero on success");
	TC_PRINT("Baudrate: %u\n", baudrate);
}
 ...
 * Line Control - RTS
 *
 * Purpose : Read the Request To Send (RTS) modem control signal.
 *           Like DTR, RTS is driven by the USB host.
 * Method  : Call uart_line_ctrl_get() with UART_LINE_CTRL_RTS.
 *           The call may return -ENOTSUP if the driver does not
 *           expose RTS.
 * Pass    : The API returns 0 and the RTS value is printed.
 * Skip    : The API returns -ENOTSUP (driver does not expose RTS).
 * Fail    : The API returns any other unexpected error code.
 */
ZTEST_F(cdc_acm_qa, test_line_ctrl_rts)
{
	uint32_t rts = 0;
	int ret;

	ret = uart_line_ctrl_get(fixture->cdc_dev, UART_LINE_CTRL_RTS, &rts);
	if (ret == -ENOTSUP) {
		TC_PRINT("RTS query not supported by driver, skipping\n");
		ztest_test_skip();
		return;
	}
	zassert_equal(ret, 0, "Failed to get RTS (%d)", ret);
	TC_PRINT("RTS state: %u\n", rts);
}

/*
 * Line Control - DCD Set
 *
 * Purpose : Assert the Data Carrier Detect (DCD) signal towards the host
 *           by sending a CDC SERIAL_STATE notification on the interrupt
 *           endpoint.
 * Precond : A USB host must be connected and DTR must be asserted;
 *           otherwise the interrupt-endpoint transfer may block forever.
 *           The test skips when DTR == 0.
 * Method  : Call uart_line_ctrl_set() with UART_LINE_CTRL_DCD = 1.
 * Pass    : The API returns 0 (DCD notification sent to host).
 * Skip    : -ENOTSUP (driver does not implement DCD signaling) or
 *           -EACCES (operation not permitted in current state) or
 *           no host connected (DTR == 0).
 * Fail    : The API returns any other error code.
 */
ZTEST_F(cdc_acm_qa, test_line_ctrl_set_dcd)
{
	int ret;
	uint32_t dtr = 0;

	/*
	 * uart_line_ctrl_set(DCD) sends a CDC serial state notification
	 * via the interrupt endpoint. If no host is connected/polling,
	 * the transfer blocks forever. Skip when DTR is 0 (no host).
	 */
	ret = uart_line_ctrl_get(fixture->cdc_dev, UART_LINE_CTRL_DTR, &dtr);
	if (ret != 0 || dtr == 0) {
		TC_PRINT("No host connected (DTR=0), skipping DCD set\n");
		ztest_test_skip();
		return;
	}
	...
	if (ret == -ENOTSUP) {
		TC_PRINT("DCD signaling not supported by driver, skipping\n");
		ztest_test_skip();
		return;
	}
	if (ret == -EACCES) {
		TC_PRINT("DCD set not permitted in current state, skipping\n");
		ztest_test_skip();
		return;
	}
...
/*
 * Line Control - DSR Set
 *
 * Purpose : Assert the Data Set Ready (DSR) signal towards the host
 *           by sending a CDC SERIAL_STATE notification on the interrupt
 *           endpoint.
 * Precond : Same as Line Control - DCD Set — a USB host must be connected with
 *           DTR asserted; otherwise the transfer blocks forever.
 * Method  : Call uart_line_ctrl_set() with UART_LINE_CTRL_DSR = 1.
 * Pass    : The API returns 0 (DSR notification sent to host).
 * Skip    : -ENOTSUP (driver does not implement DSR signaling) or
 *           -EACCES (operation not permitted in current state) or
 *           no host connected (DTR == 0).
 * Fail    : The API returns any other error code.
 */
ZTEST_F(cdc_acm_qa, test_line_ctrl_set_dsr)
{
	int ret;
	uint32_t dtr = 0;

	/*
	 * uart_line_ctrl_set(DSR) sends a CDC serial state notification
	 * via the interrupt endpoint. If no host is connected/polling,
	 * the transfer blocks forever. Skip when DTR is 0 (no host).
	 */
	ret = uart_line_ctrl_get(fixture->cdc_dev, UART_LINE_CTRL_DTR, &dtr);
	if (ret != 0 || dtr == 0) {
		TC_PRINT("No host connected (DTR=0), skipping DSR set\n");
		ztest_test_skip();
		return;
	}
	...
	if (ret == -ENOTSUP) {
		TC_PRINT("DSR signaling not supported by driver, skipping\n");
		ztest_test_skip();
		return;
	}
	if (ret == -EACCES) {
		TC_PRINT("DSR set not permitted in current state, skipping\n");
		ztest_test_skip();
		return;
	}
