...
the card and initializes the USB device stack, including
``USBD_DEFINE_MSC_LUN(sd, "SD", ...)`` so the card is registered as the MSC
LUN. ``usbd_enable()`` is deferred until after all device-side FAT tests.
Those tests then unmount ``/SD:`` and enable USB so the host is the only FAT
writer. Suite setup also runs an SDHC write warm-up to absorb post-boot
CMD24 error bursts. After the suite finishes, USB MSC stays enabled so the
host can mount the drive.
...
- SD mount and disk info: ``fs_statvfs``, ``disk_access`` init/status,
  ...
  (never writes sector 0), write-protect check
...
- USB init and descriptors: unmount device FS, ``usbd_enable``, speed, PID
- USB left enabled for the host (no device-side FAT writes)
...
The fixture mounts ``--block-dev`` and fails if another filesystem is already
on ``--mount-point``. Host coverage:

- VID:PID ``2fe3:0008`` and MSC interface class
- File create, read, delete, directories, LFN
- 10 MB file MD5 integrity and 20 small files
- Overwrite and unmount/remount persistence
- Alignment sizes 1, 511, 512, 513, 1024 bytes
- Stress: 50 x 100 KB files
