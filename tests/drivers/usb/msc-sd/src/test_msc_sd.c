 ...
 *   1. SD mount + disk info     5. Overwrite + persistence
 *   2. File CRUD                6. Regulator + stress
 *   3. Directory + LFN          7. USB init + descriptors
 *   4. Data integrity + ADMA    8. USB enable (host exclusive)
 *
 * Filesystem tests run first. USB is enabled only after the device
 * unmounts the card so the host is the sole FAT writer.
 */
...
/*
 * Unmount the device FAT view, then enable USB. The host must be the
 * only writer once the card is exposed as MSC.
 */
static int msc_usb_enable_host_exclusive(void)
{
	int err;

	err = msc_ensure_unmounted();
	if (err != 0) {
		TC_PRINT("Unmount before USB enable failed (%d)\n", err);
		return err;
	}
	return msc_usb_ensure_enabled();
}
...
static int write_test_file(const char *path, const void *data, size_t len)
{
	...
	if (usb_init_ok) {
		TC_PRINT("Refuse device FS write while USB MSC is enabled\n");
		return -EBUSY;
	}
...
static void *msc_suite_init(void)
{
	...
	/*
	 * Do not usbd_enable() here. Device-side FAT I/O and the host MSC
	 * driver must not share the card. USB is enabled only after FS
	 * tests, with the volume unmounted, and again in suite teardown.
	 */
	usb_init_ok = false;
...
ZTEST(msc_sd_qa, test_sd_mount_and_disk_info)
{
	...
	/* Write-protect check before USB is enabled */
...
/* ========================================================================
 * 4. test_data_integrity_and_alignment
 *
...
/* ========================================================================
 * 5. test_overwrite_and_persistence
 *
...
/* ========================================================================
 * 7. test_usb_init_and_descriptors
 *
 * USB init, enumeration, speed, PID. Device FS is unmounted first so
 * the host is the only FAT writer.
 * ========================================================================
 */
ZTEST(msc_sd_qa, test_usb_init_and_descriptors)
{
	...
	ret = msc_usb_enable_host_exclusive();
...
#endif
	TC_PRINT("USB init + descriptors verified (device FS unmounted)\n");
}

/* ========================================================================
 * 8. test_usb_disconnect_reconnect
 *
 * Keep USB enabled for the host. Do not write via the device FAT mount.
 * usbd_disable() is skipped due to DWC3 halt issues.
 * ========================================================================
 */
ZTEST(msc_sd_qa, test_usb_disconnect_reconnect)
{
	...
	err = msc_usb_enable_host_exclusive();
	 ...
	 * Do not write via the device FAT mount while USB is enabled.
	 */
	TC_PRINT("Skipping usbd_disable: DWC3 halt would drop host MSC\n");
	zassert_false(sd_mounted,
		      "Device FS still mounted while USB MSC is enabled");
	zassert_false(usbd_is_suspended(msc_usbd_ctx),
		      "USB unexpectedly suspended");
	TC_PRINT("USB left enabled for the host (device FS unmounted)\n");
}
