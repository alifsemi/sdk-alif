...
(``USBD_DEFINE_MSC_LUN(ram, "RAM", ...)``). ``usbd_enable()`` is deferred
until after device-side FAT tests. The volume is then unmounted and USB is
enabled so the host is the only FAT writer. After the suite finishes, USB MSC
stays enabled so the host can mount the drive.
...
- Overwrite, volatile, stress: overwrite; unmount/remount persistence within
  the session; 5 file I/O stress cycles
