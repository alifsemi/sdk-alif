 ...
 *   1. RAMDisk mount + disk access  4. Data integrity
 *   2. File CRUD                    5. Overwrite + volatile + stress
 *   3. Directory + LFN              6. USB init (host exclusive)
 *
...
ZTEST(msc_ramdisk_qa, test_reconnect_overwrite_volatile_stress)
{
	...
	zassert_true(ram_mounted, "RAM disk not mounted");
	zassert_false(usb_init_ok, "USB already enabled");
...
/* ========================================================================
 * 6. test_usb_init_and_descriptors
 *
 * Enable USB after device FS tests. Unmount first so the host is the
 * only FAT writer.
 * ========================================================================
 */
ZTEST(msc_ramdisk_qa, test_usb_init_and_descriptors)
{
	...
	zassert_equal(msc_usb_enable_host_exclusive(), 0, "usbd_enable failed");
