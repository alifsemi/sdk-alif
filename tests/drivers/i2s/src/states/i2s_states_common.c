...
char __nocache __aligned(WB_UP(4))
	_k_mem_slab_buf_mem_slab[(BLOCK_COUNT) * WB_UP(BLOCK_SIZE)];
STRUCT_SECTION_ITERABLE(k_mem_slab, mem_slab) =
	Z_MEM_SLAB_INITIALIZER(mem_slab, _k_mem_slab_buf_mem_slab,
			       WB_UP(BLOCK_SIZE), BLOCK_COUNT);
K_MEM_SLAB_DEFINE(rx_mem_slab, BLOCK_SIZE_D, BLOCK_COUNT, 4);
K_MEM_SLAB_DEFINE(tx_mem_slab, BLOCK_SIZE_D, BLOCK_COUNT, 4);
...
{
	for (int i = 0; i < SAMPLE_BIT_WIDTH; i++) {
		tx_block[2 * i] = val_l;
			       ...
			       struct k_mem_slab *slab)
{
	...
	ret = k_mem_slab_alloc(slab, (void **)&tx_block, K_NO_WAIT);
	if (ret < 0) {
		TC_PRINT("Error: tx slab alloc failed (%d)\n", ret);
		return -TC_FAIL;
	}

	fill_buf((int16_t *)tx_block, att);
	ret = i2s_write(dev_i2s, tx_block, BLOCK_SIZE_D);
	if (ret != err) {
		TC_PRINT("Error: i2s_write failed expected %d, actual %d\n",
			 err, ret);
		k_mem_slab_free(slab, tx_block);
		return -TC_FAIL;
	}
			      ...
			      struct k_mem_slab *slab)
{
	...
	if (ret < 0 || rx_size != BLOCK_SIZE_D) {
		TC_PRINT("Error: Read failed\n");
		return -TC_FAIL;
	}
	ret = verify_buf((int16_t *)rx_block, att);
...
int configure_stream(const struct device *dev_i2s, enum i2s_dir dir)
{
	...
	i2s_cfg.block_size = BLOCK_SIZE_D;
