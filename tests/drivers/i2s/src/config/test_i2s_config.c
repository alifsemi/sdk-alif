...
LOG_MODULE_REGISTER(i2s_config_test, LOG_LEVEL_INF);
...
ZTEST(i2s_config, test_config_sampling_freq_matrix)
{
	...
	for (size_t r = 0; r < ARRAY_SIZE(rates); r++) {
		for (size_t b = 0; b < ARRAY_SIZE(word_sizes); b++) {
			...
			if (ret == 0) {
				passes++;
				LOG_INF("PASS cfg rate=%u bits=%u\n",
					 rates[r], word_sizes[b]);
			} else {
				failures++;
				LOG_INF("FAIL cfg rate=%u bits=%u ret=%d\n",
	...
	LOG_INF("sampling-freq matrix: pass=%u fail=%u\n", passes, failures);
...
ZTEST(i2s_config, test_config_word_select_cycles)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(ws_cycles); i++) {
		...
		if (wss_read != ws_fields[i]) {
			failures++;
			LOG_INF("FAIL ws=%u: exp_field=%u got=%u\n",
				 ws_cycles[i], ws_fields[i], wss_read);
		} else {
			LOG_INF("PASS ws=%u: WSS field=%u\n",
...
ZTEST(i2s_config, test_config_audio_resolution_matrix)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(word_sizes); i++) {
		...
		if (tcr_wlen != expected) {
			failures++;
			LOG_INF("FAIL word_size=%u TX WLEN exp=%u got=%u\n",
				 word_sizes[i], expected, tcr_wlen);
		} else {
			LOG_INF("PASS word_size=%u TX WLEN=%u\n",
				 word_sizes[i], tcr_wlen);
		}
		if (rcr_wlen != expected) {
			failures++;
			LOG_INF("FAIL word_size=%u RX WLEN exp=%u got=%u\n",
				 word_sizes[i], expected, rcr_wlen);
		} else {
			LOG_INF("PASS word_size=%u RX WLEN=%u\n",
...
ZTEST(i2s_config, test_config_fifo_depth_and_threshold)
{
	...
	LOG_INF("FIFO default thresholds: TX TFCR[3:0]=%u  RX RFCR[3:0]=%u\n",
		 tfcr, rfcr);

	if (tfcr >= CFG_FIFO_DEPTH) {
		failures++;
		LOG_INF("FAIL TX default threshold=%u exceeds fifo depth %u\n",
			 tfcr, CFG_FIFO_DEPTH);
	}
	if (rfcr >= CFG_FIFO_DEPTH) {
		failures++;
		LOG_INF("FAIL RX default threshold=%u exceeds fifo depth %u\n",
			 rfcr, CFG_FIFO_DEPTH);
	}

	cfg_deconfigure(rx_dev, tx_dev);
	zassert_equal(failures, 0U, "fifo depth threshold failures: %u", failures);
	LOG_INF("PASS: FIFO default thresholds within depth\n");
}
...
ZTEST(i2s_config, test_config_programmable_fifo)
{
	...
	for (uint32_t level = 0U; level < CFG_FIFO_DEPTH; level++) {
		...
		if (tx_read != level) {
			failures++;
			LOG_INF("FAIL TX threshold level=%u read=%u\n",
				 level, tx_read);
		}
		if (rx_read != level) {
			failures++;
			LOG_INF("FAIL RX threshold level=%u read=%u\n",
				 level, rx_read);
		}
	}
	...
	LOG_INF("PASS: all FIFO levels 0..%u write/readback verified\n",
		 CFG_FIFO_DEPTH - 1U);
}
...
ZTEST(i2s_config, test_config_dma_handshake_interface)
{
	...
	if ((dmacr & (I2S_DMACR_TX_EN_BIT | I2S_DMACR_RX_EN_BIT)) != 0U) {
		failures++;
		LOG_INF("FAIL DMACR clear: read=0x%x\n", dmacr);
	}
	...
	if ((dmacr & I2S_DMACR_TX_EN_BIT) == 0U ||
		...
		failures++;
		LOG_INF("FAIL DMACR TX-only: read=0x%x\n", dmacr);
	}
	...
	if ((dmacr & I2S_DMACR_RX_EN_BIT) == 0U ||
		...
		failures++;
		LOG_INF("FAIL DMACR RX-only: read=0x%x\n", dmacr);
	}
	...
	if ((dmacr & (I2S_DMACR_TX_EN_BIT | I2S_DMACR_RX_EN_BIT)) !=
	    (I2S_DMACR_TX_EN_BIT | I2S_DMACR_RX_EN_BIT)) {
		failures++;
		LOG_INF("FAIL DMACR TX+RX: read=0x%x\n", dmacr);
	}
	...
	LOG_INF("PASS: DMA TX/RX enable bits independently writable\n");
}
