...
LOG_MODULE_REGISTER(i2s_loopback_test, LOG_LEVEL_INF);
 ...
 * -------------------------------------------------------------------------
 */
static const uint32_t __unused mx_rates[] = {
				...
				const struct i2s_config *cfg)
{
	...
	if (ret != -ENOSYS && ret != -ENOTSUP) {
		LOG_INF("[mx] DIR_BOTH configure failed (%d)\n", ret);
		return ret;
	}
	ret = i2s_configure(dev, I2S_DIR_RX, cfg);
	if (ret != 0) {
		LOG_INF("[mx] RX configure failed (%d)\n", ret);
		return ret;
	}
...
static int mx_trigger_start(const struct device *dev)
{
	...
	for (uint32_t attempt = 0U; attempt < 3U; attempt++) {
		...
		LOG_INF("[mx] DIR_BOTH START failed (%d), retry %u\n",
			 ret, attempt + 1U);
	...
	if (ret == -ENOSYS || ret == -ENOTSUP) {
		LOG_INF("[mx] DIR_BOTH unsupported, falling back\n");
	} else {
		LOG_INF("[mx] DIR_BOTH START permanently failed (%d)\n", ret);
		return ret;
	}

	ret = i2s_trigger(dev, I2S_DIR_RX, I2S_TRIGGER_START);
	if (ret != 0) {
		LOG_INF("[mx] RX START failed (%d)\n", ret);
		return ret;
	}
	ret = i2s_trigger(dev, I2S_DIR_TX, I2S_TRIGGER_START);
	if (ret != 0) {
		LOG_INF("[mx] TX START failed (%d)\n", ret);
 ...
 * -------------------------------------------------------------------------
 */
static void __unused mx_fill_12(void *buf, uint32_t n_samples)
{
	int16_t *b = buf;
...
static void __unused mx_fill_16(void *buf, uint32_t n_samples)
{
	int16_t *b = buf;
	const uint32_t vlen = I2S_GOLDEN_VEC_LEN;
...
static void __unused mx_fill_20(void *buf, uint32_t n_samples)
{
	int32_t *b = buf;
	const uint32_t vlen = I2S_GOLDEN_VEC_LEN;
...
static void __unused mx_fill_24(void *buf, uint32_t n_samples)
{
	int32_t *b = buf;
	const uint32_t vlen = I2S_GOLDEN_VEC_LEN;
...
static void __unused mx_fill_32(void *buf, uint32_t n_samples)
{
	uint32_t *b = buf;
	const uint32_t vlen = I2S_GOLDEN_VEC_LEN;
 ...
 * -------------------------------------------------------------------------
 */
static bool __unused mx_verify_12(const void *buf, uint32_t n_samples,
			 uint32_t blk_idx, uint32_t rate)
{
	...
	LOG_INF("[mx-12] FAIL: %uHz blk=%u phase=%u run=%u/%u (need %u)\n",
		 rate, blk_idx, best_phase, best_run, n_frames, pass_thr);
	return false;
}

static bool __unused mx_verify_16(const void *buf, uint32_t n_samples,
			 uint32_t blk_idx, uint32_t rate)
{
	const int16_t *rx = buf;
	const uint32_t vlen     = I2S_GOLDEN_VEC_LEN;
	...
	LOG_INF("[mx-16] FAIL: %uHz blk=%u phase=%u run=%u/%u (need %u)\n",
		 rate, blk_idx, best_phase, best_run, n_frames, pass_thr);
	return false;
}

static bool __unused mx_verify_20(const void *buf, uint32_t n_samples,
			 uint32_t blk_idx, uint32_t rate)
{
	...
	LOG_INF("[mx-20] FAIL: %uHz blk=%u phase=%u run=%u/%u (need %u)\n",
		 rate, blk_idx, best_phase, best_run, n_frames, pass_thr);
	return false;
}

static bool __unused mx_verify_24(const void *buf, uint32_t n_samples,
			 uint32_t blk_idx, uint32_t rate)
{
	const int32_t *rx = buf;
	const uint32_t vlen     = I2S_GOLDEN_VEC_LEN;
	...
	LOG_INF("[mx-24] FAIL: %uHz blk=%u phase=%u run=%u/%u (need %u)\n",
		 rate, blk_idx, best_phase, best_run, n_frames, pass_thr);
	return false;
}
...
static bool __unused mx_verify_32(const void *buf, uint32_t n_samples,
			 uint32_t blk_idx, uint32_t rate)
{
	...
	LOG_INF("[mx-32] FAIL: %uHz blk=%u L_run=%u/%u R_run=%u/%u (need %u)\n",
...
/* All callers of mx_run_one() are guarded by #if CONFIG_I2S_GPIO_LOOPBACK,
 * so when the loopback wire is not declared this helper is compiled but
 * never referenced. Mark it __unused to silence -Werror=unused-function
 * under twister's stricter build flags.
 */
static int __unused mx_run_one(uint32_t word_size, uint32_t rate,
		      ...
		      const char *pfx)
{
	...
	if (ret != 0) {
		LOG_INF("[mx-%s] configure failed (%d)\n", pfx, ret);
		return ret;
	}
	...
	if (!wire_ok) {
		LOG_INF("[mx-%s] SKIP: wire absent @ %u Hz\n", pfx, rate);
		return -ENODEV;
	}
	if (!verified) {
		LOG_INF("[mx-%s] FAIL: no block passed verify @ %u Hz\n",
			 pfx, rate);
		return -EIO;
	}
	return 0;
