...
LOG_MODULE_REGISTER(xip);

/*
 * Shared XIP suite for APS512XXN and S80KS2564.
 * Device-specific cases are src/test_psram.c (CONFIG_TEST_PSRAM) and
 * src/test_s80ks2564.c (CONFIG_TEST_HYPERRAM).
 */
#if IS_ENABLED(CONFIG_TEST_HYPERRAM)
#define XIP_DEV_NAME "HyperRAM"
#elif IS_ENABLED(CONFIG_TEST_PSRAM)
#define XIP_DEV_NAME "PSRAM"
#else
#define XIP_DEV_NAME "XIP"
#endif
#define PSRAM_NODE                   DT_ALIAS(spi_psram)
#define PSRAM_CTRL_NODE              DT_PARENT(PSRAM_NODE)
#define PSRAM_TEST_SIZE              ((uint32_t)DT_PROP(PSRAM_NODE, size))
#define PSRAM_XIP_BASE_ADDR          ((uint32_t)DT_PROP_BY_IDX(PSRAM_CTRL_NODE, \
							       xip_base_address, 0))
#define PSRAM_XIP_WIN_SIZE           ((uint32_t)DT_PROP_BY_IDX(PSRAM_CTRL_NODE, \
							       xip_base_address, 1))
#define PSRAM_BUS_SPEED              ((uint32_t)DT_PROP(PSRAM_CTRL_NODE, bus_speed))
...
ZTEST(xip_tests, test_xip_base_address_test)
{
	...
	LOG_INF("%s size: %u bytes (%u MB)", XIP_DEV_NAME,
...
ZTEST(xip_tests, test_basic_word_rw_test)
{
 ...
 */
ZTEST(xip_tests, test_byte_access_test)
{
...
ZTEST(xip_tests, test_halfword_access_test)
{
...
ZTEST(xip_tests, test_sequential_rw_test)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
...
ZTEST(xip_tests, test_data_pattern_test)
{
...
ZTEST(xip_tests, test_address_boundary_test)
{
	...
	*ptr = test_pattern;
	...
	IS_ENABLED(CONFIG_TEST_HYPERRAM_FULL_CHIP)
/* Full array word R/W — same coverage as samples/drivers/spi_psram */
ZTEST(xip_tests, test_full_memory_rw_test)
{
	volatile uint32_t *ptr;
...
ZTEST(xip_tests, test_performance_test)
{
	uint8_t *xip_ptr;
 ...
 */
ZTEST(xip_tests, test_unaligned_access_test)
{
...
ZTEST(xip_tests, test_memory_stress_test)
{
...
ZTEST(xip_tests, test_concurrent_region_test)
{
	uint32_t *region1;
	uint32_t *region2;
...
ZTEST(xip_tests, test_large_block_transfer_test)
{
	uint8_t *src_buf;
	uint8_t *verify_buf;
	uint8_t *xip_ptr;
	...
	if (src_buf == NULL || verify_buf == NULL) {
		k_free(src_buf);
		k_free(verify_buf);
...
ZTEST(xip_tests, test_memset_memcmp_test)
{
	uint8_t *xip_ptr = get_xip_ptr(0);
...
ZTEST(xip_tests, test_sequential_burst_test)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
	uint32_t burst_sizes[] = {16, 64, 256, 1024, 4096};

	if (PSRAM_TEST_SIZE < 4096U) {
		ztest_test_skip();
	}
...
ZTEST(xip_tests, test_xip_config_test)
{
	LOG_INF("%s device: %s", XIP_DEV_NAME, psram_dev->name);
	LOG_INF("XIP base: 0x%08x window %u bytes", PSRAM_XIP_BASE_ADDR,
		PSRAM_XIP_WIN_SIZE);
#if DT_NODE_HAS_PROP(PSRAM_NODE, latency)
	LOG_INF("%s size: %u bytes, bus-speed: %u Hz, latency: %u",
		XIP_DEV_NAME, PSRAM_TEST_SIZE, PSRAM_BUS_SPEED,
		(uint32_t)DT_PROP(PSRAM_NODE, latency));
#elif DT_NODE_HAS_PROP(PSRAM_NODE, latency_code)
	LOG_INF("%s size: %u bytes, bus-speed: %u Hz, latency-code: %u",
		XIP_DEV_NAME, PSRAM_TEST_SIZE, PSRAM_BUS_SPEED,
		(uint32_t)DT_PROP(PSRAM_NODE, latency_code));
#else
	LOG_INF("%s size: %u bytes, bus-speed: %u Hz",
		XIP_DEV_NAME, PSRAM_TEST_SIZE, PSRAM_BUS_SPEED);
#endif
 ...
 */
ZTEST(xip_tests, test_ddr_data_integrity)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
 ...
 */
ZTEST(xip_tests, test_16bit_databus_validation)
{
	volatile uint16_t *ptr = (volatile uint16_t *)PSRAM_XIP_BASE_ADDR;
	uint16_t readback;
 ...
 */
ZTEST(xip_tests, test_burst_length_boundaries)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
 ...
 */
ZTEST(xip_tests, test_linear_burst_access)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
...
ZTEST(xip_tests, test_max_throughput_validation)
{
	uint8_t *xip_ptr;
	uint8_t buf[4096];
	int64_t elapsed_ms;
 ...
 */
ZTEST(xip_tests, test_rwds_interleaved_rw)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
...
ZTEST(xip_tests, test_alternating_pattern_integrity)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
 ...
 */
ZTEST(xip_tests, test_data_retention)
{
	volatile uint32_t *ptr = (volatile uint32_t *)PSRAM_XIP_BASE_ADDR;
	uint32_t num_words = 1024;
 ...
 */
ZTEST(xip_tests, test_region_fraction_access)
{
	...
	LOG_INF("Total %s size: %u bytes", XIP_DEV_NAME, PSRAM_TEST_SIZE);
...
{
	LOG_INF("=============================================");
	LOG_INF("%s XIP Test Suite Started", XIP_DEV_NAME);
	LOG_INF("=============================================");
	return NULL;
}
...
static void psram_test_suite_teardown(void *fixture)
{
	ARG_UNUSED(fixture);
	LOG_INF("=============================================");
	LOG_INF("%s XIP Test Suite Completed", XIP_DEV_NAME);
	LOG_INF("=============================================");
}
...
static void psram_test_before(void *fixture)
{
	...
	zassert_true(device_is_ready(psram_dev),
		     "%s device not ready: %s", XIP_DEV_NAME, psram_dev->name);
...
ZTEST_SUITE(xip_tests, NULL, psram_test_suite_setup,
