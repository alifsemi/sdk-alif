...
* ``src/main.c`` — XIP pointer access (byte / halfword / word), data
  patterns, address boundaries, unaligned access, burst-length offsets and
  linear burst, DDR and 16-bit data-bus integrity, RWDS interleaved
  read/write, region-fraction access, burst/throughput checks, idle data
  retention (refresh) validation, optional full-array walk matching the
  sample
