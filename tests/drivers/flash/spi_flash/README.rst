...
**Basic build using snippet system:**
...
Available frequency overlays in ``tests/drivers/flash/spi_flash/boards``:

* ``ospi0_80Mhz.overlay``
* ``ospi1_50Mhz.overlay``
* ``ospi1_100Mhz.overlay``
* ``ospi1_200Mhz.overlay``

Overlay target mapping:

* ``ospi0`` overlays are for **B1** platforms.
* ``ospi1`` overlays are for **Ensemble E7/E8** platforms.

**Higher-frequency run (example: OSPI1 @ 100MHz on E8 RTSS_HP):**

For higher-frequency validation, clean the build directory and provide a
frequency-specific overlay.

.. code-block:: console

   rm -rf build/; west build -b alif_e8_dk/ae822fa0e5597xx0/rtss_hp \
   tests/drivers/flash/spi_flash/ -S ospi-flash \\
   -DDTC_OVERLAY_FILE="$PWD/tests/drivers/flash/spi_flash/boards/ospi1_100Mhz.overlay"
