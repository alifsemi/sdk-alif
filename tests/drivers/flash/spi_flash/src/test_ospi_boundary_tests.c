...
static void cross_page_boundary_test(void)
{
	...
	/* This test assumes page_size >= BUFF_SIZE/2 so cross_offset
	 * stays within the erased region starting at TEST_REGION_OFFSET.
	 */
	zassert_true(flash_param->page_size >= (BUFF_SIZE / 2),
		     "page_size (%zu) too small for cross-page test",
		     flash_param->page_size);

	/* Offset that crosses a page boundary (per device page_size) */
	cross_offset = SPI_FLASH_TEST_REGION_OFFSET +
		       flash_param->page_size - (BUFF_SIZE / 2);

	/* Erase the test region sectors before the cross-page write */
