...
static void write_without_erase_test(void)
{
	...
	for (size_t i = 0; i < len; i++) {
		...
		if (r_buf[i] != expected) {
			e_count++;
			if (e_count <= 5) {
				LOG_INF("Mismatch[%zu]: b=0x%02x req=0x%02x e=0x%02x g=0x%02x",
					i, before_buf[i], w_buf[i], expected,
					r_buf[i]);
			}
		}
	}
	...
	/* If 1->0 programming is possible, data should change. */
...
static void invalid_parameter_test(void)
{
	...
	/* Test 3: Request crosses end of flash without exceeding buf size */
	size_t flash_size = flash_param->num_of_sector *
			    flash_param->sector_size;
	ret = flash_read(flash_dev, flash_size - BUFF_SIZE + 1,
			 buf, BUFF_SIZE);
	zassert_not_equal(ret, 0,
			  "Should fail when offset + length exceeds flash size");
...
static void unaligned_address_test(void)
{
	...
	for (i = 0; i < 3; i++) { /* Test a few unaligned write offsets */
		...
		uint8_t fallback_pattern[64];
		size_t pattern_len = ARRAY_SIZE(test_pattern);
		uint8_t read_back[sizeof(fallback_pattern)];
		...
		if (pattern_len == 0U) {
			size_t wb = flash_param->write_block_size;
			size_t pattern_sz = ARRAY_SIZE(test_pattern);

			if (wb > sizeof(fallback_pattern)) {
				LOG_INF("Skip unaligned write 0x%x: wb %zu > fb %zu",
					unaligned_write_offset, wb,
					sizeof(fallback_pattern));
				continue;
			}

			for (size_t j = 0; j < wb; j++) {
				fallback_pattern[j] = test_pattern[j % pattern_sz];
			}

			write_buf = fallback_pattern;
			pattern_len = wb;
			LOG_INF("Using fallback pattern (%zu bytes) at 0x%x",
				pattern_len, unaligned_write_offset);
		}
