	...
	for (size_t p = 0; p < num_pages; p++) {
		...
		for (size_t i = 0; i < page_size; i++) {
			if (r_buf[i] != w_buf[i]) {
				e_count++;
				if (e_count <= 5) {
					LOG_INF("Page %zu[%zu] w[0x%02x] r[0x%02x]",
						p, i, w_buf[i], r_buf[i]);
				}
			}
		}
...
static void multi_sector_test(void)
{
	...
	/* Write into Sector 5 */
	ret = flash_write(flash_dev, SPI_FLASH_SECTOR_13_OFFSET, w_buf, len);
	zassert_equal(ret, 0,
	...
	LOG_INF("Test 4: read sector %d", SPI_FLASH_SECTOR_12_OFFSET);

	ret = flash_read(flash_dev, SPI_FLASH_SECTOR_12_OFFSET, r_buf, len);
	zassert_equal(ret, 0,
	...
	zassert_equal(e_count, 0,
		      "Sec 12 verify failed - %d mismatches",
		      e_count);

	/* Read from Sector 13 */
	LOG_INF("Sec13: Read and Verify written data");
	int e_count5 = 0;

	memset(r_buf, 0, len);

	LOG_INF("Test 4: read sector %d", SPI_FLASH_SECTOR_13_OFFSET);

	ret = flash_read(flash_dev, SPI_FLASH_SECTOR_13_OFFSET, r_buf, len);
	zassert_equal(ret, 0,
		     "Error: Flash read Failed Sec 13! [%d]", ret);
	...
	if (e_count5) {
		LOG_INF("Error: Data read NOT matches data written in Sec 13");
		LOG_INF(" -- number of Unmatched data [%d]", e_count5);
	} else {
		LOG_INF("Data read matches data written in Sec 13. Good!!");
	}

	zassert_equal(e_count5, 0,
		      "Sec 13 verify failed - %d mismatches",
		      e_count5);

	/* Erase multiple Sector 12+13 */
	LOG_INF("Test 4: Erase Sector 12 and 13");
	LOG_INF("Flash Erase from Sector %d Size to Erase %d",
		SPI_FLASH_SECTOR_12_OFFSET, SPI_FLASH_SECTOR_SIZE * 2);
	...
	ret = flash_read(flash_dev, SPI_FLASH_SECTOR_12_OFFSET,
			 r_buf, BUFF_SIZE);
	...
	/* Read Erased value and compare */
	ret = flash_read(flash_dev, SPI_FLASH_SECTOR_13_OFFSET,
			 r_buf, BUFF_SIZE);
	zassert_equal(ret, 0, "Error: Reading Erased value [%d]", ret);
	...
	zassert_equal(count_1, 0,
		     "Sector 4 erase verify failed - %d non-erased bytes",
		     count_1);
	zassert_equal(count_2, 0,
		     "Sector 5 erase verify failed - %d non-erased bytes",
		     count_2);
