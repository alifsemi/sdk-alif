...
static void dac_twoscomp_input_range_test(void)
{
	...
	/* Over-range values must be rejected in twoscomp mode */
	ret_1 = dac_write_value(dac_dev, DAC_CHANNEL_ID, 0x1FFF);
	zassert_equal(ret_1, -EINVAL,
		      "Expected -EINVAL for 0x1FFF in twoscomp mode, got [%d]", ret_1);

	ret_2 = dac_write_value(dac_dev, DAC_CHANNEL_ID, 0xFFFF);
	zassert_equal(ret_2, -EINVAL,
		      "Expected -EINVAL for 0xFFFF in twoscomp mode, got [%d]", ret_2);

	TC_PRINT("DAC two's complement input range test PASSED\n");
}
