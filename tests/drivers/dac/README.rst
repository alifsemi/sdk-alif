...
- **R-BUG-3**: Rapid write stress test (test_dac_basic.c)
  * 200 alternating min/max writes without per-sample delays
...
Test Suites
***********
- **test_dac_basic** (11 tests): Invalid resolution, basic write,
  DAC0/DAC1 independent init, dual channel write, dual channel
  simultaneous operation, invalid channel_id rejection (R-BUG-1),
  two's complement input range (R-BUG-2), rapid write stress (R-BUG-3),
...
- **test_dac_boundary** (7 tests): Valid boundary values, out-of-range rejection,
  alternating min/max, power-of-two, repeated setup, zero resolution,
...
.. code-block:: console
    ...
    -- -DDTC_OVERLAY_FILE=alif_dac0.overlay

    # Two's-complement mode: twos-complement tests enabled
    west build -b alif_e8_dk/ae822fa0e5597xx0/rtss_he ../alif/tests/drivers/dac/ \
    -- -DDTC_OVERLAY_FILE=alif_dac0_twoscomp.overlay

    # Maximum output current (1.5 mA) - HRM feature test
    west build -b alif_e8_dk/ae822fa0e5597xx0/rtss_he ../alif/tests/drivers/dac/ \
    -- -DDTC_OVERLAY_FILE=alif_dac0_max_current.overlay

    # Minimum output current (0 uA) - light load testing
    west build -b alif_e8_dk/ae822fa0e5597xx0/rtss_he ../alif/tests/drivers/dac/ \
    -- -DDTC_OVERLAY_FILE=alif_dac0_min_current.overlay
...
- **R-BUG-3 (rapid write stress)**
  - Current test coverage: 200 alternating min/max writes without per-sample
    delays.
...
:dtcompatible:`alif,dac` as a compatible.

Use the dual overlay for full dual-channel testing:

.. code-block:: console

    west build -b <board> -- -DDTC_OVERLAY_FILE=alif_dac_dual.overlay
