...
ZTEST(crc_test, crc_8_bit_reflect_output_en)
{
	uint32_t crc_output, software_output;
	struct crc_params params = {
		.data_in     = (uint8_t *)crc_test_data,
	...
	LOG_INF("8-bit CRC SW output (reflected): 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_8_bit_swap_en)
{
	uint32_t crc_output, software_output;
	...
	software_output = gen_crc(CRC8_PRIME, reflected_input,
				  sizeof(reflected_input));
	LOG_INF("8-bit CRC SW output (swapped): 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_8_bit_invert_output_en)
{
	uint32_t crc_output, software_output;
	struct crc_params params = {
		.data_in     = (uint8_t *)crc_test_data,
	...
	LOG_INF("8-bit CRC SW output (inverted): 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_8_bit_single_byte)
{
	uint32_t crc_output, software_output;
	...
	zassert_equal(crc_compute(crc_dev, &params), 0,
		      "CRC compute failed");
	LOG_INF("8-bit CRC HW single byte output: 0x%X",
		 crc_output);

	software_output = gen_crc(CRC8_PRIME, single_byte, 1);
	LOG_INF("8-bit CRC SW single byte output: 0x%X",
		 software_output);

	zassert_equal((crc_output & 0xFF), software_output,
		      "HW/SW CRC mismatch: 0x%X != 0x%X",
		      (crc_output & 0xFF), software_output);
}

/* ZTC-NEW: 8-bit CRC with non-zero seed */
ZTEST(crc_test, crc_8_bit_nonzero_seed)
{
	...
	LOG_INF("8-bit CRC seed=0x00: 0x%X, seed=0xFF: 0x%X",
		crc_seed0, crc_seedFF);
...
ZTEST(crc_test, crc_8_bit_reflect_invert_combined)
{
	uint32_t crc_output, software_output;
	struct crc_params params = {
		.data_in     = (uint8_t *)crc_test_data,
