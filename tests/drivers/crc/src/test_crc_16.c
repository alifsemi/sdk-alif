...
ZTEST(crc_test, crc_16_bit)
{
	...
	software_output = gen_crc(CRC16_802_15_4,
				  (const unsigned char *)crc_test_data,
				  crc_test_data_len);
	LOG_INF("16-bit CRC SW output: 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_16_bit_reflect_output_en)
{
	uint32_t crc_output, software_output;
	struct crc_params params = {
		.data_in     = (uint8_t *)crc_test_data,
	...
	LOG_INF("16-bit CRC SW output (reflected): 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_16_bit_swap_en)
{
	uint32_t crc_output, software_output;
	...
	LOG_INF("16-bit CRC SW output (swapped): 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_16_bit_invert_output)
{
	uint32_t crc_output, software_output;
	struct crc_params params = {
		.data_in     = (uint8_t *)crc_test_data,
	...
	LOG_INF("16-bit CRC SW output (inverted): 0x%X",
		 software_output);
...
ZTEST(crc_test, crc_16_bit_single_byte)
{
	uint32_t crc_output, software_output;
	...
	zassert_equal(crc_compute(crc_dev, &params), 0,
		      "CRC compute failed");
	LOG_INF("16-bit CRC HW single byte output: 0x%X",
		 crc_output);

	software_output = gen_crc(CRC16_802_15_4, single_byte, 1);
	LOG_INF("16-bit CRC SW single byte output: 0x%X",
		 software_output);

	zassert_equal((crc_output & 0xFFFF), software_output,
		      "HW/SW CRC mismatch: 0x%X != 0x%X",
		      (crc_output & 0xFFFF), software_output);
}

/* ZTC-NEW: 16-bit CRC with non-zero seed */
ZTEST(crc_test, crc_16_bit_nonzero_seed)
{
	...
	LOG_INF("16-bit CRC seed=0x0000: 0x%X, seed=0xFFFF: 0x%X",
		crc_seed0, crc_seedFFFF);
...
ZTEST(crc_test, crc_16_bit_reflect_invert_combined)
{
	uint32_t crc_output, software_output;
	struct crc_params params = {
		.data_in     = (uint8_t *)crc_test_data,
