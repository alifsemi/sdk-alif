...
/*
 * Phase delay in microseconds. k_busy_wait() is used instead of k_usleep()
 * because precise microsecond-level timing is required for accurate quadrature
 * signal generation. A Twister timeout of 120s is set in testcase.yaml to
 * account for the busy-wait delays during test execution.
 */
