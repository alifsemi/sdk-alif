...
/*
 * Channels 0,2,5,7 share one FIR set; channels 1,3,4,6 share the other.
 */
static const uint32_t fir_ch_0257[PDM_MAX_FIR_COEFFICIENT] = {
...
static const uint32_t fir_ch_1346[PDM_MAX_FIR_COEFFICIENT] = {
...
static const uint32_t * const fir_table[8] = {
	fir_ch_0257, fir_ch_1346, fir_ch_0257, fir_ch_1346,
	fir_ch_1346, fir_ch_0257, fir_ch_1346, fir_ch_0257
};

#if NUM_CHANNELS == 0
#error "Enable at least one CONFIG_TEST_PDM_CHx"
#endif
			  ...
			  struct dmic_cfg *cfg, uint8_t block_count)
{
	...
	zassert_true(expected <= (size_t)DATA_SIZE,
		     "capture %zu bytes exceeds pcmj_data %zu",
		     expected, (size_t)DATA_SIZE);
	...
	for (i = 0; i < block_count; ++i) {
		...
		if ((k + data) > (size_t)DATA_SIZE) {
			LOG_ERR("PCM buffer overflow: have %zu need %zu cap %zu",
				k, k + data, (size_t)DATA_SIZE);
			k_mem_slab_free(&mem_slab, buffer);
			(void)dmic_trigger(dmic_dev, DMIC_TRIGGER_STOP);
			return -ENOSPC;
		}
