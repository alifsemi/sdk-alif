...
ZTEST(display_functional, test_display_fb_readback_verify)
{
	...
	if (caps.layer[0].x_resolution < 200 + rect_w ||
	    caps.layer[0].y_resolution < 200 + rect_h) {
		TC_PRINT("Display too small for readback regions, skipping test\n");
		ztest_test_skip();
	}
	buf_size = rect_w * rect_h * (size_t)pixel_size;
...
ZTEST(display_functional, test_display_invalid_layer)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(invalid_indices); i++) {
		...
		for (size_t b = 0; b < sizeof(fb_desc); b++) {
			zassert_equal(((const uint8_t *)&fb_desc)[b], 0xAA,
				      "fb_desc modified for invalid idx=%u", idx);
		}
...
ZTEST(display_functional, test_display_undersized_buffer)
{
	...
	/* Driver does not yet validate desc->buf_size; running this reads past
	 * the source buffer (potential fault/UB). Skip until the driver adds
	 * buf_size validation, then remove this skip.
	 */
	ztest_test_skip();
...
ZTEST(display_functional, test_display_null_params)
{
	...
	/*
	 * This test is skipped until the driver adds NULL checks.
	 * Remove the ztest_test_skip() once it does.
	 */
	ztest_test_skip();
...
ZTEST(display_functional, test_display_read_cache_coherency)
{
	...
	if (caps.layer[0].x_resolution < 300 + rect_w ||
	    caps.layer[0].y_resolution < 300 + rect_h) {
		TC_PRINT("Display too small for cache coherency region, skipping test\n");
		ztest_test_skip();
	}
...
ZTEST(display_api, test_display_write_multiple_rects)
{
	...
	zassert_equal(cdc200_display_write(display_dev, 0,
					   ...
					   &buf_desc, buf), 0,
		      "cdc200_display_write (green) failed");
...
ZTEST(display_api, test_display_read)
{
	...
	TC_PRINT("display_read() returned: %s (%d)\n",
		 display_errno_to_string(ret), ret);
	if (ret == -ENOTSUP) {
		TC_PRINT("Read not supported, skipping test\n");
		ztest_test_skip();
	}
	zassert_equal(ret, 0, "display_read failed: %d", ret);
	display_free_buffer(buf);
}
...
ZTEST(display_api, test_display_set_brightness)
{
	...
	zassert_equal(ret, 0, "display_set_brightness failed: %d", ret);
}
