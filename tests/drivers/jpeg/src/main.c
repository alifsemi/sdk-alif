...
ZTEST(jpeg_hantro, test_jpeg_pitch_with_padding)
{
	...
	if (!in || !out) {
		...
		zassert_unreachable("Buffer alloc failed (in=%p out=%p)",
				    in, out);
	}
...
ZTEST(jpeg_hantro, test_jpeg_encode_large_1920x1080)
{
	...
	if (!in || !out) {
		...
		TC_PRINT("Skipping 1920x1080: buffer pool too small "
			 "(bump CONFIG_VIDEO_BUFFER_POOL_SZ_MAX)\n");
		ztest_test_skip();
		return;
	}
...
ZTEST(jpeg_hantro, test_jpeg_encode_max_width)
{
	...
	if (!in || !out) {
		...
		TC_PRINT("Skipping 8192x32: buffer pool too small\n");
		ztest_test_skip();
		return;
	}
