...
/* Stress context - data buffers and transfer helpers only */
...
/* Async mode flag for stress tests */
static bool stress_test_async_mode;

/* Async-capable stress xfer helpers */
static int stress_xfer_write(const struct device *dev, const uint8_t *data,
			     size_t len, uint16_t addr)
{
	if (!stress_test_async_mode) {
...
static int stress_xfer_read(const struct device *dev, uint8_t *data,
			    size_t len, uint16_t addr)
{
	if (!stress_test_async_mode) {
...
static int stress_xfer_write_read(const struct device *dev, uint16_t addr,
...
static int run_stress_loop(const struct device *dev,
			   ...
			   uint32_t iterations)
{
	...
	for (uint32_t i = 0; i < iterations; i++) {
		...
		if ((tx_len > 0U) && (rx_len > 0U)) {
			ret = stress_xfer_write_read(dev, TGT_I2C_ADDR,
				     tx_data, tx_len,
				     rx_data, rx_len);
		} else if (tx_len > 0U) {
			ret = stress_xfer_write(dev, tx_data, tx_len, TGT_I2C_ADDR);
		} else {
			ret = stress_xfer_read(dev, rx_data, rx_len, TGT_I2C_ADDR);
		}
		...
		if ((tx_len > 0U) && (rx_len > 0U)) {
			/* Validate controller read (target_tx -> rx_data) */
			ret = i2c_validate_data_match(stress_ctx.target_tx,
						      rx_data, rx_len,
						      "stress rd");
			if (ret != 0) {
				failures++;
			...
			/* Validate target received write (tx_data -> target_rx) */
			ret = i2c_validate_data_match(tx_data,
			...
			/* Write-only: validate target received the data */
			ret = i2c_validate_data_match(tx_data,
						      stress_ctx.target_rx, tx_len,
						      "stress wr-only");
			if (ret != 0) {
			...
			/* Read-only: validate controller received target data */
			ret = i2c_validate_data_match(stress_ctx.target_tx,
						      rx_data, rx_len,
						      "stress rd-only");
			if (ret != 0) {
...
static void reset_runtime_counters(void)
{
	...
	/* Note: driver_errors, data_corruption, and hardware_errors accumulate
	 * across all tests for teardown validation. Do not reset them here.
	 */
...
static int cb_read_requested(struct i2c_target_config *config, uint8_t *val)
{
	...
	/* Reset index at start of every controller-read transaction so the target
	 * always serves data from the beginning of its TX buffer.
	 */
	stress_ctx.target_tx_index = 0U;
...
{
	int ret;
	uint32_t controller_cfg = I2C_MODE_CONTROLLER |
				  I2C_SPEED_SET(I2C_SPEED_FAST);
	uint32_t target_cfg = I2C_SPEED_SET(I2C_SPEED_FAST);

	ret = i2c_configure(stress_ctx.controller_dev, controller_cfg);
	if (ret != 0) {
...
static int controller_write_transfer(const uint8_t *tx_buf, uint32_t len)
{
	...
	if (!stress_test_async_mode) {
		return i2c_transfer(stress_ctx.controller_dev, &msg, 1,
				  TGT_I2C_ADDR);
	}
	return i2c_do_xfer(stress_ctx.controller_dev, &msg, 1,
			   TGT_I2C_ADDR, true);
}
					      ...
					      uint8_t *rx_buf, uint32_t rx_len)
{
	...
	return i2c_do_xfer(stress_ctx.controller_dev, msgs, ARRAY_SIZE(msgs),
			   TGT_I2C_ADDR, true);
}
...
static void i2c_stress_teardown(void *fixture)
{
	...
	zassert_equal(stress_ctx.driver_errors, 0U,
		      "Driver errors detected: %u", stress_ctx.driver_errors);
	zassert_equal(stress_ctx.data_corruption, 0U,
		      "Data corruption detected: %u", stress_ctx.data_corruption);
}
...
ZTEST(i2c_stress, test_write_oversize)
{
	...
	for (int f = 0; f < freq_count; f++) {
		...
		match_ret = validate_data_match(tx_oversize,
						stress_ctx.target_rx,
						BUFF_PERF,
						"Oversize Ctrl TX -> Tar RX (clamped)");
		zassert_equal(match_ret, 0,
...
#if IS_ENABLED(CONFIG_I2C_CALLBACK)
ZTEST(i2c_stress, test_stress_async)
{
	stress_test_async_mode = true;
	...
	/* Basic write stress */
	{
		...
		i2c_test_skip_if_no_freqs();
		freq_count = i2c_test_get_enabled_freqs(freqs,
						      ARRAY_SIZE(freqs));
		for (int f = 0; f < freq_count; f++) {
			int ret;

			ret = i2c_test_configure_controller_freq(
				stress_ctx.controller_dev, &i2c_test_ctx, &freqs[f]);
			if (ret == -ENOTSUP) {
				continue;
			}
			zassert_equal(ret, 0,
				      "Failed to configure %s: %d",
				      freqs[f].name, ret);
			ret = run_stress_loop(stress_ctx.controller_dev,
				      stress_ctx.controller_tx,
				      STRESS_WRITE_LEN_MEDIUM, NULL, 0,
				      10U);
			zassert_equal(ret, 0,
				      "Async write stress failed at %s",
				      freqs[f].name);
		}
	}
	...
	/* Write-read stress */
	{
		...
		for (int f = 0; f < freq_count; f++) {
			...
			ret = run_stress_loop(stress_ctx.controller_dev,
				      stress_ctx.controller_tx,
				      STRESS_WRITE_LEN_SMALL,
				      stress_ctx.controller_rx,
				      STRESS_READ_LEN, 10U);
			zassert_equal(ret, 0,
				      "Async wr-rd stress failed at %s",
				      freqs[f].name);
		}
	}
	...
	/* Restart integrity */
	{
		...
		freq_count = i2c_test_get_enabled_freqs(freqs,
						      ARRAY_SIZE(freqs));
		for (int f = 0; f < freq_count; f++) {
			uint8_t rx_buf[STRESS_WRITE_LEN_SMALL];
			...
			zassert_equal(ret, 0,
				      "Failed to configure %s: %d",
				      freqs[f].name, ret);
			ret = controller_write_read_transfer(
				stress_ctx.controller_tx,
				STRESS_WRITE_LEN_SMALL,
				rx_buf, sizeof(rx_buf));
			zassert_equal(ret, 0,
				      "Async restart stress failed at %s",
				      freqs[f].name);
		}
	}
