			     ...
			     uint32_t len)
{
	...
	LOG_INF("Received %d bytes from controller: ", len);
	for (idx = 0; idx < len; idx++) {
		...
		if (i2c_test_ctx.tgt_rx_cnt < BUFF_PERF) {
			...
			LOG_DBG("Ignoring extra target RX byte 0x%x",
				data_buf[idx]);
		}
	}
}
...
static void register_perf_target_speed_i2c(uint32_t i2c_cfg)
{
	...
	zassert_equal(ret, 0, "I2C target config failed");
 ...
 * 1. Controller writes data to target
 * 2. Controller reads data from target
...
enum i2c_perf_mode {
	I2C_PERF_TX_ONLY,   /* Controller write to target only */
	I2C_PERF_RX_ONLY,   /* Controller read from target only */
 ...
 * - I2C_PERF_TX_ONLY: Controller write to target, measures TX performance
 * - I2C_PERF_RX_ONLY: Controller read from target, measures RX performance
 * - I2C_PERF_TXRX:    Write then read, measures full transceive
 *
 * @param mode Transfer mode (TX_ONLY, RX_ONLY, or TXRX)
 * @return 0 on success, negative error code on failure
 */
int start_i2c_transfer_mode(enum i2c_perf_mode mode, bool async)
{
	...
	if (ret != 0) {
		LOG_ERR("I2C controller configuration failed: %d", ret);
		return ret;
	}
	...
	if (mode == I2C_PERF_TX_ONLY || mode == I2C_PERF_TXRX) {
		...
		ret = i2c_do_xfer(i2c_controller_dev, &msgs[0], 1,
				  TGT_I2C_ADDR, async);
	...
	if (mode == I2C_PERF_RX_ONLY || mode == I2C_PERF_TXRX) {
		...
		ret = i2c_do_xfer(i2c_controller_dev, &msgs[1], 1,
				  TGT_I2C_ADDR, async);
...
static void test_perf_tx_scenario(bool async)
{
	...
	for (i = 0; i < freq_count; i++) {
		...
		ret = start_i2c_transfer_mode(I2C_PERF_TX_ONLY, async);
...
static void test_perf_xcv_scenario(bool async)
{
	struct i2c_test_freq_desc freqs[I2C_TEST_MAX_FREQS];
	int ret;
	int freq_count;
	...
	for (i = 0; i < freq_count; i++) {
		...
		register_perf_target_speed_i2c(
			I2C_SPEED_SET(freqs[i].zephyr_speed));
		ret = start_i2c_transfer_mode(I2C_PERF_TXRX, async);
...
static void test_perf_speed_reconfig_scenario(bool async)
{
	struct i2c_test_freq_desc freqs[I2C_TEST_MAX_FREQS];
	int ret;
	int freq_count;
...
ZTEST(i2c_perf, test_perf_tx_sync)
{
	test_perf_tx_scenario(false);
}

#if IS_ENABLED(CONFIG_I2C_CALLBACK)
ZTEST(i2c_perf, test_perf_tx_async)
{
	test_perf_tx_scenario(true);
}
#endif

ZTEST(i2c_perf, test_perf_xcv_sync)
{
	test_perf_xcv_scenario(false);
}

#if IS_ENABLED(CONFIG_I2C_CALLBACK)
ZTEST(i2c_perf, test_perf_xcv_async)
{
	test_perf_xcv_scenario(true);
}
#endif

ZTEST(i2c_perf, test_perf_speed_reconfig_sync)
{
	test_perf_speed_reconfig_scenario(false);
}

#if IS_ENABLED(CONFIG_I2C_CALLBACK)
ZTEST(i2c_perf, test_perf_speed_reconfig_async)
{
	test_perf_speed_reconfig_scenario(true);
}
#endif
