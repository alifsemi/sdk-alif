...
void i2c_test_configure_controller_freq_runtime_config(struct i2c_test_ctx *ctx);
...
{
	ctx->stop_count++;
	ctx->stop_rx_snapshot = ctx->tgt_rx_cnt;
}
...
/*
 * Slave callback contract validator.
 *
 * Each I2C slave transaction MUST produce exactly the following callback
 * pattern (register mode):
 *   WRITE  : write_requested==1, write_received==len, stop==1
 *   READ   : read_requested ==1, read_processed==len-1, stop==1
 *   W-R-R  : write_requested==1, write_received==w_len, read_requested==1,
 *            read_processed==r_len-1, stop==1
 *
 * Any deviation indicates a driver ISR state-machine bug (stale START_DET,
 * missed STOP_DET, duplicate write_requested, etc.). Call this immediately
 * after every validated transfer to lock down the contract.
 *
 * In buffer mode (CONFIG_I2C_TARGET_BUFFER_MODE) the byte-granularity
 * callbacks do not fire; only the aggregate counts are validated.
 */
static inline void i2c_test_assert_target_contract(
	const struct i2c_test_ctx *ctx,
	enum i2c_test_scenario scenario,
	size_t w_len, size_t r_len)
{
	const uint32_t w = (uint32_t)w_len;
	const uint32_t r = (uint32_t)r_len;

	/*
	 * The slave STOP_DET interrupt can fire a few microseconds after
	 * i2c_write() / i2c_read() returns on the controller side. Yield briefly
	 * so the target ISR completes and updates stop_count before we check.
	 */
	k_busy_wait(50);

	zassert_equal(ctx->callback_errors, 0U,
		      "slave callback-order violation: 0x%x",
		      (unsigned int)ctx->callback_errors);

#ifdef CONFIG_I2C_TARGET_BUFFER_MODE
	ARG_UNUSED(r); /* r_len not used in buffer mode; aggregate counts only */
	switch (scenario) {
	case I2C_TRANSMIT_ONLY:
		zassert_equal(ctx->buf_write_received_count, 1U,
			      "buf_write_received_count=%u expected 1",
			      (unsigned int)ctx->buf_write_received_count);
		zassert_equal(ctx->buf_last_write_len, w,
			      "buf_last_write_len=%u expected %u",
			      (unsigned int)ctx->buf_last_write_len,
			      (unsigned int)w);
		break;
	case I2C_RECEIVE_ONLY:
		zassert_equal(ctx->buf_read_requested_count, 1U,
			      "buf_read_requested_count=%u expected 1",
			      (unsigned int)ctx->buf_read_requested_count);
		break;
...
#else
	/* read_processed fires for all bytes except NACK'd last; guard underflow */
	const uint32_t r_min = (r == 0U) ? 0U : (r - 1U);

	switch (scenario) {
	case I2C_TRANSMIT_ONLY:
		zassert_equal(ctx->write_requested_count, 1U,
			      "write_requested_count=%u expected 1",
			      (unsigned int)ctx->write_requested_count);
		zassert_equal(ctx->write_received_count, w,
			      "write_received_count=%u expected %u",
			      (unsigned int)ctx->write_received_count,
			      (unsigned int)w);
		break;
	case I2C_RECEIVE_ONLY:
		zassert_equal(ctx->read_requested_count, 1U,
			      "read_requested_count=%u expected 1",
			      (unsigned int)ctx->read_requested_count);
		/* read_processed fires per byte except the NACK'd last one */
		zassert_true(ctx->read_processed_count >= r_min,
			     "read_processed_count=%u expected >=%u",
			     (unsigned int)ctx->read_processed_count,
			     (unsigned int)r_min);
		break;
	case I2C_TRANSMIT_RECEIVE:
		zassert_equal(ctx->write_requested_count, 1U,
...
#endif
	zassert_equal(ctx->stop_count, 1U,
		      "stop_count=%u expected 1",
		      (unsigned int)ctx->stop_count);
}
