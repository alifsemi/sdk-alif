					...
					uint32_t iterations)
{
	...
	for (uint32_t i = 0; i < iterations; i++) {
		...
		if ((tx_len > 0U) && (rx_len > 0U)) {
			ret = i2c_write_read(dev, SLV_I2C_ADDR, tx_data,
		...
		if ((tx_len > 0U) && (rx_len > 0U)) {
			ret = i2c_validate_data_match(tx_data, rx_data,
						      tx_len, "stress loop");
			if (ret != 0) {
				failures++;
				  ...
				  uint8_t *data_buf, uint32_t len)
{
	...
	uint32_t copy_len = MIN(len, stress_ctx.buffer_size);

	memcpy(stress_ctx.target_rx, data_buf, copy_len);
	stress_ctx.target_rx_count = copy_len;
	stress_ctx.buf_last_write_len = len;
...
ZTEST(i2c_stress, test_write_large)
{
	...
	zassert_equal(stress_ctx.stop_count, 1U,
		      "stop callback count %u", stress_ctx.stop_count);
	...
	zassert_equal(stress_ctx.buf_last_write_len, expected_len,
			  ...
			  stress_ctx.buf_last_write_len, expected_len);
#else
	expected_len = tx_len;
...
ZTEST(i2c_stress, test_write_byte_exact)
{
	...
	zassert_equal(stress_ctx.write_requested_count, 1U,
			  ...
			  stress_ctx.write_requested_count);
	zassert_equal(stress_ctx.stop_count, 1U,
		      "stop count %u", stress_ctx.stop_count);
...
{
	static const uint8_t len_cases[] = {
	...
	for (uint32_t i = 0U; i < ARRAY_SIZE(len_cases); i++) {
		...
		expected_len = MIN(tx_len,
				   (uint32_t)CONFIG_I2C_TAR_DATA_BUF_MAX_LEN);
		zassert_equal(stress_ctx.buf_write_received_count, 1U,
			      "Case %u (len=%u): buf_write count %u",
				  i, tx_len,
				  stress_ctx.buf_write_received_count);
		zassert_equal(stress_ctx.buf_last_write_len, expected_len,
		...
		zassert_equal(stress_ctx.write_received_count, expected_len,
				  ...
				  stress_ctx.write_received_count);
		zassert_equal(stress_ctx.stop_rx_snapshot, expected_len,
			      "Case %u (len=%u): stop saw %u bytes",
				  i, tx_len, stress_ctx.stop_rx_snapshot);
#endif

		zassert_equal(stress_ctx.target_rx_count, expected_len,
			      "Case %u (len=%u): target_rx_count %u",
				  i, tx_len, stress_ctx.target_rx_count);
		match_ret = validate_data_match(
			stress_ctx.master_tx, stress_ctx.target_rx,
			expected_len,
			"Length-sweep Master TX -> Target RX");
...
ZTEST(i2c_stress, test_write_repeat_100x32)
{
	...
	for (uint32_t i = 1U; i < STRESS_LOOP_COUNT; i++) {
		...
		zassert_equal(stress_ctx.write_requested_count, 1U,
			      "Iter %u: write_requested count %u",
				  i, stress_ctx.write_requested_count);
		zassert_equal(stress_ctx.stop_count, 1U,
			      "Iter %u: stop count %u",
				  i, stress_ctx.stop_count);

#ifdef CONFIG_I2C_TARGET_BUFFER_MODE
		zassert_equal(stress_ctx.buf_write_received_count, 1U,
			      "Iter %u: buf_write_received count %u",
				  i, stress_ctx.buf_write_received_count);
			zassert_equal(stress_ctx.buf_last_write_len,
				      expected_len,
				      "Iter %u: buf_write len %u, exp %u",
					  i, stress_ctx.buf_last_write_len,
					  expected_len);
#else
			zassert_equal(stress_ctx.write_received_count,
				      expected_len,
				      "Iter %u: write_received %u, exp %u",
					  i, stress_ctx.write_received_count,
					  expected_len);
#endif

		zassert_equal(stress_ctx.target_rx_count, expected_len,
			      "Iter %u: target_rx_count %u, expected %u",
				  i, stress_ctx.target_rx_count, expected_len);
...
ZTEST(i2c_stress, test_write_oversize)
{
	...
	match_ret = validate_data_match(
		tx_oversize, stress_ctx.target_rx, BUFF_PERF,
		"Oversize Master TX -> Target RX (clamped)");
	zassert_equal(match_ret, 0,
...
ZTEST(i2c_stress, test_read_repeat_100)
{
	...
	for (uint32_t i = 0U; i < STRESS_LOOP_COUNT; i++) {
		...
		zassert_equal(stress_ctx.read_requested_count, rx_len,
			      "Iter %u: read_requested %u, exp %u",
				  i, stress_ctx.read_requested_count, rx_len);
		zassert_equal(stress_ctx.read_processed_count, rx_len,
			      "Iter %u: read_processed %u, exp %u",
				  i, stress_ctx.read_processed_count, rx_len);
		zassert_equal(stress_ctx.stop_count, 1U,
			      "Iter %u: stop count %u", i,
				  stress_ctx.stop_count);
...
ZTEST(i2c_stress, test_wr_callback_bal)
{
	...
	zassert_equal(stress_ctx.read_requested_count, 0U,
			  ...
			  stress_ctx.read_requested_count);
	zassert_equal(stress_ctx.stop_count, 1U,
		      "stop count %u, expected 1", stress_ctx.stop_count);
	...
	zassert_equal(stress_ctx.stop_count, 2U,
		      "stop count %u, expected 2", stress_ctx.stop_count);
...
ZTEST(i2c_stress, test_buf_write_max)
{
	...
	const uint32_t expected_len =
		MIN(tx_len, (uint32_t)CONFIG_I2C_TAR_DATA_BUF_MAX_LEN);
	int ret;
	int match_ret;
	...
	ret = master_write_transfer(stress_ctx.master_tx, tx_len);
	zassert_equal(ret, 0,
		      "Master write over-max transfer failed: %d", ret);
	zassert_equal(stress_ctx.buf_write_received_count, 1U,
	...
	match_ret = validate_data_match(stress_ctx.master_tx,
					...
					"Master TX -> Target RX");
	zassert_equal(match_ret, 0,
		      "Truncated buffer-mode payload mismatch");
}
...
ZTEST(i2c_stress, test_wr_restart_integrity)
{
	...
	zassert_equal(stress_ctx.buf_last_write_len, expected_write_len,
		      "buf_write_received len %u, expected %u",
			  stress_ctx.buf_last_write_len, expected_write_len);
#else
	...
	zassert_equal(match_ret, 0,
		      "Restart read phase data integrity failed");
