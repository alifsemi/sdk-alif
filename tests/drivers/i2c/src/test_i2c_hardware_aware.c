				   ...
				   uint16_t addr)
{
	...
	ret = i2c_write(controller, tx_data, len, addr);
	if (ret != 0) {
		return ret;
	}
					...
					uint8_t *rx_data, size_t len)
{
	...
	ret = i2c_write_read(controller, addr, tx_data, len, rx_data, len);
	if (ret != 0) {
		return ret;
	}
				  ...
				  uint8_t *rx_data, size_t len)
{
	...
	ret = i2c_read(controller, rx_data, len, addr);
	if (ret != 0) {
		return ret;
	}
...
ZTEST(i2c_hardware_suite, test_irq_robustness)
{
	...
	for (int i = 0; i < 100; i++) {
		...
		ret = i2c_write(controller, &test_data[0], 1, TGT_I2C_ADDR);
		...
		if (ret == 0) {
			...
			i2c_test_prime_buffers(NULL, 0U, 1U);
			ret = i2c_write(controller, &test_data[1], 1,
					TGT_I2C_ADDR);
	...
	for (int i = 0; i < 50; i++) {
		...
		for (int j = 0; j < 10; j++) {
			...
			ret = i2c_write(controller, &test_data[j % 4], 1,
					TGT_I2C_ADDR);
			k_sched_unlock();
...
ZTEST(i2c_hardware_suite, test_clock_stretch)
{
	 ...
	 * from i2c_test_get_enabled_freqs() and configures both master
	 * and slave to that speed. A nested enabled-frequency loop here
...
ZTEST(i2c_hardware_suite, test_signal_integrity)
{
	 ...
	 * and programs both master and slave to that speed. A nested
...
/**
 * @brief Test repeated-START between two write phases (no intervening STOP)
 *
 * Objective: Exercise the WRITE -> RESTART -> WRITE bus pattern and verify
 * that the target driver delivers write_requested() once per write phase
 * (i.e. twice total), per the Zephyr I2C target API contract.
 *
 * Hardware/Driver issue exposed:
 *   In the DesignWare I2C slave ISR, write_requested() is gated by the
 *   transition out of I2C_DW_CMD_SEND. That transition normally happens
 *   on STOP_DET. When two write phases are stitched together by a
 *   repeated START (no STOP in between), only START_DET marks the phase
 *   boundary. If START_DET is not unmasked in the slave register path,
 *   dw->state remains CMD_SEND across the restart and the second phase's
 *   rx_full skips write_requested(), yielding a count of 1 instead of 2.
 *
 * Expected (with fix): write_requested_count == 2
 * Observed (without fix): write_requested_count == 1
 */
ZTEST(i2c_hardware_suite, test_restart_write_write)
{
	const struct device *controller = hw_ctx.controller;
	...
	for (int f = 0; f < freq_count; f++) {
		...
		ret = i2c_transfer(controller, msgs, ARRAY_SIZE(msgs),
				   TGT_I2C_ADDR);
		...
		/* Contract check: write_requested must fire once per
		 * write phase (i.e. twice). This is the symptom of the
		 * missing START_DET unmask in the slave ISR path.
		 */
		zassert_equal(i2c_test_ctx.write_requested_count, 2U,
			      "write_requested fired %u times at %s, expected 2 "
			      "(missing START_DET unmask in slave ISR?)",
			      i2c_test_ctx.write_requested_count,
			      freqs[f].name);

		/* Every byte should have been delivered to write_received */
		zassert_equal(i2c_test_ctx.write_received_count,
			      sizeof(expected),
			      "write_received fired %u times at %s, expected %zu",
			      i2c_test_ctx.write_received_count,
			      freqs[f].name, sizeof(expected));
	}
...
/**
 * @brief Test repeated-START between two read phases (no intervening STOP)
 *
 * Objective: Exercise the READ -> RESTART -> READ bus pattern and verify
 * that the target driver delivers read_requested() once per read phase
 * (i.e. twice total), per the Zephyr I2C target API contract:
 *   read_requested()  - invoked ONCE at the start of a read transaction
 *   read_processed()  - invoked for every subsequent byte
 *
 * Hardware/Driver issue exposed:
 *   In the DesignWare I2C slave ISR, dw->read_in_progress latches true
 *   on the first RD_REQ and was previously cleared only on STOP_DET.
 *   When a master ends a read phase with a repeated START (no STOP),
 *   the slave sees TX_ABRT (master NACK on last byte) and START_DET,
 *   but read_in_progress remained true. The next read phase's RD_REQ
 *   then invoked read_processed() for the first byte instead of
 *   read_requested(), violating the API contract.
 *
 * Expected (with fix): read_requested_count == 2
 * Observed (without fix): read_requested_count == 1
 */
ZTEST(i2c_hardware_suite, test_restart_read_read_tx_abrt)
{
	const struct device *controller = hw_ctx.controller;
	uint8_t rx1[8];
	uint8_t rx2[8];
	...
	for (int f = 0; f < freq_count; f++) {
		 ...
		 * between them. Master NACKs the last byte of phase 1
		 * (standard I2C behavior), which causes TX_ABRT on the
		 * slave side; only START_DET marks the boundary before
		...
		/* Contract check: read_requested must fire once per
		 * read phase (i.e. twice). Failure here indicates
		 * read_in_progress was not cleared on the TX_ABRT /
		 * START_DET boundary between the two read phases.
		 */
		zassert_equal(i2c_test_ctx.read_requested_count, 2U,
			      "read_requested fired %u times at %s, expected 2 "
			      "(read_in_progress not cleared on tx_abrt/start_det?)",
			      i2c_test_ctx.read_requested_count,
			      freqs[f].name);

		/* Subsequent-byte callback should fire for every byte
		 * that is not the "first byte of a phase": 2 phases * 8
		 * bytes = 16 total bytes, minus 2 first-bytes handled by
		 * read_requested, yields 14 read_processed invocations.
		 */
		zassert_equal(i2c_test_ctx.read_processed_count,
			      (sizeof(rx1) + sizeof(rx2)) - 2U,
			      "read_processed fired %u times at %s, expected %zu",
			      i2c_test_ctx.read_processed_count,
			      freqs[f].name,
			      (sizeof(rx1) + sizeof(rx2)) - 2U);

		/* No stray "read_processed before read_requested" flag */
		zassert_equal(i2c_test_ctx.callback_errors &
				      I2C_CB_ERR_READ_PROCESSED_BEFORE_REQUEST,
			      0U,
			      "callback_errors 0x%x at %s "
			      "(read_processed before read_requested)",
			      i2c_test_ctx.callback_errors, freqs[f].name);
	}
