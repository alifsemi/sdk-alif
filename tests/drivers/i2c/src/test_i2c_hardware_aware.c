...
static void hw_scenario_irq_robustness(void)
{
	...
	for (int i = 0; i < 50; i++) {
		/* Generate distinct target TX pattern - must differ from controller TX */
		fill_target_tx_distinct(hw_target_tx_pattern, 2U, SEED_TARGET_TX);

		/* Prime target with distinct TX data, prepare target RX */
		i2c_test_prime_buffers(hw_target_tx_pattern, 2U, 2U);
		...
		if (ret == 0) {
			...
			zassert_ok(ret,
				   ...
				   i, ret);
			ret = validate_controller_rx(hw_target_tx_pattern, rx_buffer, 2U);
			zassert_ok(ret,
	...
	for (int i = 0; i < 10; i++) {
		...
		if (ret == -ETIMEDOUT) {
			...
			zassert_ok(ret,
				   ...
				   ret);
			register_target_speed_i2c(
				I2C_SPEED_SET(freqs[f].zephyr_speed));
...
static void i2c_hardware_suite_after(void *fixture)
{
	struct i2c_test_ctx *ctx = fixture;
	int ret;
...
ZTEST_SUITE(i2c_hardware_suite, NULL, i2c_hardware_suite_setup,
	    i2c_hardware_suite_before, i2c_hardware_suite_after, NULL);
