...
static void test_start_i2c_transmit(void)
{
	...
	for (i = 0; i < freq_count; i++) {
		...
		register_slave_speed_i2c(I2C_SPEED_SET(freqs[i].zephyr_speed));
 ...
 * Function to test master slave transceive at all enabled frequencies
 */
...
static void test_start_i2c_transceive(void)
{
	...
	for (i = 0; i < freq_count; i++) {
		TC_PRINT("Testing XCV at %s (%u Hz)\n", freqs[i].name,
			 freqs[i].freq_hz);
 ...
 * Function to test master slave with 7 bit addressing mode at all enabled
 * frequencies
 */
 ...
 * Function to test master and slave with 10 bit addressing modes at all enabled
 * frequencies
 */
static void test_start_i2c_10Bit_transfer(void)
{
	...
	for (i = 0; i < freq_count; i++) {
		...
		/* 10-bit uses 10-bit slave registration */
...
ZTEST(i2c, test_nack_recovery)
{
	...
	for (i = 0; i < freq_count; i++) {
		...
		ret = i2c_test_configure_master_freq(i2c_master_dev,
						     &i2c_test_ctx, &freqs[i]);
		if (ret == -ENOTSUP) {
			TC_PRINT("Skipping %s: not supported\n", freqs[i].name);
			continue;
