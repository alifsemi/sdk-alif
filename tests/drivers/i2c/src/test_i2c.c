					...
					uint8_t val)
{
	...
	if (i2c_test_ctx.tgt_rx_cnt < tgt_rx_expected) {
		...
		LOG_DBG("NACKing extra byte 0x%02X", val);
		return -ENOMEM;     /* tells controller to NACK */
	}
	return 0;
}
...
/*
 * Perform a bidirectional I2C transfer: the controller writes
 * random data to the target and then reads back from it.
 * Data integrity is validated for both directions.
 */
...
void register_target_i2c_10Bit(uint32_t i2c_cfg)
{
	register_target_i2c_common(i2c_cfg,
				  TGT_I2C_10BITADDR,
...
static int sc_10bit(bool async)
{
	...
	for (int i = 0; i < freq_count; i++) {
		...
		register_target_i2c_10Bit(I2C_SPEED_SET(freqs[i].zephyr_speed));
