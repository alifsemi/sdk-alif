...
struct bme680_fixture {
	const struct device *dev;
};

static struct bme680_fixture bme680_fix;

static void *bme680_setup(void)
{
	bme680_fix.dev = DEVICE_DT_GET_ONE(bosch_bme680);
	zassert_true(device_is_ready(bme680_fix.dev),
		     "BME680 device not ready");
	return &bme680_fix;
}

ZTEST_F(i2c_sensor, test_bme680_read)
{
	const struct device *const dev = fixture->dev;
...
ZTEST_SUITE(i2c_sensor, NULL, bme680_setup, NULL, NULL, NULL);
