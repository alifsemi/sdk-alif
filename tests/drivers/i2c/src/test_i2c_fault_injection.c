...
ZTEST(i2c_fault_suite, test_config_edge)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(invalid_configs); i++) {
		...
		zassert_not_equal(ret, 0,
			"Invalid config %zu (0x%08x) was accepted",
			i, invalid_configs[i]);
	}
...
static void *i2c_fault_suite_setup(void)
{
	...
	/* Target registration is handled in before() hook to ensure
...
static void i2c_fault_suite_before(void *fixture)
{
	ARG_UNUSED(fixture);
	int ret;
	uint32_t std_cfg = I2C_MODE_CONTROLLER |
			   I2C_SPEED_SET(I2C_SPEED_STANDARD);

	reset_fault_error_counters();

	/* Reset controller to known speed */
	ret = i2c_configure(fault_ctx.controller, std_cfg);
	zassert_ok(ret, "before: controller reset failed: %d", ret);

	/* Re-register target at the same known speed.
	 * register_target_speed_i2c() unregisters first if already registered.
	 */
	register_target_speed_i2c(I2C_SPEED_SET(I2C_SPEED_STANDARD));
}
