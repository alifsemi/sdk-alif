...
typedef struct mac154app_cmd {
	...
	uint16_t cmd_code;
	/* Dummy parameter whose meaning is upper layer dependent and which is returned
	 * in command complete event and indications sent during command handling.
	 * It can be used as a sequence number for instance.
	 */
	uint16_t dummy;
...
typedef struct mac154app_run_tx_cmd {
	...
	/* Channel */
	uint8_t channel;
	/* acknowledgment_asked */
	uint8_t acknowledgment_asked;
...
typedef struct mac154app_tx_single_cmp_evt {
	...
	/* eventual acknowledgment Received signal strength indicator in dBm */
	int8_t ack_rssi;
	/* eventual acknowledgment timestamp when the frame was received in microseconds. */
...
typedef struct mac154app_error_msg_ind {
	...
	uint16_t dummy;
	/* Type of error that has occurred (see enum #mac154app_err_code) */
	uint16_t err_code;
 ...
 * MAC154APP_PENDINGS_LONG_ID_INSERT, MAC154APP_PENDINGS_LONG_ID_REMOVE MAC154APP_CCA_MODE_SET,
 * MAC154APP_ED_THRESHOLD_SET
 */
/*@TRACE*/
