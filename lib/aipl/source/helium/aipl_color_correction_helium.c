 ...
 *  STATIC PROTOTYPES
 **********************/
aipl_error_t aipl_color_correction_24bit_helium(const void *input, void *output, uint32_t pitch,
 ...
 *   GLOBAL FUNCTIONS
 **********************/
aipl_error_t aipl_color_correction_rgb_helium(const void *input, void *output, uint32_t pitch,
					      uint32_t width, uint32_t height,
					      aipl_color_format_t format, const float *ccm)
{
	switch (format) {
	case AIPL_COLOR_ARGB8888:
		return aipl_color_correction_argb8888_helium(input, output, pitch, width, height,
							     ccm);
	case AIPL_COLOR_RGBA8888:
		return aipl_color_correction_rgba8888_helium(input, output, pitch, width, height,
							     ccm);
	case AIPL_COLOR_ARGB4444:
		return aipl_color_correction_argb4444_helium(input, output, pitch, width, height,
							     ccm);
	case AIPL_COLOR_ARGB1555:
		return aipl_color_correction_argb1555_helium(input, output, pitch, width, height,
							     ccm);
	case AIPL_COLOR_RGBA4444:
		return aipl_color_correction_rgba4444_helium(input, output, pitch, width, height,
							     ccm);
	case AIPL_COLOR_RGBA5551:
		return aipl_color_correction_rgba5551_helium(input, output, pitch, width, height,
							     ccm);
	case AIPL_COLOR_BGR888:
		return aipl_color_correction_bgr888_helium(input, output, pitch, width, height,
							   ccm);
	case AIPL_COLOR_RGB888:
		return aipl_color_correction_rgb888_helium(input, output, pitch, width, height,
							   ccm);
	case AIPL_COLOR_RGB565:
		return aipl_color_correction_rgb565_helium(input, output, pitch, width, height,
							   ccm);
...
aipl_error_t aipl_color_correction_rgb_img_helium(const aipl_image_t *input, aipl_image_t *output,
						  const float *ccm)
{
...
aipl_error_t aipl_color_correction_argb8888_helium(const void *input, void *output, uint32_t pitch,
						   uint32_t width, uint32_t height,
						   const float *ccm)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_color_correction_argb4444_helium(const void *input, void *output, uint32_t pitch,
						   uint32_t width, uint32_t height,
						   const float *ccm)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_color_correction_argb1555_helium(const void *input, void *output, uint32_t pitch,
						   uint32_t width, uint32_t height,
						   const float *ccm)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_color_correction_rgba8888_helium(const void *input, void *output, uint32_t pitch,
						   uint32_t width, uint32_t height,
						   const float *ccm)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_color_correction_rgba4444_helium(const void *input, void *output, uint32_t pitch,
						   uint32_t width, uint32_t height,
						   const float *ccm)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_color_correction_rgba5551_helium(const void *input, void *output, uint32_t pitch,
						   uint32_t width, uint32_t height,
						   const float *ccm)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_color_correction_bgr888_helium(const void *input, void *output, uint32_t pitch,
						 uint32_t width, uint32_t height, const float *ccm)
{
	return aipl_color_correction_24bit_helium(input, output, pitch, width, height, ccm, 2, 1,
						  0);
}

aipl_error_t aipl_color_correction_rgb888_helium(const void *input, void *output, uint32_t pitch,
						 uint32_t width, uint32_t height, const float *ccm)
{
	return aipl_color_correction_24bit_helium(input, output, pitch, width, height, ccm, 0, 1,
						  2);
}

aipl_error_t aipl_color_correction_rgb565_helium(const void *input, void *output, uint32_t pitch,
						 uint32_t width, uint32_t height, const float *ccm)
{
	if (input == NULL || output == NULL) {
		return AIPL_ERR_NULL_POINTER;
