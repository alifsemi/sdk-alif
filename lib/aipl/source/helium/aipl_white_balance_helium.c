 ...
 *  STATIC PROTOTYPES
 **********************/
aipl_error_t aipl_white_balance_24bit_helium(const void *input, void *output, uint32_t pitch,
 ...
 *   GLOBAL FUNCTIONS
 **********************/
aipl_error_t aipl_white_balance_rgb_helium(const void *input, void *output, uint32_t pitch,
					   uint32_t width, uint32_t height,
					   aipl_color_format_t format, float ar, float ag, float ab)
{
	switch (format) {
	case AIPL_COLOR_ARGB8888:
		return aipl_white_balance_argb8888_helium(input, output, pitch, width, height, ar,
							  ag, ab);
	case AIPL_COLOR_RGBA8888:
		return aipl_white_balance_rgba8888_helium(input, output, pitch, width, height, ar,
							  ag, ab);
	case AIPL_COLOR_ARGB4444:
		return aipl_white_balance_argb4444_helium(input, output, pitch, width, height, ar,
							  ag, ab);
	case AIPL_COLOR_ARGB1555:
		return aipl_white_balance_argb1555_helium(input, output, pitch, width, height, ar,
							  ag, ab);
	case AIPL_COLOR_RGBA4444:
		return aipl_white_balance_rgba4444_helium(input, output, pitch, width, height, ar,
							  ag, ab);
	case AIPL_COLOR_RGBA5551:
		return aipl_white_balance_rgba5551_helium(input, output, pitch, width, height, ar,
							  ag, ab);
	case AIPL_COLOR_BGR888:
		return aipl_white_balance_bgr888_helium(input, output, pitch, width, height, ar, ag,
							ab);
	case AIPL_COLOR_RGB888:
		return aipl_white_balance_rgb888_helium(input, output, pitch, width, height, ar, ag,
							ab);
	case AIPL_COLOR_RGB565:
		return aipl_white_balance_rgb565_helium(input, output, pitch, width, height, ar, ag,
							ab);
...
aipl_error_t aipl_white_balance_rgb_img_helium(const aipl_image_t *input, aipl_image_t *output,
					       float ar, float ag, float ab)
{
	...
	return aipl_white_balance_rgb_helium(input->data, output->data, input->pitch, input->width,
					     input->height, input->format, ar, ag, ab);

	return AIPL_ERR_OK;
}

aipl_error_t aipl_white_balance_argb8888_helium(const void *input, void *output, uint32_t pitch,
						uint32_t width, uint32_t height, float ar, float ag,
						float ab)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_white_balance_argb4444_helium(const void *input, void *output, uint32_t pitch,
						uint32_t width, uint32_t height, float ar, float ag,
						float ab)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_white_balance_argb1555_helium(const void *input, void *output, uint32_t pitch,
						uint32_t width, uint32_t height, float ar, float ag,
						float ab)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_white_balance_rgba8888_helium(const void *input, void *output, uint32_t pitch,
						uint32_t width, uint32_t height, float ar, float ag,
						float ab)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_white_balance_rgba4444_helium(const void *input, void *output, uint32_t pitch,
						uint32_t width, uint32_t height, float ar, float ag,
						float ab)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_white_balance_rgba5551_helium(const void *input, void *output, uint32_t pitch,
						uint32_t width, uint32_t height, float ar, float ag,
						float ab)
{
	if (input == NULL || output == NULL) {
...
aipl_error_t aipl_white_balance_bgr888_helium(const void *input, void *output, uint32_t pitch,
					      uint32_t width, uint32_t height, float ar, float ag,
					      float ab)
{
	return aipl_white_balance_24bit_helium(input, output, pitch, width, height, ar, ag, ab, 2,
					       1, 0);
}

aipl_error_t aipl_white_balance_rgb888_helium(const void *input, void *output, uint32_t pitch,
					      uint32_t width, uint32_t height, float ar, float ag,
					      float ab)
{
	return aipl_white_balance_24bit_helium(input, output, pitch, width, height, ar, ag, ab, 0,
					       1, 2);
}

aipl_error_t aipl_white_balance_rgb565_helium(const void *input, void *output, uint32_t pitch,
					      uint32_t width, uint32_t height, float ar, float ag,
					      float ab)
{
	if (input == NULL || output == NULL) {
