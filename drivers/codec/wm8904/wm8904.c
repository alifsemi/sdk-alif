...
static int wreg(const struct wm8904_driver_config *dev_cfg, uint8_t reg, uint16_t val)
{
	return i2c_reg_write_16bit_dt(&dev_cfg->i2c, reg, val);
}

/* Helper function to read a register */
static int rreg(const struct wm8904_driver_config *dev_cfg, uint8_t reg, uint16_t *val)
{
	return i2c_reg_read_16bit_dt(&dev_cfg->i2c, reg, val);
}

/* Configure audio interface format */
static int configure_audio_interface(const struct device *dev, struct audio_codec_cfg *cfg)
{
...
static int configure_clock_rate(const struct device *dev, uint32_t rate)
{
	struct wm8904_data *data = dev->data;
...
static int wm8904_configure(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	/* Configure audio interface format */
	ret = configure_audio_interface(dev, cfg);
	if (ret) {
	...
	ret = configure_clock_rate(dev, sample_rate);
	if (ret) {
...
static void wm8904_start_output(const struct device *dev)
{
	...
	ret = wreg(dev_cfg, WM8904_CLOCK_RATES_1, data->clock_rate);
	if (ret) {
		LOG_DBG("Failed to set sample rate (clock rates 1): %d", ret);
		return;
	}

	/* Set high performance bias and disable bias current generator */
	ret = wreg(dev_cfg, WM8904_BIAS_CONTROL_0, BIAS_CNTL_ISEL_HP_BIAS);
	if (ret) {
		LOG_DBG("Failed to set bias control: %d", ret);
		return;
	}

	/* Enable VMID buffer to unused outputs, vmid reference voltage with fast startup */
	ret = wreg(dev_cfg, WM8904_VMID_CONTROL_0,
		VMID_CNTL0_VMID_BUF_ENA | VMID_CNTL0_VMID_RES_FAST | VMID_CNTL0_VMID_ENA);
	if (ret) {
		LOG_DBG("Failed to enable VMID buffer: %d", ret);
		return;
	}
	...
	if (ret) {
		LOG_DBG("Failed to set VMID reference voltage: %d", ret);
		return;
	}

	/* Enable bias current generator */
	ret = wreg(dev_cfg, WM8904_BIAS_CONTROL_0,
		BIAS_CNTL_ISEL_HP_BIAS | BIAS_CNTL_BIAS_ENA);
	if (ret) {
		LOG_DBG("Failed to enable bias current generator: %d", ret);
		return;
	}

	/* Enable ADC left and right input programmable gain amplifiers */
	ret = wreg(dev_cfg, WM8904_POWER_MANAGEMENT_0,
		PWR_MGMT0_INL_ENA | PWR_MGMT0_INR_ENA);
	if (ret) {
		LOG_DBG("Failed to enable ADC input gain amplifiers: %d", ret);
		return;
	}

	/* Enable left and right headphone output */
	ret = wreg(dev_cfg, WM8904_POWER_MANAGEMENT_2,
		PWR_MGMT2_HPL_PGA_ENA | PWR_MGMT2_HPR_PGA_ENA);
	if (ret) {
		LOG_DBG("Failed to enable headphone output: %d", ret);
		return;
	}

	/* Configure DAC digital settings based on cached parameters */
	ret = wreg(dev_cfg, WM8904_DAC_DIGITAL_1, data->dac_digital_settings);
	if (ret) {
		LOG_DBG("Failed to configure DAC digital settings: %d", ret);
		return;
	}
	 ...
	 */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT12_ZC, 0x0000);
	if (ret) {
		LOG_DBG("Failed to configure output routing: %d", ret);
		return;
	}

	/******************************************************************************************/
	/* Enable charge pump digits. Adjusts output voltage to optimize power consumption */
	ret = wreg(dev_cfg, WM8904_CHARGE_PUMP_0, CHRG_PMP_CP_ENA);
	if (ret) {
		LOG_DBG("Failed to enable charge pump: %d", ret);
		return;
	}

	/* Enable dynamic charge pump power based on real time audio level */
	ret = wreg(dev_cfg, WM8904_CHARGE_PUMP_0, CLS_W0_CP_DYN_PWR);
	if (ret) {
		LOG_DBG("Failed to enable dynamic charge pump power: %d", ret);
		return;
	}
	...
	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_1, 0x0000);
	if (ret) {
		LOG_DBG("Failed to configure FLL control 1: %d", ret);
		return;
	}

	/* Configure FLL parameters using values from wm8904_configure */
	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_2,
		FLL_C2_OUTDIV(data->fll_outdiv) |
		(data->fll_fratio == 8 ? FLL_C2_FRATIO_DIV8 : 0));
	if (ret) {
		LOG_DBG("Failed to configure FLL control 2: %d", ret);
		return;
	}

	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_3, FLL_C3_K(data->fll_k));
	if (ret) {
		LOG_DBG("Failed to configure FLL control 3: %d", ret);
		return;
	}

	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_4, FLL_C4_N(data->fll_n));
	if (ret) {
		LOG_DBG("Failed to configure FLL control 4: %d", ret);
		return;
	}

	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_5, FLL_C5_CLK_REF_SRC_BCLK);
	if (ret) {
		LOG_DBG("Failed to configure FLL control 5: %d", ret);
		return;
	}

	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_1, FLL_C1_FRACN_ENA | FLL_C1_FLL_ENA);
	if (ret) {
		LOG_DBG("Failed to enable FLL: %d", ret);
		return;
	}
	k_msleep(5); /* Delay for FLL startup */

	/* Apply sample rate configuration */
	ret = wreg(dev_cfg, WM8904_CLOCK_RATES_0, data->clock_rate);
	if (ret) {
		LOG_DBG("Failed to configure sample rate: %d", ret);
		return;
	}

	/* Set SYSCLK source to FLL output, Enable system clock, DSP clock enable */
	ret = wreg(dev_cfg, WM8904_CLOCK_RATES_2,
		CLK_RTE2_SYSCLK_SRC | CLK_RTE2_CLK_SYS_ENA | CLK_RTE2_CLK_DSP_ENA);
	if (ret) {
		LOG_DBG("Failed to configure system clock source: %d", ret);
		return;
	}

	/* Apply audio interface format configuration */
	ret = wreg(dev_cfg, WM8904_AUDIO_INTERFACE_1, data->aif_format);
	if (ret) {
		LOG_DBG("Failed to configure audio interface format: %d", ret);
		return;
	}

	/* Set up IN2L and IN2R as the ADC inputs, Single ended mode(default) */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_LEFT_INPUT_1, ANLG_LIN1_IP_SEL_N_IN2L);
	if (ret) {
		LOG_DBG("Failed to configure left input: %d", ret);
		return;
	}
	ret = wreg(dev_cfg, WM8904_ANALOGUE_RIGHT_INPUT_1, ANLG_RIN1_IP_SEL_N_IN2R);
	if (ret) {
		LOG_DBG("Failed to configure right input: %d", ret);
		return;
	}
	...
	if (data->is_mono) {
		...
		ret = wreg(dev_cfg, WM8904_AUDIO_INTERFACE_0, AUD_INT0_AIFADCR_SRC);
		if (ret) {
			LOG_DBG("Failed to configure mono mode: %d", ret);
			return;
		}
	} else {
		ret = wreg(dev_cfg, WM8904_AUDIO_INTERFACE_0, 0);
		if (ret) {
			LOG_DBG("Failed to configure stereo mode: %d", ret);
			return;
		}
	}
	...
	/******************************************************************************************/
	/* Enable DAC and ADC */
	ret = wreg(dev_cfg, WM8904_POWER_MANAGEMENT_6,
		PWR_MGMT6_DACL_ENA | PWR_MGMT6_DACR_ENA | PWR_MGMT6_ADCL_ENA | PWR_MGMT6_ADCR_ENA);
	if (ret) {
		LOG_DBG("Failed to enable DAC and ADC: %d", ret);
		return;
	}
	k_msleep(5); /* Delay for DAC/ADC startup */

	/* Unmute analog input PGA and use 0dB default volume */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_LEFT_INPUT_0, ANLG_LIN0_VOL(0x05));
	if (ret) {
		LOG_DBG("Failed to set left input volume: %d", ret);
		return;
	}

	ret = wreg(dev_cfg, WM8904_ANALOGUE_RIGHT_INPUT_0, ANLG_RIN0_VOL(0x05));
	if (ret) {
		LOG_DBG("Failed to set right input volume: %d", ret);
		return;
	}

	/* Enable headphone output stages in sequence */
	/* Enable input stage of headphones */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_HP_0, ANLG_HP0_HPL_ENA | ANLG_HP0_HPR_ENA);
	if (ret) {
		LOG_DBG("Failed to enable headphone input stage: %d", ret);
		return;
	}

	/* Enable intermediate stage of headphones */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_HP_0,
		ANLG_HP0_HPL_ENA |
		ANLG_HP0_HPR_ENA |
		ANLG_HP0_HPL_ENA_DLY |
		ANLG_HP0_HPR_ENA_DLY);
	if (ret) {
		LOG_DBG("Failed to enable headphone intermediate stage: %d", ret);
		return;
	}

	/* Enable DC servo channels */
	ret = wreg(dev_cfg, WM8904_DC_SERVO_0,
		DC_SRV0_DCS_ENA_CHAN_0 | DC_SRV0_DCS_ENA_CHAN_1 |
		DC_SRV0_DCS_ENA_CHAN_2 | DC_SRV0_DCS_ENA_CHAN_3);
	if (ret) {
		LOG_DBG("Failed to enable DC servo channels: %d", ret);
		return;
	}

	/* Enable DC servo startup mode */
	ret = wreg(dev_cfg, WM8904_DC_SERVO_1,
		DC_SRV1_DCS_TRIG_STARTUP_0 | DC_SRV1_DCS_TRIG_STARTUP_1 |
		DC_SRV1_DCS_TRIG_STARTUP_2 | DC_SRV1_DCS_TRIG_STARTUP_3);
	if (ret) {
		LOG_DBG("Failed to enable DC servo startup mode: %d", ret);
		return;
	}
	...
	if (ret) {
		LOG_DBG("Failed to enable headphone output stage: %d", ret);
		return;
	}
	...
	if (ret) {
		LOG_DBG("Failed to remove shorts from headphone outputs: %d", ret);
		return;
	}

	/* Set headphone volume (both channels) */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT1_LEFT,
		ANLG_OUT1_HPOUTL_VU | ANLG_OUT1_HPOUTL_VOL(data->hp_volume));
	if (ret) {
		LOG_DBG("Failed to set left headphone volume: %d", ret);
		return;
	}

	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT1_RIGHT,
		ANLG_OUT1_HPOUTR_VU | ANLG_OUT1_HPOUTR_VOL(data->hp_volume));
	if (ret) {
		LOG_DBG("Failed to set right headphone volume: %d", ret);
		return;
	}

	k_msleep(100); /* Delay for volume setting to take effect */

	LOG_DBG("Applied cached configuration");

	/* Unmute DAC digital path */
	uint16_t current_dac_settings;

	ret = rreg(dev_cfg, WM8904_DAC_DIGITAL_1, &current_dac_settings);
	if (ret) {
		LOG_DBG("Failed to read current DAC settings: %d", ret);
		return;
	}
	current_dac_settings &= ~DAC_DG1_MUTE;
	ret = wreg(dev_cfg, WM8904_DAC_DIGITAL_1, current_dac_settings);
	if (ret) {
		LOG_DBG("Failed to unmute DAC: %d", ret);
		return;
	}
	...
	ret = rreg(dev_cfg, WM8904_ANALOGUE_OUT1_LEFT, &current_hpoutl_settings);
	if (ret) {
		LOG_DBG("Failed to read current headphone left settings: %d", ret);
		return;
	}
	current_hpoutl_settings &= ~ANLG_OUT1_HPOUTL_MUTE;
	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT1_LEFT, current_hpoutl_settings);
	if (ret) {
		LOG_DBG("Failed to unmute headphone left output: %d", ret);
		return;
	}
	...
	ret = rreg(dev_cfg, WM8904_ANALOGUE_OUT1_RIGHT, &current_hpoutr_settings);
	if (ret) {
		LOG_DBG("Failed to read current headphone right settings: %d", ret);
		return;
	}
	current_hpoutr_settings &= ~ANLG_OUT1_HPOUTR_MUTE;
	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT1_RIGHT, current_hpoutr_settings);
	if (ret) {
		LOG_DBG("Failed to unmute headphone right output: %d", ret);
		return;
	}
...
static void wm8904_stop_output(const struct device *dev)
{
	...
	ret = wreg(dev_cfg, WM8904_DAC_DIGITAL_1, current_dac_settings);
	if (ret) {
		LOG_DBG("Failed to mute DAC: %d", ret);
		return;
	}
	...
	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT1_LEFT, current_hpoutl_settings);
	if (ret) {
		LOG_DBG("Failed to mute headphone left output: %d", ret);
		return;
	}
	...
	ret = wreg(dev_cfg, WM8904_ANALOGUE_OUT1_RIGHT, current_hpoutr_settings);
	if (ret) {
		LOG_DBG("Failed to mute headphone right output: %d", ret);
		return;
	}
	...
	ret = wreg(dev_cfg, WM8904_ANALOGUE_HP_0,
		...
		ANLG_HP0_HPL_ENA | ANLG_HP0_HPR_ENA);
	if (ret) {
		LOG_DBG("Failed to re-apply shorts to headphone outputs: %d", ret);
		return;
	}
	...
	ret = wreg(dev_cfg, WM8904_ANALOGUE_HP_0,
		...
		ANLG_HP0_HPL_ENA | ANLG_HP0_HPR_ENA);
	if (ret) {
		LOG_DBG("Failed to disable headphone output stage: %d", ret);
		return;
	}
	...
	ret = wreg(dev_cfg, WM8904_ANALOGUE_HP_0,
		ANLG_HP0_HPL_ENA | ANLG_HP0_HPR_ENA);
	if (ret) {
		LOG_DBG("Failed to disable headphone intermediate stage: %d", ret);
		return;
	}

	/* Disable input stage of headphones */
	ret = wreg(dev_cfg, WM8904_ANALOGUE_HP_0, 0);
	if (ret) {
		LOG_DBG("Failed to disable headphone input stage: %d", ret);
		return;
	}

	/* Disable DAC and ADC to save power */
	ret = wreg(dev_cfg, WM8904_POWER_MANAGEMENT_6, 0);
	if (ret) {
		LOG_DBG("Failed to disable DAC and ADC: %d", ret);
		return;
	}

	/* Disable clocks to save power */
	ret = wreg(dev_cfg, WM8904_CLOCK_RATES_2, 0);
	if (ret) {
		LOG_DBG("Failed to disable clocks: %d", ret);
		return;
	}

	/* Disable FLL if it was enabled */
	ret = wreg(dev_cfg, WM8904_FLL_CONTROL_1, 0);
	if (ret) {
		LOG_DBG("Failed to disable FLL: %d", ret);
		return;
	}

	/* Disable charge pump */
	ret = wreg(dev_cfg, WM8904_CHARGE_PUMP_0, 0);
	if (ret) {
		LOG_DBG("Failed to disable charge pump: %d", ret);
		return;
	}

	/* Disable VMID */
	ret = wreg(dev_cfg, WM8904_VMID_CONTROL_0, 0);
	if (ret) {
		LOG_DBG("Failed to disable VMID: %d", ret);
		return;
	}

	/* Disable bias generator */
	ret = wreg(dev_cfg, WM8904_BIAS_CONTROL_0, 0);
	if (ret) {
		LOG_DBG("Failed to disable bias generator: %d", ret);
		return;
	}
			       ...
			       audio_channel_t channel, audio_property_value_t val)
{
	...
	case AUDIO_PROPERTY_OUTPUT_MUTE:
		ret = wreg(dev_cfg, WM8904_DAC_DIGITAL_1, val.mute ? DAC_DG1_MUTE : 0);
		if (ret) {
...
static int wm8904_clear_errors(const struct device *dev)
{
	/* This codec doesn't have a specific error register to clear */
	return 0;
}

static int wm8904_register_error_callback(const struct device *dev, audio_codec_error_callback_t cb)
{
...
static const struct audio_codec_api wm8904_driver_api = {
	...
	.clear_errors = wm8904_clear_errors,
	.register_error_callback = wm8904_register_error_callback};
...
int wm8904_init(const struct device *dev)
{
	...
	LOG_DBG("Initializing WM8904 codec");

	/* Reset device and then read ID */
	ret = wreg(dev_cfg, WM8904_SW_RESET_AND_ID, 0xFFFF);
	if (ret) {
	...
	ret = rreg(dev_cfg, WM8904_SW_RESET_AND_ID, &dev_id);
	if (ret) {
	...
	ret = wm8904_configure(dev, &audio_cfg);
	if (ret) {
	...
	wm8904_start_output(dev);
	LOG_DBG("WM8904 output started. TODO: Remove");
