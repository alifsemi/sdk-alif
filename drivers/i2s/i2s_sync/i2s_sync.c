...
static int enable_clock(struct i2s_t *i2s, size_t const bit_depth)
{
	...
	switch (bit_depth) {
	case 16:
		i2s_configure_clk(i2s, WSS_CLOCK_CYCLES_16);
		break;
	case 24:
		i2s_configure_clk(i2s, WSS_CLOCK_CYCLES_24);
		break;
	case 32:
		i2s_configure_clk(i2s, WSS_CLOCK_CYCLES_32);
		break;
	default:
		return -EINVAL;
	}
...
static int i2s_sync_configure_impl(const struct device *dev, struct i2s_sync_config const *cfg)
{
	...
	if (cfg->bit_depth < 16 || (cfg->bit_depth % 16) != 0) {
	...
	ret = enable_clock(i2s, cfg->bit_depth);
	if (ret) {
