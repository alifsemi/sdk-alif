...
{
	/* Get pointer to driver data from the timer container */
	struct ch201_data *dev_data = CONTAINER_OF(timer_id,
				      struct ch201_data, ch201_timer);

	ch_group_trigger(&dev_data->ch201_group);
}
...
static int ch201_init(const struct device *dev)
{
	...
	k_timer_init(&data->ch201_timer, ch201_periodic_timer_trigger, NULL);
		...
		case SENSOR_CH201_ATTR_ENABLE:
			if (val->val1) {
				...
				if (data->dev_config.mode == CH_MODE_TRIGGERED_TX_RX) {
					...
					ch201_periodic_timer_init(dev,
								data->meas_interval_ms);
				} else {
