...
void ScreenLayoutInit(const void *imgData, size_t imgSize, int imgWidth, int imgHeight,
		      unsigned short imgZoom, bool hasProgressBar)
{
	...
	lv_obj_set_flex_align(screen, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_CENTER,
	...
	if (imgData != nullptr) {
		...
		/* (Serves to determine screen layout, as layout can get weird for zoomed image
		 * widgets) */
	...
	if (imgData != nullptr) {
		...
		imageDesc.header.stride =
			imgWidth * (LV_COLOR_FORMAT_GET_SIZE(imageDesc.header.cf));
