...
LOG_MODULE_REGISTER(LiveMicInput, LOG_LEVEL_DBG);
...
bool LiveMicInput::GetInputData(void* buffer) {
    ...
    while (OutputSize > offset) {
        ...
        int stero_samples = slab_size / I2S_SAMPLE_SIZE;

        rc = i2s_mix_mono_output(
            static_cast<int16_t*>(slab),
            stero_samples,
        ...
        k_mem_slab_free(&mem_slab, slab);
        offset += (stero_samples / 2) * sizeof(int16_t);
    }
