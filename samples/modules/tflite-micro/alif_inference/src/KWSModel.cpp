...
// TODO: by having these as global state prevents us from having multiple instances of KWSModel
...
bool KWSModel::Init() {
    ...
    m_pInterpreter = std::make_unique<tflite::MicroInterpreter>(
        tflite::GetModel(arm::app::kws::GetModelPointer()),
        m_resolver,
        tensorArena,
    ...
    if (rc != kTfLiteOk) {
        LOG_ERR("failed to AllocateTensors");
        return false;
    }

    auto* inputTensor = m_pInterpreter->input(0);
    size_t numMfccFeatures = inputTensor->dims->data[arm::app::MicroNetKwsModel::ms_inputColsIdx];
...
bool KWSModel::PreProcess() {
    if (!m_preProcess->DoPreProcess(audio_inf, m_index)) {
        LOG_ERR("preProcess.DoPreProcess failed");
        return false;
    }
...
bool KWSModel::RunInference() {
    ...
    if (rc != kTfLiteOk) {
        LOG_ERR("Failed to run inference");
        return false;
    }
