...
public:
    void ProcessOutput(const T& result) {
        const auto it = std::max_element(result.confidences.begin(), result.confidences.end());
...
int main()
{
    InferenceRunner<KWSModel,
        LiveMicInput,
        PrintHighestConfidence<KWSModel::Result>> 
        runner;

    LOG_INF("Starting runner");
    runner.Start();
