...
LOG_MODULE_REGISTER(AsrPostProcess);
...
namespace app {
    ...
    bool AsrPostProcess::DoPostProcess()
    {
        ...
        /* Other health checks. */
        if (0 == elemSz) {
            LOG_ERR("Tensor type not supported for post processing\n");
            return false;
        } else if (elemSz * this->m_totalLen > this->m_outputTensor->Bytes()) {
            LOG_ERR("Insufficient number of tensor bytes\n");
            return false;
        }
        ...
        if (this->m_reductionAxisIdx == this->m_classifier.m_outputTensorRowsIdx) {
            ...
            LOG_ERR("Unsupported axis index: %" PRIu32 "\n", this->m_reductionAxisIdx);
            return false;
        }
                                      ...
                                      const uint32_t axisIdx) const
    {
        ...
        if (axisIdx >= shape.size()) {
            LOG_ERR("Invalid axis index: %" PRIu32 "; Max: %zu\n", axisIdx, shape.size());
            return false;
        }

        if (this->m_totalLen != shape[axisIdx]) {
            LOG_ERR("Unexpected tensor dimension for axis %" PRIu32 ", got %zu.\n",
                       axisIdx,
                       shape[axisIdx]);
            return false;
        }
    ...
    uint32_t AsrPostProcess::GetNumFeatureVectors() const
    {
        ...
        if (inputRows == 0) {
            LOG_ERR("Error getting number of input rows for axis: %" PRIu32 "\n",
    ...
    uint32_t AsrPostProcess::GetOutputContextLen() const
    {
        ...
        if (inputRows <= 2 * this->m_inputContextLen || inputRows <= inputInnerLen) {
            LOG_ERR("Input rows not compatible with ctx of %" PRIu32 "\n",
