...
LOG_MODULE_REGISTER(AsrClassifier);
...
namespace app {
        ...
        bool use_softmax)
    {
            ...
            if (outputTensor == nullptr) {
                ...
                LOG_ERR("Output tensor expected to be %dD\n", minTensorDims);
                return false;
            } else if (outputShape[outColsIdx] < topNCount) {
                LOG_ERR("Output vectors are smaller than %" PRIu32 "\n", topNCount);
                return false;
            ...
            if (topNCount != 1) {
                LOG_WRN("TopNCount value ignored in this implementation\n");
            }
            ...
            if (!resultState) {
                LOG_ERR("Failed to get sorted set\n");
                return false;
            }
