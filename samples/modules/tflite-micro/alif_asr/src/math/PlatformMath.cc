...
namespace math {
                               ...
                               const FftType type)
    {
        ...
        if (ARM_MATH_SUCCESS != status) {
            LOG_ERR("Failed to initialise FFT for len %d\n", fftLen);
        } else {
                           ...
                           arm::app::math::FftInstance& fftInstance)
    {
        if (!fftInstance.m_initialised) {
            ...
            LOG_ERR("FFT len: %" PRIu16 "; input len: %zu\n",
                fftInstance.m_fftLen, input.size());
            return;
        } else if (fftOutput.size() < input.size()) {
            LOG_ERR("Output vector len insufficient to hold FFTs\n");
            return;
        }
        ...
        case FftType::complex:
            if (input.size() < fftInstance.m_fftLen * 2) {
                LOG_ERR("Complex FFT instance should have input size >= (FFT len x 2)");
                return;
            }
#if (defined(__ARM_FEATURE_DSP) && (__ARM_FEATURE_DSP == 1))
            if (fftInstance.m_optimisedOptionAvailable) {
    ...
    {
        if (dstLen < srcLen/2) {
            LOG_ERR("dstLen must be greater than srcLen/2");
            return false;
        }
