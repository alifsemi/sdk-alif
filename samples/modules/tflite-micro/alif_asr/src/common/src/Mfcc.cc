...
namespace audio {
    ...
    void MfccParams::Log() const
    {
        ...
        LOG_DBG("\t Sampling frequency:         %f\n", (double)this->m_samplingFreq);
        LOG_DBG("\t Number of filter banks:     %" PRIu32 "\n", this->m_numFbankBins);
        LOG_DBG("\t Mel frequency limit (low):  %f\n", (double)this->m_melLoFreq);
        LOG_DBG("\t Mel frequency limit (high): %f\n", (double)this->m_melHiFreq);
        LOG_DBG("\t Number of MFCC features:    %" PRIu32 "\n", this->m_numMfccFeatures);
        LOG_DBG("\t Frame length:               %" PRIu32 "\n", this->m_frameLen);
        LOG_DBG("\t Padded frame length:        %" PRIu32 "\n", this->m_frameLenPadded);
        LOG_DBG("\t Using HTK for Mel scale:    %s\n", this->m_useHtkMethod ? "yes" : "no");
        LOG_DBG("\t Mel energy minimum value:   %f\n", (double)this->m_melEnergyMin);
    }
