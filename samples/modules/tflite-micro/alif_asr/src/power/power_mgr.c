...
#define MEMORY_RETENTION_CONFIG                                                                    \
	(((CONFIG_FLASH_BASE_ADDRESS == 0) ? 0 : MRAM_MASK) | RET_A1 | RET_A2 | RET_B | RET_C |    \
	 RET_D | RET_E | SERAM_MASK)

#elif defined(CONFIG_ENSEMBLE_GEN2)
#define MEMORY_RETENTION_CONFIG                                                                    \
	(((CONFIG_FLASH_BASE_ADDRESS == 0) ? 0 : MRAM_MASK) | RET_A1 | RET_A2 | RET_B |            \
	 SERAM_MASK | SRAM0_1_RET_MASK | SRAM0_2_RET_MASK | SRAM0_3_RET_MASK | SRAM0_4_RET_MASK)
#else
#define MEMORY_RETENTION_CONFIG                                                                    \
	(((CONFIG_FLASH_BASE_ADDRESS == 0) ? 0 : MRAM_MASK) | RET_A1 | RET_A2 | RET_B | SERAM_MASK)
#endif

#define DEFAULT_DCDC_VOLTAGE 825
...
static int set_run_params(void)
{
	...
	run_profile_t runp = {
		...
		.memory_blocks = MEMORY_RETENTION_CONFIG | SRAM0_MASK | SRAM1_MASK,
#endif
	};
