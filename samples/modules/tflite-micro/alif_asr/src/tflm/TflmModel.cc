...
std::string InferNpuMemoryModeFromEthosUOp(const tflite::Model* model)
{
    ...
    for (uint32_t i = 0; i < subgraph->operators()->size(); ++i) {
		...
		// we can cast to uint32_t since ethos_u_opcode is guaranteed to be non-negative
        if (!op || op->opcode_index() != (uint32_t)ethos_u_opcode) {
            continue;
        }
                     ...
                     const void* backendData)
{
    ...
    LOG_INF("loading model from @ %p\n", nnModel.data);
    LOG_INF("model size: %" PRIu32 " bytes.\n", nnModel.size);
    ...
    if (!this->m_backendData.m_pAllocator) {
        ...
        LOG_INF("Creating allocator using tensor arena at 0x%p\n", computeBuffer.data);
        ...
        LOG_INF("Created new allocator @ 0x%p\n", this->m_backendData.m_pAllocator);
    } else {
        LOG_INF("Using existing allocator @ 0x%p\n", this->m_backendData.m_pAllocator);
    }
...
void TflmModel::LogTensorInfo(std::shared_ptr<iface::TensorIface> tensor)
{
    ...
    LOG_INF("\ttensor is assigned to 0x%p\n", tensor.get());
    LOG_INF("\ttensor type is %s\n", GetTensorDataTypeName(tensor->Type()));
    LOG_INF("\ttensor occupies %zu bytes with dimensions\n", tensor->Bytes());
...
void TflmModel::LogInterpreterInfo()
{
    ...
    LOG_INF("Number of operators: %" PRIu32 "\n", nOperators);
