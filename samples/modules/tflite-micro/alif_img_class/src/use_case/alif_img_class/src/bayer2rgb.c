...
{
	LOG_DBG(">>> 1 bayer:0x%X rgb:0x%X sx:0x%X sy:0x%X tile:0x%X <<<",(uint32_t)bayer,(uint32_t)rgb,sx,sy,tile);
	...
	LOG_DBG(">>> dc1394_bayer_Simple 1 <<<");

	LOG_DBG(">>> 2 bayer:0x%X rgb:0x%X sx:0x%X sy:0x%X tile:0x%X imax:0x%X iinc:0x%X i:0x%X<<<",(uint32_t)bayer,(uint32_t)rgb,sx,sy,tile,imax,iinc,i);
	...
	LOG_DBG(">>> dc1394_bayer_Simple 11 <<<");

	iinc = (sx - 1) * 3;

	LOG_DBG(">>> bayer:0x%X rgb:0x%X sx:0x%X sy:0x%X tile:0x%X imax:0x%X iinc:0x%X i:0x%X<<<",(uint32_t)bayer,(uint32_t)rgb,sx,sy,tile,imax,iinc,i);
	...
	LOG_DBG(">> dc1394_bayer_Simple 111 <<<");

	rgb += 1;
	width -= 1;
	height -= 1;

	LOG_DBG(">>> dc1394_bayer_Simple 2 <<<");
...
CE(, [over_count] "+Te"(over_count), [under_count] "+Te"(under_count))
   ...
   : "q0", "q1", "q2", "q3", "q4", "lr", "cc", "memory");
	...
	LOG_DBG(">>> dc1394_bayer_Simple END <<<");
