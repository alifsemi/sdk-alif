...
/* Crops a packed RAW8 Bayer image in place.
 * After return, the cropped image occupies the top-left crop_width x crop_height
 * region of the same buffer, with stride == crop_width (packed).
 */
    ...
    int crop_height)
{
    ...
    /* Bounds safety */
    assert(crop_x >= 0 && crop_y >= 0);
    assert(crop_x + crop_width  <= src_width);
    assert(crop_y + crop_height <= src_height);

    const int src_stride = src_width;     // packed RAW8
    const int dst_stride = crop_width;    // packed result

    /* Copy rows forward into the top-left.
     * Destination rows are earlier in memory than source rows => safe.
	 */
    for (int y = 0; y < crop_height; ++y)
    {
        uint8_t* dst_row       = buf + y * dst_stride;
        const uint8_t* src_row = buf + (crop_y + y) * src_stride + crop_x;

        memmove(dst_row, src_row, (size_t)crop_width);
    }
}
