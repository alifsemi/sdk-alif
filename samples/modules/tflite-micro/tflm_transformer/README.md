...
**boards/alif_e8_dk_rtss_hp.overlay:**
```dts
/* Enable SRAM1 and OSPI1 for HP core */
&sram1 {
    status = "okay";
};
...
**boards/enable_ethosu85.overlay:**
```dts
/* Configure Ethos-U85 NPU */
&ethosu {
    status = "okay";
};
```
...
├── README.md                         # This file
├── README.rst                        # ReStructuredText version
├── BUILDING.md                       # Detailed build guide
│
├── boards/
│   ├── alif_e8_dk_rtss_hp.overlay   # HP core device tree
│   ├── alif_e8_dk_rtss_he.overlay   # HE core device tree
│   └── enable_ethosu85.overlay       # NPU configuration
│
└── src/
    ├── main.cpp                      # Application entry
    └── inference_process_bert.cpp    # BERT inference wrapper

Model Files (alif/include/ethosu/models/bert_tiny/u85/):
├── model_u85_256.h                   # Model header (arena size config)
├── model_u85_256.c                   # Model data (869KB)
├── input.h                           # Sample input data
└── output.h                          # Expected output data
```
...
## Migration Notes (Zephyr 3.6 → 4.1)

### Changes Made

✅ Updated deprecated `CONFIG_BUILD_NO_GAP_FILL`  
✅ Core-specific tensor arena sizing via Kconfig  
✅ Memory-optimized for DTCM constraints  
✅ Stack size reductions (8KB→4KB main, 4KB→2KB workqueue)  
✅ Heap size reductions (32KB→16KB pool, 16KB→8KB newlib)  

### Compatibility

- **Zephyr API**: Uses Zephyr 4.1 kernel APIs
- **TFLite Micro**: Compatible with SDK version
- **Device Tree**: Updated for Zephyr 4.1 bindings
- **Build System**: West + CMake (no changes required)
