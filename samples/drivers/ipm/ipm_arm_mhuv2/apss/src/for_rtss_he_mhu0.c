...
int main(void)
{
	...
	msg_received = false;
	msg_sent = false;
	printk("A32 RTSS-HE MHU 0 example on %s\n", CONFIG_BOARD);
	mhu0_r = DEVICE_DT_GET(DT_NODELABEL(rtsshe_apss_mhu0_r));
	mhu0_s = DEVICE_DT_GET(DT_NODELABEL(apss_rtsshe_mhu0_s));
