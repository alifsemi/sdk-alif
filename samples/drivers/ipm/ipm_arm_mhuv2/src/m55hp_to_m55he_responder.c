/* Copyright (C) 2024 Alif Semiconductor - All Rights Reserved.
 * Use, distribution and modification of this code is permitted under the
 * terms stated in the Alif Semiconductor Software License Agreement
 *
 * You should have received a copy of the Alif Semiconductor Software
 * License Agreement with this file. If not, please write to:
 * contact@alifsemi.com, or visit: https://alifsemi.com/license
 *
 */
#include <zephyr/kernel.h>
#include <zephyr/sys/printk.h>
#include <zephyr/drivers/ipm.h>
#include <string.h>

#define ITERATIONS 10
#define M55 "M55-HP"

const struct device *mhu0_r;
const struct device *mhu0_s;
const struct device *mhu1_r;
const struct device *mhu1_s;
const struct device *volatile mhu_s;

static K_SEM_DEFINE(sem_sent, 0, 1);
static K_SEM_DEFINE(sem_received, 0, 1);
static uint32_t tx_msg;

static void recv_cb(const struct device *dev, void *user_data,
		uint32_t id, volatile void *data)
{
	ARG_UNUSED(user_data);

	mhu_s = (dev == mhu0_r) ? mhu0_s : mhu1_s;

	printk("M55-HP: MSG received on %s Ch%d = 0x%x\n",
	       (dev == mhu0_r) ? "MHU0" : "MHU1", id, *((uint32_t *)data));
	k_sem_give(&sem_received);
}

static void send_cb(const struct device *dev, void *user_data,
		uint32_t id, volatile void *data)
{
	ARG_UNUSED(user_data);
	ARG_UNUSED(data);

	printk("M55-HP: MSG sent on %s Ch:%d is 0x%x\n",
	       (dev == mhu0_s) ? "MHU0" : "MHU1", id, tx_msg);
	k_sem_give(&sem_sent);
}

static void send(const struct device *dev, const char *mhu_name)
{
	int wait = 0;

	tx_msg = 0xA5A5FAFA;
	int ret = ipm_send(dev, wait, 0, &tx_msg, sizeof(tx_msg));

	if (ret != 0) {
		printk("M55-HP: ipm_send failed on %s: %d\n", mhu_name, ret);
		return;
	}
	k_sem_take(&sem_sent, K_FOREVER);
}

int main(void)
{
	int i = 0;

	printk("\nM55-HP <-> M55-HE MHU example on %s\n", CONFIG_BOARD);
	mhu0_r = DEVICE_DT_GET(DT_ALIAS(rtssmhu0r));
	mhu0_s = DEVICE_DT_GET(DT_ALIAS(rtssmhu0s));
	mhu1_r = DEVICE_DT_GET(DT_ALIAS(rtssmhu1r));
	mhu1_s = DEVICE_DT_GET(DT_ALIAS(rtssmhu1s));
	if (!device_is_ready(mhu0_r) || !device_is_ready(mhu0_s) ||
	    !device_is_ready(mhu1_r) || !device_is_ready(mhu1_s)) {
		printk("MHU devices not ready\n");
		return -1;
	}

	ipm_register_callback(mhu0_r, recv_cb, NULL);
	ipm_register_callback(mhu0_s, send_cb, NULL);
	ipm_register_callback(mhu1_r, recv_cb, NULL);
	ipm_register_callback(mhu1_s, send_cb, NULL);

	/* Enable Rx MHU interrupt */
	ipm_set_enabled(mhu0_r, true);
	ipm_set_enabled(mhu1_r, true);

	while (i < ITERATIONS) {
		k_sem_take(&sem_received, K_FOREVER);

		/* Capture ISR-written values under interrupt lock to prevent
		 * a second doorbell from clobbering them before we use them.
		 */
		unsigned int key = irq_lock();
		const struct device *target_dev = mhu_s;
		const char *target_mhu = (target_dev == mhu0_s) ? "MHU0" : "MHU1";

		irq_unlock(key);

		send(target_dev, target_mhu);
		++i;
	}
	/* Disable Rx MHU interrupt */
	ipm_set_enabled(mhu0_r, false);
	ipm_set_enabled(mhu1_r, false);

	printk("M55-HP: Test completed successfully!\n");
	return 0;
}
