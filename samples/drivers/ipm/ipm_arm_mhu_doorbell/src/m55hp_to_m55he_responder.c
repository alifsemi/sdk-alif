...
static bool handle_doorbell(void)
{
	...
	uint32_t heap_mem = (local_tx_dev == mhu0_tx_dev) ?
		SHARED_HEAP_HEHP_MHU0_MEM : SHARED_HEAP_HEHP_MHU1_MEM;

	/* The doorbell value is untrusted until it is confined to this heap's
	 * managed memory and can hold the complete message.
	 */
	if (local_rx_addr < heap_mem ||
	    local_rx_addr > heap_mem + SHARED_HEAP_SIZE - sizeof(struct shared_msg)) {
		printk("ERROR: doorbell addr 0x%08x outside heap [0x%08x, 0x%08x)\n",
		       local_rx_addr, heap_mem, heap_mem + SHARED_HEAP_SIZE);
		return false;
	}
