...
static int test_jpeg_encode(const struct device *jpeg_dev)
{
	...
	/* Start streaming — triggers encoding of the queued buffer */
	...
	LOG_INF("Encoding time: %u ms", duration_ms);
...
static int test_quality_levels(const struct device *jpeg_dev)
{
	uint16_t quality_levels[] = {25, 50, 75, 100};
	struct video_format fmt = {0};
	struct video_buffer *dequeued_buf = NULL;
	int ret;
	...
	/* Set input buffer address (same for all quality tests) */
	ret = video_set_ctrl(jpeg_dev, VIDEO_CID_JPEG_INPUT_BUFFER, input_buffer->buffer);
	if (ret < 0) {
		LOG_ERR("Failed to set input buffer: %d", ret);
		return ret;
	...
	LOG_INF("Quality, Size(bytes), Ratio, Time(ms)");
	LOG_INF("----------------------------------------");

	for (int i = 0; i < ARRAY_SIZE(quality_levels); i++) {
		uint16_t quality = quality_levels[i];
		uint32_t start_time, end_time, duration_ms;
		...
		/* Use the same output buffer for all iterations */
		output_buffer->bytesused = OUTPUT_BUFFER_SIZE;
		...
		ret = video_dequeue(jpeg_dev, VIDEO_EP_OUT, &dequeued_buf, K_SECONDS(5));
		if (ret < 0) {
			LOG_ERR("Failed to dequeue: %d", ret);
			continue;
		}
		...
		/* Print results */
		LOG_INF("%3u,     %6u,      %.2f:1,  %4u", quality, dequeued_buf->bytesused,
			(double)INPUT_BUFFER_SIZE / dequeued_buf->bytesused, duration_ms);
		LOG_INF("Jpeg: Capture Image: dump memory "
			"\"/home/$USER/capture_img_q%d.jpg\" 0x%08x 0x%08x\n",
			quality_levels[i], (uint32_t)output_buffer->buffer,
			(uint32_t)output_buffer->buffer + dequeued_buf->bytesused);
	}
	...
	LOG_INF("Quality level test completed!");
	return 0;
}
...
int main(void)
{
	...
	/* Run quality level test */
	ret = test_quality_levels(jpeg_dev);
	if (ret < 0) {
		LOG_ERR("Quality level test failed: %d", ret);
		return ret;
	}
