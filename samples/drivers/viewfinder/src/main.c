...
int main(void)
{
	...
	for (loop_ctr = NUM_CAMS - 1; loop_ctr >= 0; loop_ctr--) {
		...
		while (caps[loop_ctr].format_caps[i].pixelformat) {
			...
			if (fcap->pixelformat == PIPELINE_FORMAT) {
				...
				if (IS_ENABLED(
				    CONFIG_DT_HAS_HIMAX_HM0360_ENABLED)) {
					fmt.width = 320;
					fmt.height = 240;
				} else if (IS_ENABLED(
				    CONFIG_DT_HAS_OVTI_OV5640_ENABLED)) {
					fmt.width = 160;
					fmt.height = 120;
				} else {
	 ...
	 */
	{
		...
		const struct device *_disp =
			DEVICE_DT_GET(DT_CHOSEN(zephyr_display));

		if (!device_is_ready(_disp)) {
			LOG_ERR("Display device not ready for framebuffer");
			return -1;
		}
		cdc200_get_framebuffer(_disp, 0, &fb_l2);
		if (fb_l2.fb_addr == 0 || fb_l2.fb_size < bsize) {
			LOG_ERR("FB too small: addr 0x%x size %u (need %u)",
				(uint32_t)fb_l2.fb_addr, fb_l2.fb_size, bsize);
			return -1;
		}
	...
	LOG_INF("Conversion buffer (framebuffer): addr - 0x%x, size - %d",
		(uint32_t)conv_buf->buffer, bsize);
	...
	/* Blit Alif logo below the camera image area (480x480) */
	blit_logo_to_fb((uint16_t *)conv_buf->buffer,
			capabilities.x_panel_resolution,
			480,
			capabilities.y_panel_resolution);
	sys_cache_data_flush_range(conv_buf->buffer,
		(size_t)capabilities.x_panel_resolution *
		capabilities.y_panel_resolution * 2);
	LOG_DBG("Logo blitted below 480x480 image area");
	...
	while (1) {
		...
		t_begin = k_cyc_to_us_floor32(k_cycle_get_32());
...
#if !ISP_ENABLED
		/* AIPL demosaic path: unpack Y10P to 8-bit Bayer in-place,
		 * then AIPL demosaics 480x480 region directly to framebuffer.
		 * Camera outputs 560x560; we crop to 480x480
		 ...
		 * tightly packed output (pitch=width=480=display pitch),
