...
static int display_init(void)
{
	...
	if (!device_is_ready(display_dev)) {
		LOG_ERR("CDC200 device not ready");
		return -1;
	}
...
static void display_streaming_thread(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		if (!streaming_active) {
			...
			if (THREAD_TO_BE_SUSPEND) {
				...
				if (panel_dev) {
					...
					/*
					 * Block here until the panel driver's
					 * deferred reinit work has actually
					 * completed. Guarantees the panel is
					 * fully re-initialized before actual
					 * stream starts or we ever
					 * proceed to a subsequent SUSPEND, so
					 * suspend's non-blocking work-cancel
					 * never races an in-flight resume.
					 */
					(void)display_pm_wait_ready(panel_dev);
				}
#endif /* defined(CONFIG_MIPI_DSI) */
		...
		if (capabilities.layer[0].layer_en) {
			...
			sys_cache_data_flush_range((void *)fb_l2.fb_addr, fb_l2.fb_size);
