...
LOG_MODULE_REGISTER(pwm_pm, LOG_LEVEL_INF);
...
#define STEP_SLEEP_MSEC 25U
#define FADE_DURATION_MSEC (3 * 1000U)
...
static void dut_run_and_print(const char *phase_label)
{
	uint32_t pulse[ARRAY_SIZE(pwm_leds)];
	uint32_t step[ARRAY_SIZE(pwm_leds)];
	uint8_t dir = 1U;
	int64_t end = k_uptime_get() + FADE_DURATION_MSEC;
	int ret;

	LOG_INF("=== %s: PWM fade ===", phase_label);

	for (size_t i = 0; i < ARRAY_SIZE(pwm_leds); i++) {
		pulse[i] = 0;
		step[i] = pwm_leds[i].period / NUM_STEPS;
	}

	while (k_uptime_get() < end) {
		for (size_t i = 0; i < ARRAY_SIZE(pwm_leds); i++) {
			ret = pwm_set_pulse_dt(&pwm_leds[i], pulse[i]);
			if (ret) {
				LOG_ERR("pwm_set_pulse_dt LED %d err %d", i, ret);
			}

			if (dir) {
				if (pulse[i] + step[i] >= pwm_leds[i].period) {
					pulse[i] = pwm_leds[i].period;
					dir = 0U;
				} else {
					pulse[i] += step[i];
				}
		...
		k_sleep(K_MSEC(STEP_SLEEP_MSEC));
	}

	for (size_t i = 0; i < ARRAY_SIZE(pwm_leds); i++) {
		pwm_set_pulse_dt(&pwm_leds[i], 0);
	}
	LOG_INF("fade done, %u LED(s) idle", ARRAY_SIZE(pwm_leds));
}
...
int main(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(pwm_leds); i++) {
		if (!pwm_is_ready_dt(&pwm_leds[i])) {
			LOG_ERR("PWM device %s not ready", pwm_leds[i].dev->name);
			return 0;
		}
	}
	...
	if (S2RAM_SUPPORTED) {
		LOG_INF("%s (S2RAM): PWM PM demo "
			"(RUNTIME_IDLE, SUSPEND_TO_IDLE, S2RAM STANDBY, S2RAM STOP)",
			CONFIG_BOARD);
	} else {
		LOG_INF("%s (SOFT_OFF): PWM PM demo "
	...
	dut_run_and_print("before RUNTIME_IDLE");
	...
	pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	dut_run_and_print("after SUSPEND_TO_IDLE");
	pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
#else
	...
	if (S2RAM_SUPPORTED) {
		...
		pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		dut_run_and_print("after S2RAM STANDBY");
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		...
		pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		dut_run_and_print("after S2RAM STOP");
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	}
	...
	LOG_INF("=== PWM PM SEQUENCE COMPLETED ===");
