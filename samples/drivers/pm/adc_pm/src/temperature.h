 ...
 * @return   : temperature
 */
static float get_temperature(uint32_t adc_value)
{
	uint32_t i;
	const uint32_t last = ARRAY_SIZE(tempData) - 1;

	/* check for temperature operating range */
	if (adc_value < tempData[0][0] || (adc_value > tempData[last][0])) {
		return -1;
	}
