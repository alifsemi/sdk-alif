...
LOG_MODULE_REGISTER(adc_pm, LOG_LEVEL_INF);
...
static volatile uint32_t adc_sample[10];
...
static int app_set_run_params(void)
{
	...
	runp.memory_blocks = MRAM_MASK |  SRAM0_MASK | SRAM4_MASK | SRAM5_MASK;
...
static void app_pm_lock_deeper_states(bool lock)
{
	...
	if (S2RAM_SUPPORTED) {
		...
		/*
		 * MRAM boot: S2RAM is not a valid target. Lock it once at
		 * startup and never release it so the policy selects SOFT_OFF.
		 */
		static bool s2ram_locked;
...
int main(void)
{
	...
	const struct device *const dev = DEVICE_DT_GET(DT_NODELABEL(adc0));
	...
	if (S2RAM_SUPPORTED) {
		...
		for (int i = 0; i < 3; i++) {
			LOG_INF("Main thread running - iteration %d - tick: %lld",
				i, (long long)k_uptime_ticks());
			k_sleep(K_SECONDS(2));
		}
