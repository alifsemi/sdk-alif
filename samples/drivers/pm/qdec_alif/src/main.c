...
LOG_MODULE_REGISTER(qdec_pm, LOG_LEVEL_INF);
 ...
 * QDEC configuration
 * ========================================================================
 * QDEC has no start/stop API - it decodes continuously once initialized.
 * No mechanical rotator: GPIO0.0 / GPIO0.1 emulate quadrature.
 */

#define QDEC_NODE DT_ALIAS(qdec0)
#define QDEC_SAMPLES 5
#define QENC_STEPS_PER_SAMPLE 25
#define QENC_PHASE_DELAY_US 100

BUILD_ASSERT(DT_NODE_EXISTS(DT_ALIAS(qenca)) && DT_NODE_EXISTS(DT_ALIAS(qencb)),
	"qenca/qencb aliases required — build with -S qdec-pm-mram or -S pm-s2ram-tcm");
...
static int qenc_emulate_init(void)
{
	if (!gpio_is_ready_dt(&phase_a) || !gpio_is_ready_dt(&phase_b)) {
		LOG_ERR("emulator GPIO not ready");
		return -ENODEV;
	}

	gpio_pin_configure_dt(&phase_a, GPIO_OUTPUT_INACTIVE);
	gpio_pin_configure_dt(&phase_b, GPIO_OUTPUT_INACTIVE);
	LOG_INF("GPIO encoder emulator ready");
	return 0;
}

static void qenc_emulate_step(void)
{
	gpio_pin_set_dt(&phase_a, 1);
	k_busy_wait(QENC_PHASE_DELAY_US);
...
static void qenc_emulate_burst(void)
{
	for (int s = 0; s < QENC_STEPS_PER_SAMPLE; s++) {
		qenc_emulate_step();
	}
}
...
/* ========================================================================
 * QDEC helper functions
 * ========================================================================
 */

static void qdec_position_run(const struct device *qdec_dev, const char *phase_label)
{
	struct sensor_value val = { 0 };
	int ret;

	LOG_INF("=== %s: QDEC read ===", phase_label);

	for (int i = 0; i < QDEC_SAMPLES; i++) {
		qenc_emulate_burst();
		...
		LOG_INF("sample %d/%d: position %d deg", i + 1, QDEC_SAMPLES, val.val1);
	}
...
static void pm_notify_entry(enum pm_state state)
{
	...
	uint8_t substate_id = info ? info->substate_id : 0;
...
static void pm_notify_exit(enum pm_state state)
{
	...
	/* gpio_dw has no PM_DEVICE support — re-configure emulator pins after S2RAM */
	if (state == PM_STATE_SUSPEND_TO_RAM && (substate_id == 0U || substate_id == 1U)) {
		int ret = qenc_emulate_init();

		if (ret != 0) {
			LOG_ERR("GPIO encoder re-init failed after S2RAM (err %d)", ret);
		}
	}
}
...
int main(void)
{
	...
	const struct device *const wakeup_dev = DEVICE_DT_GET(WAKEUP_SOURCE);
	const struct device *const qdec_dev = DEVICE_DT_GET(QDEC_NODE);
	int ret;
	...
	__ASSERT(device_is_ready(qdec_dev), "%s: device not ready", qdec_dev->name);

	ret = qenc_emulate_init();
	__ASSERT(ret == 0, "QDEC encoder emulator init failed (err %d)", ret);

	pm_notifier_register(&app_pm_notifier);

	if (S2RAM_SUPPORTED) {
		LOG_INF("%s (S2RAM): QDEC PM demo "
			"(RUNTIME_IDLE, SUSPEND_TO_IDLE, S2RAM STANDBY, S2RAM STOP)",
			CONFIG_BOARD);
	} else {
		LOG_INF("%s (SOFT_OFF): QDEC PM demo "
	...
	qdec_position_run(qdec_dev, "before RUNTIME_IDLE");
	...
	pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	qdec_position_run(qdec_dev, "after SUSPEND_TO_IDLE");
	pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
#else
	...
	if (S2RAM_SUPPORTED) {
		...
		pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		qdec_position_run(qdec_dev, "after S2RAM STANDBY");
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		...
		pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		qdec_position_run(qdec_dev, "after S2RAM STOP");
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	}
	...
	LOG_INF("=== QDEC PM SEQUENCE COMPLETED ===");
