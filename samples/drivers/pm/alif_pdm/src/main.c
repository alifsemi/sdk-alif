...
/* Keep stream config in static storage: dmic_configure may retain pointers. */
static struct dmic_cfg pdm_cfg;
static struct pcm_stream_cfg pdm_stream;

static int pdm_configure(const struct device *pdm_dev)
{
	int rc;

	memset(&pdm_cfg, 0, sizeof(pdm_cfg));
	memset(&pdm_stream, 0, sizeof(pdm_stream));

	pdm_stream.pcm_width = SAMPLE_BIT_WIDTH;
	pdm_cfg.streams = &pdm_stream;
	pdm_cfg.streams[0].mem_slab = &mem_slab;
	pdm_cfg.channel.req_num_streams = 1;
	pdm_cfg.channel.req_num_chan = NUM_CHANNELS;
	pdm_cfg.streams[0].block_size = PCMJ_BLOCK_SIZE;
	pdm_cfg.channel.req_chan_map_lo = PDM_CHANNELS;

	rc = dmic_configure(pdm_dev, &pdm_cfg);
	if (rc < 0) {
...
int main(void)
{
	...
	ret = pdm_configure(pdm_dev);
	__ASSERT(ret == 0, "PDM configure failed (err %d)", ret);
