...
LOG_MODULE_REGISTER(counter_pm, LOG_LEVEL_INF);
...
#define COUNTER_NODE DT_CHILD(DT_ALIAS(timer), counter)
#define COUNTER_ALARM_PERIOD_USEC (2 * 1000 * 1000)
...
static struct k_sem counter_alarm_sem;

static void counter_alarm_cb(const struct device *dev, uint8_t chan,
			 uint32_t ticks, void *user_data)
{
	ARG_UNUSED(dev); ARG_UNUSED(chan); ARG_UNUSED(ticks); ARG_UNUSED(user_data);
	k_sem_give(&counter_alarm_sem);
}

static void counter_alarm_run(const struct device *dut, const char *phase_label)
{
	struct counter_alarm_cfg cfg = {
		.flags = 0,
		.ticks = counter_us_to_ticks(dut, COUNTER_ALARM_PERIOD_USEC),
	...
	LOG_INF("=== %s: counter alarm ===", phase_label);

	counter_start(dut);
	ret = counter_set_channel_alarm(dut, 0, &cfg);
	if (ret) {
		LOG_ERR("set_alarm failed: %d", ret);
		counter_stop(dut);
		return;
	}

	ret = k_sem_take(&counter_alarm_sem, K_USEC(COUNTER_ALARM_PERIOD_USEC + 500000));
	if (ret != 0) {
		LOG_ERR("alarm timeout");
	}
	counter_stop(dut);

	counter_get_value(dut, &value);
	LOG_INF("counter value: %u", value);
}
...
int main(void)
{
	...
	const struct device *const wakeup_dev = DEVICE_DT_GET(WAKEUP_SOURCE);
	const struct device *const dut = DEVICE_DT_GET(COUNTER_NODE);
	int ret;
	...
	__ASSERT(device_is_ready(dut), "%s: device not ready", dut->name);

	k_sem_init(&counter_alarm_sem, 0, 1);

	pm_notifier_register(&app_pm_notifier);

	if (S2RAM_SUPPORTED) {
		LOG_INF("%s (S2RAM): counter PM demo "
			"(RUNTIME_IDLE, SUSPEND_TO_IDLE, S2RAM STANDBY, S2RAM STOP)",
			CONFIG_BOARD);
	} else {
		LOG_INF("%s (SOFT_OFF): counter PM demo "
	...
	counter_alarm_run(dut, "before RUNTIME_IDLE");
	...
	pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	counter_alarm_run(dut, "after SUSPEND_TO_IDLE");
	pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
#else
	...
	if (S2RAM_SUPPORTED) {
		...
		pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		counter_alarm_run(dut, "after S2RAM STANDBY");
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		...
		pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		counter_alarm_run(dut, "after S2RAM STOP");
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	}
	...
	LOG_INF("=== COUNTER PM SEQUENCE COMPLETED ===");
