...
#define N_FRAMES		10
#ifdef CONFIG_DT_HAS_OVTI_OV5675_ENABLED
#define N_VID_BUFF		2
#else
#define N_VID_BUFF		2
#endif
...
#if ISP_ENABLED
static struct video_format input_fmt;
#endif
...
/* Result of the last capture cycle, reported by camera_pm_wait_capture_done() */
...
static int camera_init(void)
{
	...
	LOG_INF("Width - %u, Pitch - %u, Height - %u, Buff size - %u",
		fmt.width, fmt.pitch, fmt.height, (uint32_t)bsize);
	...
	for (i = 0; i < ARRAY_SIZE(buffers); i++) {
		...
		LOG_INF("- addr - 0x%x, size - %u, bytesused - %u",
	...
	while (1) {
		...
		if (!capture_active) {
			...
			if (THREAD_TO_BE_SUSPEND) {
				...
				if (THREAD_TO_BE_STOPPED) {
					LOG_INF("Camera: thread stopping (suspend)");
					return;
				}
				 ...
				 */
#if ISP_ENABLED
				/* Re-apply input format (pipeline/sensor format) */
				ret = video_set_format(video_dev, VIDEO_EP_IN, &input_fmt);
				if (ret) {
					LOG_ERR("Failed to re-set ISP input format: %d", ret);
				}
#endif
				ret = video_set_format(video_dev, VIDEO_EP_OUT, &fmt);
				if (ret) {
			...
			if (ret) {
				...
				/* Report the failure so the waiter fails fast
		...
		for (j = 0; j < N_FRAMES; j++) {
			...
			if (j < N_VID_BUFF) {
				...
				last_timestamp = vbuf->timestamp;
				if (frame_time > 0) {
					fps_x10 = 10000U / frame_time;
					LOG_INF("FPS: %u.%u", fps_x10 / 10U, fps_x10 % 10U);
				} else {
		...
		if (cycle_ret == 0 && frame < N_FRAMES) {
			LOG_ERR("Capture produced only %u of %u frames", frame,
		...
		/* Signal main thread that the capture cycle is done, carrying
		 * the outcome so a truncated cycle is not reported as success.
		 */
		capture_result = cycle_ret;
		k_sem_give(&camera_capture_done_sem);
...
int camera_pm_thread_suspend(void)
{
	...
	ret = k_sem_take(&camera_pm_suspended_sem, K_SECONDS(30));
	if (ret) {
		LOG_ERR("Camera thread did not acknowledge suspend (err: %d)", ret);
		return ret;
	}
...
{
	int ret = k_sem_take(&camera_capture_done_sem, K_SECONDS(30));

	if (ret) {
		return ret;
	}

	/* The cycle may have ended early on a pipeline error, so report the
	 * outcome instead of the mere fact that the thread finished.
	 */
