...
#elif DT_HAS_COMPAT_STATUS_OKAY(ovti_ov5675)
	sensor_dev = DEVICE_DT_GET_ONE(ovti_ov5675);
#else
	sensor_dev = NULL;
#endif
	...
	while (1) {
		...
		for (j = 0; j < N_FRAMES; j++) {
			...
			if (j < N_VID_BUFF) {
				...
				last_timestamp = vbuf->timestamp;
				if (frame_time > 0) {
					LOG_INF("FPS: %u", 1000U / frame_time);
				} else {
...
int camera_pm_thread_suspend(void)
{
	...
	/* PM-suspend CSI2 and DPHY BEFORE the sensor to prevent
	 * PHY TX error interrupts when sensor MIPI lanes drop.
	 */
	if (csi_dev) {
		int ret = pm_device_action_run(csi_dev,
				PM_DEVICE_ACTION_SUSPEND);
		if (ret && ret != -EALREADY && ret != -ENOSYS) {
			LOG_ERR("CSI2 PM suspend failed: %d", ret);
		}
	...
	if (dphy_dev) {
		int ret = pm_device_action_run(dphy_dev,
				PM_DEVICE_ACTION_SUSPEND);
		if (ret && ret != -EALREADY && ret != -ENOSYS) {
			LOG_ERR("DPHY PM suspend failed: %d", ret);
		}
