...
LOG_MODULE_REGISTER(cam_pm, LOG_LEVEL_DBG);
...
static void pm_notify_pre_device_resume(enum pm_state state)
{
	...
	/* Enable HFOSC (38.4 MHz) and CFG (100 MHz) clock for camera */
...
int main(void)
{
	...
	LOG_INF("RTSS_HE: PM states demo WITH Camera Capture");
	...
	LOG_INF("RTSS_HP: PM states demo WITH Camera Capture");
	camera_pm_thread_init();
	camera_pm_thread_start();
