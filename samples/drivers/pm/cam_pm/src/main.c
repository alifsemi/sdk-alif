...
static void app_pm_lock_deeper_states(bool lock)
{
	if (lock) {
		 ...
		 * the 10 s sleep exceeds the reduced min-residency values.
		 */
...
static int app_enter_normal_sleep(uint32_t sleep_usec)
{
	...
	ret = camera_pm_thread_suspend();
	if (ret) {
		LOG_ERR("Failed to suspend camera thread: %d", ret);
		return ret;
	}
...
int main(void)
{
	...
	if (S2RAM_SUPPORTED) {
		...
		ret = camera_pm_wait_capture_done();
		__ASSERT(ret == 0, "Camera capture after STANDBY did not complete (err %d)",
			 ret);
		...
		ret = camera_pm_wait_capture_done();
		__ASSERT(ret == 0, "Camera capture after STOP did not complete (err %d)", ret);
