...
static int configure_and_start_dma(const struct device *dma_dev,
				 ...
				 bool use_hw_sg)
{
	/* source_gather_en is a dma_block_config field — set it on the head block */
	head_block->source_gather_en = use_hw_sg ? 1 : 0;
									...
									bool use_hw_sg)
{
	...
	ret = configure_and_start_dma(dma_dev, &blocks[0], use_hw_sg);
	if (ret) {
		return ret;
	}
...
int main(void)
{
	...
	/* SW scatter-gather test*/
	LOG_INF("Running SW scatter-gather test");
	ret = run_scatter_gather_test(dma_dev, false);
	if (ret == 0) {
	...
	/* HW scatter-gather test*/
	LOG_INF("Running HW scatter-gather test");
	ret = run_scatter_gather_test(dma_dev, true);
	if (ret == 0) {
		LOG_INF("All blocks passed");
	} else {
		LOG_ERR("Test failed");
