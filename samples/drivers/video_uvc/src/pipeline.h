...
/*
 * Encoder output buffer: the JPEG header plus the compressed data. The encoder
 * never writes past it; a frame that does not fit fails with -ENOSPC and is
 * dropped. It must not exceed width * height: for MJPEG the UVC class
 * advertises width * height as the maximum frame size
 * (dwMaxVideoFrameBufferSize / dwMaxVideoFrameSize), so a larger frame would
 * overflow the buffers the host sized from it. At JPEG_QUALITY, frames of a
 * camera scene are typically a small fraction of this.
 */
#define JPEG_OUTPUT_MAX_BYTES (CAPTURE_WIDTH * CAPTURE_HEIGHT)
BUILD_ASSERT(JPEG_OUTPUT_MAX_BYTES <= UVC_FRAME_WIDTH * UVC_FRAME_HEIGHT,
	     "the UVC class advertises width * height as the maximum MJPEG frame size");
