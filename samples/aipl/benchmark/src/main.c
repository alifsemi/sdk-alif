...
static uint8_t __attribute__((section(".alif_sram1"))) d0_heap[D1_HEAP_SIZE];
...
static void print_benchmark_result_table(void)
{
	printk("%" STR(TABLE_COL_WIDTH) "s", " ");

	for (int i = 0; i < NUM_OPERATIONS; ++i) {
		printk("%" STR(TABLE_COL_WIDTH) "s", bench_names[i]);
	}
	printk("\r\n");

	for (int i = 0; i < COLOR_FORMATS; ++i) {
		printk("%" STR(TABLE_COL_WIDTH) "s", aipl_color_format_str(i));

		for (int j = 0; j < NUM_OPERATIONS; ++j) {
			if (bench_results[i][j].avg_time) {
				printk("%10u us %5.1f%%", bench_results[i][j].avg_time,
				       (double)bench_results[i][j].cpu_load * 100);
				printk("%s,%s,%10u,%5.1f%%\r\n", bench_names[j],
				       aipl_color_format_str(i), bench_results[i][j].avg_time,
				       (double)bench_results[i][j].cpu_load * 100);
			} else {
				printk("%" STR(TABLE_COL_WIDTH) "s", "xxxx");
			}
		}
		printk("\r\n");
	}
...
int main(void)
{
	...
	if (aipl_dave2d_init() != D2_OK) {
		LOG_ERR("D/AVE2D initialization falied\n");
		return -1;
	}
