...
gapm_callbacks_t cgms_append_cbs(gapm_callbacks_t *gapm_append_cbs)
{
...
static void on_control_req(uint8_t conidx, uint8_t char_type, uint16_t token, co_buf_t *p_buf)
{
	...
	cgmss_set_value_cfm(conidx, GAP_ERR_NO_ERROR, token);
}
...
/* static void on_set_cccd_req(uint8_t conidx, uint8_t char_type, uint16_t token, co_buf_t *p_buf)
{
    ...
    bool ok = ((char_type == CGMS_CHAR_TYPE_MEASUREMENT) && (value == PRF_CLI_START_NTF))
           || ((char_type != CGMS_CHAR_TYPE_MEASUREMENT) && (value == PRF_CLI_START_IND));

    if (ok) {
        app_env.bond_data.cccd_state_bf |= CO_BIT(char_type);

        // Only let demo send when MEASUREMENT notifications are enabled
        if (char_type == CGMS_CHAR_TYPE_MEASUREMENT) {
...
void cgms_server_configure(void)
{
	uint16_t err;
...
/* void cgms_process(uint16_t measurement)
{
	...
	if (s_shared_ptr && s_shared_ptr->connected && app_env.ready_to_send) {
...
/* void cgms_process(uint8_t conidx, uint16_t measurement)
{
    // Guard 1: shared ptr not set yet 
    if (s_shared_ptr == NULL) {
        return;
    }

    // Guard 2: only run when connected 
    if (!s_shared_ptr->connected) {
        return;
    }

    // Guard 3: conidx must be valid 
    if (conidx == 0xFF) {
        return;
    }

    // Guard 4: only send after CCCD enable happened 
    if (!app_env.ready_to_send) {
        return;
    }
    ...
    send_measurement(conidx, measurement);
    app_env.ready_to_send = false;
} */
...
void cgms_disc_notify(uint16_t reason)
{
	app_env.ready_to_send = false;
	cgms_conidx = 0xFF;  
}