...
struct unicast_client_ase {
	...
	// Codec Configuration
	// bap_cfg_t codec_cfg;
	uint8_t state_bitmask; // \ref enum ase_state_bits
	uint8_t retx_number;
};
...
{
	uint32_t const index = (!bitmask) ? 0 : 32 - __builtin_clz(bitmask);
	return bap_sampling_freq_index_to_hz(index);
}
...
static void configure_streams(struct k_work *p_job)
{
	...
	uint16_t err = bap_uc_cli_create_group(1 + UNICAST_CLIENT_GROUP_TYPE_MEDIA, &grp_param,
	...
	struct bap_cfg codec_cfg = {
		// FIXME: SDU size bigger than 84B cause issues with encoding
...
static void configure_qos(struct k_work *p_job)
{
	...
	for (size_t iter = 0; iter < ARRAY_SIZE(unicast_env.ase_sink); iter++) {
		...
		uint16_t err = bap_uc_cli_configure_qos(p_ase->ase_lid, unicast_env.group_lid[type],
...
static void enable_streaming(struct k_work *const p_job)
{
	...
	bap_cfg_metadata_t cfg_metadata = {
		//.param.context_bf = BAP_CONTEXT_TYPE_MEDIA_BIT | BAP_CONTEXT_TYPE_UNSPECIFIED_BIT,
		//.param.context_bf = BAP_CONTEXT_TYPE_ALL,
...
{
	static const char *cmd_type_str[] = {"DISCOVER",  "CONFIGURE_CODEC", "CONFIGURE_QOS",
...
#else
		// Discover PACS
		uint16_t err = bap_capa_cli_discover(con_lid, GATT_INVALID_HDL, GATT_INVALID_HDL);
		if (GAF_ERR_NO_ERROR != err) {
...
{
	const char *states_str[] = {"IDLE",      "CODEC_CONFIGURED", "QOS_CONFIGURED", "ENABLING",
				      ...
				      const bap_cfg_ptr_t *const p_cfg)
{
	...
	if (ase_lid == (unicast_env.nb_ases_sink - 1)) {
		...
		// TODO: resolve why stack won't work from here!
...
{
	// type: enum bap_capa_cli_cmd_type
	const char *cmd_str[] = {"DISCOVER", "GET", "SET_CFG", "SET_LOCATION"};
...
int unicast_source_configure(void)
{
	...
	uint16_t err = bap_uc_cli_configure(&bap_cli_cbs, &bap_cli_cfg);
	if (GAF_ERR_NO_ERROR != err) {
...
{
	uint16_t err = gaf_scan_start(SCAN_CONFIG_BITS, SCAN_DURATION_SEC, GAP_PHY_1MBPS);
	if (GAF_ERR_NO_ERROR != err) {
