...
int audio_datapath_create(struct audio_datapath_config *cfg)
{
	...
	size_t const num_queues = (!env.sdu_queue_r) ? 1 : 2;
	env.decoder = audio_decoder_create(
		CONFIG_ALIF_BLE_AUDIO_FS_HZ, decoder_stack, CONFIG_LC3_DECODER_STACK_SIZE, queues,
		num_queues, env.audio_queue, CONFIG_ALIF_LC3_FRAME_DURATION_STATIC_CONFIG);
	if (env.decoder == NULL) {
