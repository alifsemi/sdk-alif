...
static int sink_enable(void)
{
	...
	LOG_INF("Sink enabled: pa_lid=%u bcast_id=%02x:%02x:%02x L=%u R=%u chosen_bf=0x%08x",
		sink_env.pa_lid, sink_env.bcast_id.id[0], sink_env.bcast_id.id[1],
		sink_env.bcast_id.id[2], sink_env.left_channel_pos, sink_env.right_channel_pos,
...
static int start_streaming(void)
{
	...
	LOG_INF("Start streaming: grp=%u L=%u R=%u", sink_env.grp_lid, sink_env.left_channel_pos,
		sink_env.right_channel_pos);
				   ...
				   uint16_t metadata_len)
{
	...
	for (uint8_t i = 0; i < nb_subgroups; i++) {
		...
		LOG_DBG("BASS add: get_sgrp[%u] rc=%u bis=0x%08x ctx=0x%04x", i, err, bis_mask,
			meta_ptr.param.context_bf);
		if (err != GAF_ERR_NO_ERROR) {
			break;
		}
	}
				      ...
				      uint16_t metadata_len)
{
	...
	for (uint8_t i = 0; i < nb_subgroups; i++) {
		...
		LOG_DBG("BASS modify source request: get_sgrp[%u] rc=%u bis=0x%08x ctx=0x%04x", i,
			err, bis_mask, meta_ptr.param.context_bf);
		if (err != GAF_ERR_NO_ERROR) {
			break;
		}
	}
					   ...
					   const bap_cfg_metadata_ptr_t *p_metadata)
{
	...
	LOG_INF("Datapath cfg: sr=%uHz octets=%u 10ms=%u", sink_env.datapath_cfg.sampling_rate_hz,
		sink_env.octets_per_frame, sink_env.datapath_cfg.frame_duration_is_10ms);
}
...
{
	LOG_INF("SINK enable_req: grp=%u src=%u con=%u stream_bf=0x%08x", grp_lid, src_lid, con_lid,
		stream_pos_bf);
...
static void sd_handle_add(const struct sd_evt *e)
{
	...
	LOG_INF("Add source request: src=%u con=%u bcast_id=%02x:%02x:%02x", e->src_lid, e->con_lid,
	...
	if (e->pa_sync_req && sink_env.pa_lid == GAF_INVALID_LID) {
		uint16_t rc = bap_bc_scan_pa_synchronize(
			&sink_env.adv_id, 0, BAP_BC_SCAN_REPORT_MASK, SYNCHRONISATION_TIMEOUT,
			SCAN_TIMEOUT, &sink_env.pa_lid);
