...
static int storage_load_bond_data(void)
{
    ...
    err = storage_load(SETTINGS_NAME_KEYS, &app_con_bond_data.keys, sizeof(app_con_bond_data.keys));
    if (err != 0) {
                    ...
                    uint8_t const key_type)
{
    ...
    (void)storage_save(SETTINGS_NAME_BOND_DATA, &app_con_bond_data.bond_data,
             sizeof(app_con_bond_data.bond_data));
}
...
static void on_key_received(uint8_t conidx, uint32_t metainfo, const gapc_pairing_keys_t *p_keys)
{
    ...
    (void)storage_save(SETTINGS_NAME_KEYS, &app_con_bond_data.keys, sizeof(app_con_bond_data.keys));
            ...
            uint16_t const offset, uint16_t const max_len)
{
    ...
    LOG_INF("on_name_get method called");
...
{
    LOG_INF("on_appearance_get method called");
