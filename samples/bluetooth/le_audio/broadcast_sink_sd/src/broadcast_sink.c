...
static inline bool wait_sem(struct k_sem *s, k_timeout_t to)
{
	return k_sem_take(s, to) == 0;
}

static inline void sem_clear(struct k_sem *s)
{
	while (k_sem_count_get(s) > 0) {
		(void)k_sem_take(s, K_NO_WAIT);
	}
}
...
static void sd_teardown_sink_and_wait(void)
{
	if (sink_env.grp_lid != GAF_INVALID_LID) {
		...
		bool ok = wait_sem(&sem_sink_disabled, SD_WAIT);
		if (!ok) {
...
static void on_bass_bcast_code(uint8_t src_lid, uint8_t con_lid, const gaf_bcast_code_t *p_bcast_code)
{
					...
					const bap_cfg_metadata_ptr_t *p_metadata)
{
	...
	LOG_INF("Subgroup: id=%u stream_bf=0x%08x loc_bf=0x%04x frame_oct=%u samp=%u frame_dur=%u frames_sdu=%u",
...
static void on_bap_bc_sink_cmp_evt(uint8_t cmd_type, uint16_t status, uint8_t grp_lid, uint8_t stream_pos)
{
	LOG_INF("Sink cmp event: cmd=%u status=%u grp=%u stream=%u", cmd_type, status, grp_lid, stream_pos);
				...
				const uint16_t *p_conhdl)
{
	...
	case BAP_BC_SINK_ESTABLISHED: {
		LOG_INF("SINK established (streams=0x%08x, nb_bis=%u) -> deferring PA terminate & start_streaming",
...
static void on_bap_bc_sink_enable_req(uint8_t grp_lid, uint8_t src_lid, uint8_t con_lid, uint32_t stream_pos_bf)
{
...
static void sd_handle_remote_scan(const struct sd_evt *e)
{
	if (e->remote_scan_state) {
		...
		bool ok = wait_sem(&sem_scan_started, SD_WAIT);
		if (!ok) {
		...
		bool ok = wait_sem(&sem_scan_stopped, SD_WAIT);
		if (!ok) {
...
static void sd_handle_add(const struct sd_evt *e)
{
	...
	if (e->pa_sync_req && sink_env.pa_lid == GAF_INVALID_LID) {
		uint16_t rc = bap_bc_scan_pa_synchronize(&sink_env.adv_id, 0, BAP_BC_SCAN_REPORT_MASK,
							SYNCHRONISATION_TIMEOUT, SCAN_TIMEOUT,
							&sink_env.pa_lid);
		LOG_INF("PA sync start: rc=%u -> pa_lid=%u", rc, sink_env.pa_lid);
		bool ok = wait_sem(&sem_pa_synced, SD_WAIT);
		if (!ok) {
...
{
	LOG_INF("Modify source request: src=%u con=%u pa_sync=%u", e->src_lid, e->con_lid, e->pa_sync_req);
...
int broadcast_sink_init(void)
{
	...
	err = bap_bc_scan_configure(BAP_ROLE_SUPP_BC_SINK_BIT | BAP_ROLE_SUPP_BC_DELEG_BIT | BAP_ROLE_SUPP_BC_SCAN_BIT,
					&scan_cbs);
	if (err != GAP_ERR_NO_ERROR) {
	...
	err = bap_bc_sink_configure(BAP_ROLE_SUPP_BC_SINK_BIT | BAP_ROLE_SUPP_BC_DELEG_BIT | BAP_ROLE_SUPP_BC_SCAN_BIT,
					&sink_cbs);
	if (err != GAP_ERR_NO_ERROR) {
