					   ...
					   uint8_t const state, bap_qos_cfg_t *const p_qos_cfg)
{
	...
	default: {
		// LOG_ERR("  ... state %u not handled!", state);
		break;
	}
	}
}
						     ...
						     const bap_cfg_ptr_t *p_cfg)
{
	...
	bap_qos_req_t qos_req = {
		...
		.framing = 0,
		.phy_bf = (GAP_PHY_LE_1MBPS | GAP_PHY_LE_2MBPS), // 3,
	...
	// LOG_DBG("    Freq = %s, Duration = %s, Length = %d, Location = 0x%04X, Channels =
	//	"%d",
	//	sampling_freq_name[p_cfg->param.sampling_freq],
	//	frame_dur_name[p_cfg->param.frame_dur], p_cfg->param.frame_octet,
	//	p_cfg->param.location_bf, __builtin_popcount(p_cfg->param.location_bf));
	...
	// unicast_env.datapath_config.pres_delay_us = 27086;
	...
	// Prepare audio data path channel
#if 0
	if (audio_datapath_create_channel(p_cfg->param.frame_octet, ase_lid_cfm) < 0) {
		free(p_ase_codec_cfg);
		p_ase_codec_cfg = NULL;
		goto bap_codec_config_cfm;
	}
#else
...
bap_codec_config_cfm:
	...
	bap_uc_srv_configure_codec_cfm(
		...
		&qos_req, p_ase_codec_cfg, 0 /*PRESENTATION_DELAY_US*/, DATA_PATH_CONFIG);
}
				      ...
				      uint32_t const location_bf_src)
{
	...
	uint16_t err = bap_capa_srv_configure(&capa_srv_cbs, &capa_srv_cfg);
...
int unicast_sink_init(void)
{
	...
	uint16_t err = bap_uc_srv_configure(&bap_uc_srv_cb, &uc_srv_cfg);
	if (err) {
...
#define ADV_TIMEOUT           0 // until GAF_ADV_STOP
#define ADV_SID               1
