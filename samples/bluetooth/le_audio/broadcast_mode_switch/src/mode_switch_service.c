...
struct service_env {
    ...
    bool ntf_ongoing;
    uint16_t ntf_cfg;
    uint8_t active_mode;  /* Current mode: 0=idle, 1=sink, 2=source */
};
...
static const gatt_att_desc_t mss_att_db[MSS_IDX_NB] = {
    ...
    [MSS_IDX_MODE_TOGGLE_VAL] = {
        {0xFC, 0x34, 0x9B, 0x5F, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 
    ...
    [MSS_IDX_MODE_STATUS_VAL] = {
        {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x80, 0x00, 0x00, 0x80, 0x5F, 0x9B, 0x34, 0xFD}, 
        ATT_UUID(128) | PROP(RD) | PROP(N), 
        OPT(NO_OFFSET)
    },
    [MSS_IDX_MODE_STATUS_NTF_CFG] = {ATT_128_CLIENT_CHAR_CFG, ATT_UUID(16) | PROP(RD) | PROP(WR), 0},
};
...
static uint16_t send_mode_notifications(uint32_t conidx_mask)
{
    ...
    memcpy(co_buf_data(p_buf), &env.active_mode, sizeof(env.active_mode));

    status = gatt_srv_event_send(conidx, env.user_lid, MSS_METAINFO_MODE_STATUS_NTF_SEND, GATT_NOTIFY,
                            ...
                            uint16_t hdl, uint16_t offset, uint16_t max_length)
{
    ...
    do {
        ...
        case MSS_IDX_MODE_TOGGLE_VAL:
            att_val_len = sizeof(env.active_mode);
            att_val = &env.active_mode;
            LOG_DBG("Read mode toggle characteristic");
            break;

        case MSS_IDX_MODE_STATUS_VAL:
            att_val_len = sizeof(env.active_mode);
            att_val = &env.active_mode;
            LOG_DBG("Read mode status characteristic");
            break;

        case MSS_IDX_MODE_STATUS_NTF_CFG:
            /* This would be the CCC value - for now return 0 */
            att_val_len = sizeof(ccc_value);
            att_val = &ccc_value;
            LOG_DBG("Read mode status CCC");
            break;
                           ...
                           uint16_t offset, co_buf_t *p_data)
{
    ...
    do {
        ...
        uint8_t att_idx = hdl - env.start_hdl;
        LOG_INF("Attribute index: %u (hdl=%u, service_hdl=%u)", att_idx, hdl, env.start_hdl);
        ...
        case MSS_IDX_MODE_TOGGLE_VAL: {
            if (sizeof(uint8_t) != co_buf_data_len(p_data)) {
                ...
                case MODE_SWITCH_SOURCE:
                    ...
                    break;
                default:
                    LOG_ERR("Invalid mode: %u (valid: 0=idle, 1=sink, 2=source)", new_mode);
                    status = ATT_ERR_REQUEST_NOT_SUPPORTED;
                    ret = -1;
                    break;
                }
            ...
            if (sizeof(uint16_t) != co_buf_data_len(p_data)) {
                ...
                if (GATT_CCC_START_NTF == cfg || GATT_CCC_STOP_NTFIND == cfg) {
                    ...
                    if (cfg == GATT_CCC_START_NTF) {
                        LOG_INF("Mode status CCC enabled for connection %u", conidx);
                    } else {
                        LOG_INF("Mode status CCC disabled for connection %u", conidx);
                    }
                } else {
...
static uint16_t service_init(void)
{
    ...
    /* Log the expected handles for debugging */
    LOG_INF("Expected handles: Service=%u, ToggleChar=%u, ToggleVal=%u, StatusChar=%u, StatusVal=%u, StatusCfg=%u",
           env.start_hdl + MSS_IDX_SERVICE,
           env.start_hdl + MSS_IDX_MODE_TOGGLE_CHAR,
           env.start_hdl + MSS_IDX_MODE_TOGGLE_VAL,
           env.start_hdl + MSS_IDX_MODE_STATUS_CHAR,
           env.start_hdl + MSS_IDX_MODE_STATUS_VAL,
           env.start_hdl + MSS_IDX_MODE_STATUS_NTF_CFG);
...
int mode_switch_service_set_mode(uint8_t mode)
{
    ...
    case MODE_SWITCH_SOURCE:
        ...
        break;
    default:
        LOG_ERR("Invalid mode: %u (valid: 0=idle, 1=sink, 2=source)", mode);
        return -EINVAL;
    }
