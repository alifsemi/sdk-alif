                                ...
                                uint8_t grp_lid, uint8_t sgrp_lid)
{
    ...
    case BAP_BC_SRC_CMD_TYPE_ENABLE_PA:
        LOG_INF("Periodic advertising %s (status %u)", status ? "enable done (err)" : "enabled", status);
        if (status == 0) {
            (void)bap_bc_src_enable(source_env.bcast_grp_lid);
            source_env.state = SRC_STATE_PA_ENABLED;
        }
        break;

    case BAP_BC_SRC_CMD_TYPE_ENABLE:
        LOG_INF("Broadcast group %u %s", source_env.bcast_grp_lid, status ? "enable done (err)" : "enabled");
        if (status == 0) {
            source_env.state = SRC_STATE_GROUP_ENABLED;
            uint16_t err = bap_bc_src_start_streaming(source_env.bcast_grp_lid, 0xFFFFFFFF);
            if (err) LOG_ERR("Failed to start streaming, err %u", err);
            else     LOG_INF("Start streaming command sent");
        }
        break;
    ...
    case BAP_BC_SRC_CMD_TYPE_START_STREAMING:
        ...
        if (status == 0) {
            ...
            LOG_INF("Number of streams to start: %zu", ARRAY_SIZE(source_env.stream_ids));
            for (size_t i = 0; i < ARRAY_SIZE(source_env.stream_ids); i++) {
                LOG_INF("Starting stream %zu with ID %u", i, source_env.stream_ids[i]);
...
static int get_adv_param(bap_bc_adv_param_t *p_adv_param)
{
    ...
    if (source_env.adv_sid > (0x0F - 0x01)) source_env.adv_sid = 1;
...
static int broadcast_source_configure_group(void)
{
    ...
    LOG_INF("Broadcast group added, got local ID %u (SID=%u)", source_env.bcast_grp_lid, adv_param.adv_sid);
    ...
    err = bap_bc_src_set_subgroup(source_env.bcast_grp_lid, SUBGROUP_ID, &codec_id, sgrp_cfg, sgrp_meta);
    if (err) {
    ...
    for (size_t iter = 0; stream_bits != 0; stream_bits >>= 1, iter++) {
        ...
        err = bap_bc_src_set_stream(source_env.bcast_grp_lid, stream_id, SUBGROUP_ID, dp_id, 0, stream_cfg);
        if (err) {
            ke_free(stream_cfg);
        ...
        if (err) {
            LOG_ERR("Failed to create source channel for stream %u, err %d", stream_id, err);
            return -1;
        }
...
static int broadcast_source_enable(void)
{
    ...
    uint16_t err = bap_bc_src_enable_pa(source_env.bcast_grp_lid, sizeof(ad_data), 0,
                                        ad_data, NULL,
...
int broadcast_source_start_broadcasting(void)
{
    ...
    if (!source_env.bap_source_modules_configured) {
        LOG_ERR("BAP BC source modules not configured, call broadcast_source_configure() first");
        return -ENODEV;
    }
...
{ 
    ...
    if (source_env.state == SRC_STATE_STREAMING || source_env.state == SRC_STATE_GROUP_ENABLED ||
        source_env.state == SRC_STATE_PA_ENABLING || source_env.state == SRC_STATE_PA_ENABLED) {
        source_env.state = SRC_STATE_STOPPING_STREAMING;
        uint16_t err = bap_bc_src_stop_streaming(source_env.bcast_grp_lid, 0xFFFFFFFF);
        if (err) {
            LOG_WRN("Failed to stop streaming immediately, err %u. Proceeding to DISABLE.", err);
            source_env.state = SRC_STATE_DISABLING_GROUP;
