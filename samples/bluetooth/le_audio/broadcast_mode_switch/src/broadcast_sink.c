                                   ...
                                   uint8_t stream_pos)
{
    ...
    case BAP_BC_SINK_CMD_TYPE_ENABLE:
        LOG_INF("enable cmd complete, status %u, grp %u, stream %u", status, grp_lid, stream_pos);
        break;
...
static bool bap_modules_configured = false;
...
int broadcast_sink_stop(void)
{
    ...
    if (sink_env.grp_lid != INVALID_LID) {
        ...
        if (err != 0) {
            LOG_WRN("Failed to disable broadcast sink group after retries, err %u", err);
        } else {
