...
static gapc_pairing_keys_t peer_keys[MAX_PEERS];
static gapc_bond_data_t peer_bond_data[MAX_PEERS];
static struct peer_data peer_contexts[MAX_PEERS];

static sys_slist_t free_peers_contexts;

struct peer_data *storage_allocate_peer_context(void)
{
	struct peer_data *p_peer = (struct peer_data *)sys_slist_get(&free_peers_contexts);

	return p_peer;
}

static struct peer_data *get_free_entry_by_stored_index(uint16_t index)
{
	struct peer_data *entry, *tmp;
	sys_snode_t *prev_node = NULL;

	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&free_peers_contexts, entry, tmp, node) {
		if (entry->storage_index != index) {
			prev_node = &entry->node;
			continue;
		}

		sys_slist_remove(&free_peers_contexts, prev_node, &entry->node);
		return entry;
	}
...
void storage_free_peer_context(struct peer_data *p_peer)
{
	...
	sys_slist_append(&free_peers_contexts, &p_peer->node);
}
			     ...
			     settings_read_cb const read_cb, void *cb_arg, void *param)
{
	...
	LOG_INF("Load %s len: %d (index %d)", key, len, storage_index);
	...
	if (!node) {
		node = get_free_entry_by_stored_index(storage_index);
		if (!node) {
			LOG_ERR("Failed to allocate memory for peer data (index %d)",
				storage_index);
			return -ENOMEM;
		}
	...
	ssize_t const cb_len = read_cb(cb_arg, p_data->data, p_data->size);

	if (cb_len != len) {
		LOG_ERR("Unable to read data from storage %u != %u data len %u", cb_len, len,
			p_data->size);
		return cb_len;
	}
...
int storage_init(void)
{
	...
	sys_slist_init(&free_peers_contexts);

	for (uint16_t i = 0; i < MAX_PEERS; i++) {

		struct peer_data *p_peer = &peer_contexts[i];

		p_peer->keys.data = &peer_keys[i];
		p_peer->keys.size = sizeof(gapc_pairing_keys_t);
		p_peer->connection.data = &peer_bond_data[i];
		p_peer->connection.size = sizeof(gapc_bond_data_t);
		p_peer->actv_idx = GAP_INVALID_ACTV_IDX;
		p_peer->conidx = GAP_INVALID_CONIDX;
		p_peer->storage_index = i;
...
int storage_delete(const char *key, const int index)
{
	...
	snprintf(key_str, sizeof(key_str), SETTINGS_BASE "/%d/%s", index, key);
	err = settings_delete(key_str);
	if (err) {
		LOG_ERR("Failed to delete %s data (err %d)", key, err);
	}
	return err;
}
