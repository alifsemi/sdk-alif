...
2. Build the sample:
        ...
        To enable PTS tester feature and disable power management features, add the following to your build command:

                west build -b <your_board> samples/bluetooth/le_periph_cgms -- -DCONFIG_IUT_TESTER_ENABLED=y
...
- **Record Store Size**: When PTS feature 'CONFIG_IUT_TESTER_ENABLED' is enabled by west build command, the record store size toggles between 3 and 250. Toggle record size is done by pressing the SW0 button. A larger store is required for certain Bluetooth SIG test cases, such as:
