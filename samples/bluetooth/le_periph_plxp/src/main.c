				  ...
				  uint16_t status)
{
	...
	case GAPM_API_SEC_CONNECTED_KNOWN_DEVICE:
		...
		LOG_INF("Restoring PLXS with evt_cfg: 0x%" PRIx8, restored_evt_cfg);
		err = plxs_enable(con_idx, restored_evt_cfg);
		if (err) {
			LOG_ERR("Error 0x%" PRIx16 " enabling PLXS for known device", err);
		} else {
	...
	case GAPM_API_DEV_DISCONNECTED:
		...
		ready_to_send = false;
		features_indication_ongoing = false;
		LOG_INF("Connection index %" PRIu8 " disconnected, reason 0x%" PRIx16,
			con_idx, status);
		break;
