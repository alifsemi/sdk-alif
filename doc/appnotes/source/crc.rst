...
This sample demonstrates Zephyr Power Management (PM) states using the
Alif hardware CRC engine on RTSS cores. The application computes a CRC over a
known buffer, cycles through PM states, and repeats the CRC after
``SUSPEND_TO_IDLE``, S2RAM STANDBY, and S2RAM STOP to verify that the CRC
peripheral resumes correctly.

The following PM states are exercised. Available states are determined at
runtime by capability predicates:
...
Build the CRC power management sample for the supported HE and HP cores
...
.. code-block:: text
   ...
   *** Booting Zephyr OS build ***
   [00:00:00.000,000] <inf> crc_pm: alif_e7_dk (S2RAM): CRC PM demo (RUNTIME_IDLE, SUSPEND_TO_IDLE, S2RAM STANDBY, S2RAM STOP)
