...
These config features are already selected when building with
``-S serial-display-2lane`` or ``-S serial-display-1lane``.
...
.. code-block:: console
   ...
   *** Booting Zephyr OS build ***
   ...
   [00:00:00.272,000] <inf> disp: Panel resolution, supported formats - (480, 800), 25
   ...
   [00:00:00.272,000] <inf> disp:  (x_res, y_res) - (480, 800)
   ...
   [00:00:00.272,000] <inf> disp: FB0 - 0x02000000, size - 1152000
...
.. figure:: _static/serial_lane_1_display_output.png
   :alt: 1-lane Serial Display Output
   :align: center
...
management states combined with a CDC200 + MIPI DSI display pipeline on
...
the CDC200, DSI host, D-PHY, and panel driver all resume correctly and
...
.. note::
   Use ``serial_display_2lane.overlay`` for Ensemble (E7 and E8) and
   ``serial_display_1lane.overlay`` for Balletto (B1). Both use
   ``serial_display.conf``. Run the commands from the Zephyr repository
   so ``$PWD`` resolves those paths. For more information, refer to the
   `ZAS User Guide`_, under the section
   ``Setting Up and Building Zephyr Applications``.

HE Core — TCM boot S2RAM (Balletto B1, 1-lane)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: console
   ...
   west build -p auto \
     ...
     -DEXTRA_DTC_OVERLAY_FILE=$PWD/../alif/samples/drivers/pm/display_pm/serial_display_1lane.overlay \
     -DEXTRA_CONF_FILE=$PWD/../alif/samples/drivers/pm/display_pm/serial_display.conf

HE Core — MRAM boot SOFT_OFF (Ensemble E7, 2-lane)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: console
   ...
   west build -p auto \
     ...
     ../alif/samples/drivers/pm/display_pm/ \
     -S display-pm-mram -- \
     -DEXTRA_DTC_OVERLAY_FILE=$PWD/../alif/samples/drivers/pm/display_pm/serial_display_2lane.overlay \
     -DEXTRA_CONF_FILE=$PWD/../alif/samples/drivers/pm/display_pm/serial_display.conf

HP Core — MRAM boot SOFT_OFF (Ensemble E7, 2-lane)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
HP Core — MRAM boot SOFT_OFF (Ensemble E8, 2-lane)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
The output below is from an E7 DK HE core TCM-boot run
...
.. code-block:: console
   ...
   *** Booting Zephyr OS build 3a2b84d96961 ***
   [00:00:00.000,000] <inf> disp_pm: alif_e7_dk (S2RAM): Display PM demo (RUNTIME_IDLE, SUSPEND_TO_IDLE, S2RAM STANDBY, S2RAM STOP)
