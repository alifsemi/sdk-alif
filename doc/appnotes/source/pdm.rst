...
- **Data Line Configuration**:
  - Connect pin `P5_4` on Flat board J14 to pin `P6_2` on Flat board J12.
- **Clock Line Configuration**:
  - Connect pin `P6_7` on Flat board J15 to pin `P6_3` on Flat board J15.
...
- If `PCM_BLOCK_SIZE` is 30000 and `MEM_SLAB_NUM_BLOCKS` is 1, 30000 PCM samples can be stored in the `pcmj_data` buffer.

- If `PCM_BLOCK_SIZE` is 30000 and `MEM_SLAB_NUM_BLOCKS` is 2, 60000 PCM samples can be stored in the `pcmj_data` buffer.
...
The text below shows channels 4 and 5 enabled. The buffer address is ``0x20000e74``, and 60,000 PCM samples are stored in the ``pcmj_data`` buffer and printed.
...
correctly after each power state transition.
...
The same verification is performed after the S2RAM STOP state. The test
completes when the application displays:
