...
- The ``comp_reg1`` register controls the analog portion of all CMP instances and is available **only in the CMP0 register map**.
...
1. In ``RTE_Device.h``, configure input muxes for **CMP0**.
2. Set **positive input** via:
   ``RTE_CMP0_SEL_POSITIVE = RTE_CMP0_POSITIVE_PIN_PO_00`` → selects analog pin **P0_0**.
3. Set **negative input** via:
   ``RTE_CMP0_SEL_NEGATIVE = RTE_CMP_NEGATIVE_DAC6`` → uses **DAC6** as internal 0.8 V reference.
...
.. code-block:: console
   ...
   [00:00:02.100,000] <inf> ALIF_CMP: negative input voltage is greater than the positive input voltage
   [00:00:02.150,000] <inf> ALIF_CMP: positive input voltage is greater than negative input voltage
   [00:00:02.200,000] <inf> ALIF_CMP: negative input voltage is greater than the positive input voltage
   [00:00:02.250,000] <inf> ALIF_CMP: positive input voltage is greater than negative input voltage
   [00:00:02.300,000] <inf> ALIF_CMP: negative input voltage is greater than the positive input voltage
