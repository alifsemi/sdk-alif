...
The PWM fade runs before ``RUNTIME_IDLE`` and again after
``SUSPEND_TO_IDLE``, S2RAM STANDBY, and S2RAM STOP. This verifies that the
PWM driver suspends and resumes correctly.

HE cores support the S2RAM path when booting from TCM and the SOFT_OFF path
when booting from MRAM. HP cores use the SOFT_OFF path.
...
system resets when it wakes from ``SOFT_OFF``.
