...
def getPullRequest(Map a) {
    return request("GET", "/repos/${a.owner}/${a.repo}/pulls/${a.prNumber}",
        null, a.token)
}
...
def addLabel(Map a) {
    String argJson = withEnv(["LABEL_VALUE=${a.label}"]) { sh(script: '''#!/bin/bash
        set +x;jq -n --arg l "$LABEL_VALUE" '[$l]'
        ''', returnStdout: true).trim()
    }
...
def removeLabel(Map a) {
    def label = java.net.URLEncoder.encode(a.label, "UTF-8").replace("+", "%20")
    request("DELETE", "/repos/${a.owner}/${a.repo}/issues/${a.prNumber}/labels/${label}",
        null, a.token, 1, [404])
}
